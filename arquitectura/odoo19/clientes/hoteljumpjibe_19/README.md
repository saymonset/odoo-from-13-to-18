...
```
## Bjar los puertos , cambia el nombre del proyecto o modulo a tumbar por ejemplo: hoteljumpjibe_19 en los start_odoo.sh
```bash
...
```bash
CREATE ROLE hoteljumpjibe_19 WITH LOGIN PASSWORD '123456' CREATEDB SUPERUSER;
```
```bash
\q
\exit
...
docker exec -it odoo-db18-n8n bash
 psql -U hoteljumpjibe_19 -d postgres
 CREATE DATABASE dbhoteljumpjibe_19;
 DROP DATABASE dbhoteljumpjibe_19;
...
```bash
 ./odoo/odoo-bin -d dbhoteljumpjibe_19 -c clientes/hoteljumpjibe_19/conf/odoo.cfg --dev=all
```
# actualizar modulo
```bash
 ./odoo/odoo-bin -d dbhoteljumpjibe_19 -c clientes/hoteljumpjibe_19/conf/odoo.cfg -u all
 ./odoo/odoo-bin -d dbhoteljumpjibe_19 -c clientes/hoteljumpjibe_19/conf/odoo.cfg --dev=all -u web -u website --stop-after-init
```
# Accedemos
```bash
...
```bash
 ./odoo/odoo-bin --test-enable --stop-after-init  -d dbhoteljumpjibe_19 -i a_hospital_19 -c clientes/hoteljumpjibe_19/conf/odoo.cfg 
 ```
# Coloca lo que haga en los fuentes a los addons de docker
```bash
cd /root/odoo/odoo-from-13-to-18/arquitectura/odoo19/clientes/hoteljumpjibe_19/extra-addons/extra
```
```bash
