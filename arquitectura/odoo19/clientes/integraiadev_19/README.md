...
```
## Bjar los puertos , cambia el nombre del proyecto o modulo a tumbar por ejemplo: integraiadev_19 en los start_odoo.sh
```bash
...
```bash
CREATE ROLE integraiadev_19 WITH LOGIN PASSWORD '123456' CREATEDB;
...
```bash
 ...
 ./odoo/odoo-bin -d dbintegraiadev_19 -c clientes/integraiadev_19/conf/odoo.cfg --update=web --stop-after-init
```
# actualizar modulo
```bash
...
```bash
 ./odoo/odoo-bin --test-enable --stop-after-init  -d dbintegraiadev_19 -i a_hospital_19 -c clientes/integraiadev_19/conf/odoo.cfg 
 ```
# Coloca lo que haga en los fuentes a los addons de docker
```bash
cd /root/odoo/odoo-from-13-to-18/arquitectura/odoo19/clientes/integraiadev_19/extra-addons/extra
```
```bash
