...
```bash
./odoo/odoo-bin --addons-path=/root/odoo/odoo-from-13-to-18/arquitectura/odoo19/clientes/saymon_19/extra-addons/extra -d dbcontabo19 --i18n-export=clientes/saymon_19/extra-addons/extra/evolution-api/i18n/evolution_api.pot --modules=evolution-api -c clientes/saymon_19/conf/odoo.cfg
```
...
./odoo/odoo-bin --addons-path=/root/odoo/odoo-from-13-to-18/arquitectura/odoo19/clientes/saymon_19/extra-addons/extra -d dbcontabo19 -u evolution-api -c clientes/saymon_19/
conf/odoo.cfg
# FIN MULTI LENGUAJE
...
```bash
CREATE ROLE saymon_19 WITH LOGIN PASSWORD '123456' CREATEDB;
 ALTER USER saymon_19 WITH SUPERUSER;
```
...
```bash
CREATE ROLE saymon_19 WITH LOGIN PASSWORD '123456' CREATEDB SUPERUSER;
```
```bash
\q
\exit
# Dentro de odoo-db18-n8n   ( docker exec -it odoo-db18-n8n bash )
 psql -U saymon_19 -d postgres
 CREATE DATABASE dbsaymon_19;
 ```
...
# Inicializamos BD
```bash
./odoo/odoo-bin -d dbsaymon_19 -i base -c clientes/saymon_19/conf/odoo.cfg
```
...
```bash
 ./odoo/odoo-bin -d dbsaymon_19 -c clientes/saymon_19/conf/odoo.cfg --dev=all
```
# actualizar modulo
```bash
 ./odoo/odoo-bin -d dbsaymon_19 -c clientes/saymon_19/conf/odoo.cfg -u all
```
# Accedemos
```bash
...
```bash
 ./odoo/odoo-bin --test-enable --stop-after-init  -d dbsaymon_19 -i a_hospital_19 -c clientes/saymon_19/conf/odoo.cfg 
 ```
# Coloca lo que haga en los fuentes a los addons de docker
```bash
cd /root/odoo/odoo-from-13-to-18/arquitectura/odoo19/clientes/saymon_19/extra-addons/extra
```
```bash
