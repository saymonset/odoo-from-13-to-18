...
class SessionState(models.Model):
    ...
    def procesar_paso(self, session_id, valor, paso):
        ...
        if not registro.pasos_pendientes:
            return {'success': False, 'error': 'No hay pasos pendientes'}
        ...
        if nuevos_pasos:
            ...
            estado_actual.update({
                'paso': siguiente.get('nombre_interno'),
                'nombre_mostrar': siguiente.get('nombre_mostrar'),
                'tipo_dato': siguiente.get('tipo_dato'),
                'mensaje_prompt': siguiente.get('mensaje_prompt'),
