...
class ThemePrimeWebsiteSale(WebsiteSale):
    def _get_search_domain(
        ...
        if ratings and search_rating:
            ...
            for rating in ratings:
                rating_product_ids.extend(
                    [item["res_id"] for item in result if item["rating"] >= int(rating)]
                )
    ...
    def shop(
        ...
        **post,
        ...
        if theme_id and theme_id.name.startswith("theme_prime"):
            ...
            pricelist, category = (
                response.qcontext.get("pricelist"),
                response.qcontext.get("category"),
            )
            if request.website._get_dr_theme_config("json_shop_filters")[
                "show_category_count"
            ]:
                # Categories
                domain = self._prepare_filters_domain(
                    search=fuzzy_search_term,
                    attrib_values=attrib_values,
                    pricelist=pricelist,
                    min_price=min_price,
                    max_price=max_price,
                ...
                response.qcontext.update(
                    get_category_count=ProductTemplate._get_product_category_count(
                        domain=domain
                    )
                )
            if (
                request.website._get_dr_theme_config("json_shop_filters")[
                    "show_attribute_count"
                ]
                or request.website._get_dr_theme_config("json_shop_filters")[
                    "hide_extra_attrib_value"
                ]
            ):
                # Attributes
                domain = self._prepare_filters_domain(
                ...
                response.qcontext.update(
                    get_attrib_count=ProductTemplate._get_product_attrib_count(
                        attrib_values, domain=domain
                    )
                )
                ...
                response.qcontext.update(
                    get_ratings_count=ProductTemplate._get_product_rating_count(
                        domain=domain
                    )
                )
            ...
            _config_shop_layout = request.website._get_dr_theme_config(
                "json_shop_layout"
            )
            ...
            response.qcontext.update(
                ...
                _config_product_item=request.website._get_dr_theme_config(
                    "json_shop_product_item"
                ),
                _config_shop_filters=request.website._get_dr_theme_config(
                    "json_shop_filters"
                ),
                _config_category_pills=request.website._get_dr_theme_config(
                    "json_shop_category_pills"
                ),
                view_mode=request_args.get(
                    "view_mode", _config_shop_layout.get("default_view_mode")
                ),
                page=page,
                attributes_ids=attributes_ids,
                selected_ratings=selected_ratings,
                selected_hide_out_of_stock=selected_hide_out_of_stock,
                selected_attributes=response.qcontext.get("attrib_set")
                or request_args.get("min_price")
                or request_args.get("max_price")
                or response.qcontext.get("tags")
                or selected_ratings
                or selected_hide_out_of_stock,
            )
        return response

    def _prepare_filters_domain(
        self,
        search=None,
        pricelist=None,
        category=None,
        ...
        **post,
        ...
        search_detail = request.website._search_get_details(
            "products_only", None, options
        )[0]
        search_fields = search_detail["search_fields"]
        base_domain = search_detail["base_domain"]
        return request.env["product.template"]._search_build_domain(
            base_domain, search, search_fields, search_detail.get("search_extra")
        )
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        if request.website._get_dr_theme_config("bool_show_products_nav"):
            ProductTemplate = request.env["product.template"]
            res["prev_product_id"] = ProductTemplate.search(
                [("website_sequence", "<", product.website_sequence)]
                + request.website.website_domain(),
                limit=1,
                order="website_sequence desc",
            )
            res["next_product_id"] = ProductTemplate.search(
                [("website_sequence", ">", product.website_sequence)]
                + request.website.website_domain(),
                limit=1,
    ...
    @http.route(
        "/theme_prime/get_search_sidebar",
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    @http.route(
        "/theme_prime/get_categories_list",
        type="json",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def _get_categories_list(self, **post):
        return {
            "categories": request.env["product.public.category"].search_read(
                request.website.website_domain(), ["name", "parent_id", "child_id"]
            ),
            "category_count": request.env[
                "product.template"
            ]._get_product_category_count(domain=request.website.website_domain())
            if request.website._get_dr_theme_config("json_sidebar_config").get(
                "category_sidebar_show_count"
            )
            else {},
        }

    @http.route(
        "/theme_prime/get_quick_view_html", type="json", auth="public", website=True
    )
    def get_quick_view_html(self, options, **kwargs):
        ...
        domain = expression.AND(
            [request.website.sale_product_domain(), [("id", "=", product_tmpl_id)]]
        )
        ...
        if options.get("variant_selector"):
            ...
            return IrUiView._render_template(
                "theme_prime.product_variant_selector_dialog", values=values
            )

        if options.get("right_panel"):
            return IrUiView._render_template(
                "theme_prime.tp_product_right_panel", values=values
            )
        return IrUiView._render_template(
            "theme_prime.tp_product_quick_view", values=values
        )
            ...
            response.update(
                {
                    ...
                    and product.accessory_product_ids.mapped("product_tmpl_id").ids
                    or [],
                }
            )
    ...
    def clear_cart(self):
        ...
        return {
            "notification_info": {
                "order_amount_html": request.env[
                    "ir.qweb.field.monetary"
                ].value_to_html(
                    order.amount_total,
                    {
                        "display_currency": order.pricelist_id.currency_id
                        or request.website.company_id.currency_id
                    },
                )
            }
        }
    ...
    def _get_cart_notification_information(self, order, line_ids):
        ...
        values["order_amount_html"] = request.env[
            "ir.qweb.field.monetary"
        ].value_to_html(
            order.amount_total,
            {
...
class DroggolComboConfiguratorController(WebsiteSaleComboConfiguratorController):
    ...
    def website_sale_combo_configurator_update_cart(self, *args, **kwargs):
        ...
        if (
            theme_id
            and theme_id.name.startswith("theme_prime")
            and request.website._get_dr_theme_config("cart_flow") != "default"
        ):
...
class DroggolWishlist(WebsiteSaleWishlist):
    @http.route(
        "/theme_prime/wishlist_general", auth="public", type="json", website=True
    )
    def wishlist_general(self, product_id=False, **post):
        ...
        res.update(
            {
                "products": request.env["product.wishlist"]
...
class ThemePrimeMainClass(http.Controller):
    ...
    def _get_products(self, domain=None, fields=[], limit=25, order=None, options={}):
        ...
        products = (
            request.env[res_model]
            .with_context(pricelist=pricelist.id)
            .search(final_domain, limit=limit, order=order)
        )
        ...
        if bestseller_ids:
            ...
            bestseller_sorted_ids = [
                p_id for p_id in bestseller_ids if p_id in bestseller_ids_filtered
            ]
            ...
            products = (
                request.env[res_model]
                .with_context(pricelist=pricelist.id)
                .browse(final_product_ids[:old_limit])
            )
    ...
    def _get_last_viewed_products(self, domain, res_model, limit):
        domain = expression.AND(
            [
                ...
                request.website.get_current_website().website_domain(),
                domain,
            ]
        )
        products = request.env["website.snippet.filter"]._get_products_latest_viewed(
            request.website, limit, domain
        )
        return (
            products.mapped("product_tmpl_id").ids
            if len(products) and res_model == "product.template"
            else products.ids
        )
    ...
    def _prepare_product_data(self, products, fields, pricelist, options=None):
        ...
        if not price_public_visibility:
            visibility_label = self._get_tp_view_template(
                "theme_prime.tp_b2b_price_label"
            )
        ...
        if "dr_stock_label" in extra_data:
            showStockLabel = request.website._get_dr_theme_config(
                "json_shop_product_item"
            ).get("show_stock_label")
        ...
        for res_product, product in zip(result, products):
            if res_model == "product.product":
                ...
                res_product["name"] = product.with_context(
                    display_default_code=False
                ).display_name
            else:
                combination_info = product._get_combination_info(only_template=True)
            res_product.update(combination_info)
            price_info = self._get_computed_product_price(
                product,
                res_product,
            ...
            res_product["product_variant_id"] = (
            ...
            if "description_ecommerce" in fields:
                ...
                res_product["short_description"] = (
                    description[:100] + "..."
                    if description and len(description) > 100
                    else description or False
                )
            # label and color
            if "colors" in extra_data:
                res_product["colors"] = self._get_tp_view_template(
                    "theme_prime.product_preview_swatches",
                    {
                        "product": product
                        if res_model == "product.template"
            ...
            if "dr_label_id" in fields and product.dr_label_id:
                ...
                res_product["label_template"] = self._get_tp_view_template(
                    "theme_prime.product_label", {"label": product.dr_label_id}
                )
            if (
                "dr_stock_label" in extra_data
                and showStockLabel
                and product.dr_show_out_of_stock
            ):
                res_product["dr_stock_label"] = self._get_tp_view_template(
                    "theme_prime.product_label", {"product": product, "stock": True}
                )
    ...
    def _get_computed_product_price(
        self,
        product,
        product_data,
        price_public_visibility,
        ...
        return {
            ...
            if price_public_visibility
            else visibility_label,
            "list_price_raw": product_data["list_price"]
            if price_public_visibility
            else " ",
            "price": FieldMonetary.value_to_html(
                product_data["price"], monetary_options
            )
            if price_public_visibility
            else visibility_label,
            "list_price": FieldMonetary.value_to_html(
                product_data["list_price"], monetary_options
            )
            if price_public_visibility
            else " ",
        }
    ...
    def _get_bestseller_products(self, old_limit):
        ...
        result = (
            ...
            .read_group(
                ...
                ["product_tmpl_id", "product_uom_qty:sum"],
                ["product_tmpl_id"],
                orderby="product_uom_qty desc",
            )
        )
        return (
            [product_line["product_tmpl_id"][0] for product_line in result],
            None if len(result) else old_limit,
            None,
        )
    ...
    def _get_shop_related_data(self, options):
        ...
        if options.get("shop_config_params"):
            shop_data["shop_config_params"] = (
                request.website.get_theme_prime_shop_config()
            )
        if (options.get("wishlist_enabled")) and shop_data.get(
            "shop_config_params", {}
        ).get("is_wishlist_active"):
            shop_data["wishlist_products"] = (
    ...
    def _get_rating_template(self, rating_avg, rating_count=False):
        return request.website._get_theme_prime_rating_template(
    ...
    def _get_categories(
        ...
        if extras.get("count", False):
            count_dict = {
                k: v
                for k, v in sorted(
                    self.get_categories_count().items(),
        ...
        if order == "count":
            ...
            final_list = [i for i in category_ids if i not in extras.get("ids", [])][
                :limit
            ]
            final_domain = expression.AND([[["id", "in", final_list]], final_domain])
        result = request.env["product.public.category"].search_read(
            final_domain, fields=fields + ["display_name"], limit=limit, order=order
        )
        for res in result:
            if extras.get("count", False):
    ...
    def _get_products_for_top_categories(self, params):
        ...
        initial_domain = expression.AND(
            [request.website.website_domain(), [("website_published", "=", True)]]
        )
        for id in categoryIDs:
            domain = expression.AND(
                [initial_domain, [["public_categ_ids", operator, id]]]
            )
        ...
        pricelists = request.env["product.pricelist"].search(
            [
                (
                    "id",
                    "in",
                    request.website.get_pricelist_available(show_visible=True).ids,
                )
            ],
            limit=limit,
            order=order,
            ...
            data.update(
                {
                    ...
                    "title": "Pricelist",
                    "subtitle": data.get("name"),
                }
            )
            result.append(data)
        return {
            "active_pricelist": request.website.pricelist_id.id,
            "pricelists": result,
        }

    @http.route(
        "/theme_prime/tp_create_collection",
        type="json",
        auth="user",
        website=True,
        sitemap=False,
    ...
    def _tp_search_read(
        self, model, domain=None, fields=[], order=None, limit=20, extras={}, **post
    ):
        ...
        if model in ["product.template", "product.product"]:
            ...
            if extras.get("brands"):
                ...
                domain = expression.AND(
                    [domain, [("attribute_id", "in", brand_attributes.ids)]]
                )
                return (
                    request.env[model]
                    .with_context(show_attribute=False)
                    .search_read(domain, fields=fields, limit=limit, order=order)
                )
            ...
            return self._get_pricelist_data(
                fields=fields, limit=limit, order=order, extras=extras
            )
            ...
            if snippetCollections and extras.get("unlink", False):
                ...
                return []
            data = []
            related_field = snippetCollectionsModel._dr_get_related_field(
                extras.get("dr_res_model")
            )
            ...
            views = (
                request.env["ir.ui.view"].sudo().search([("key", "in", templateIDs)])
            )
            ...
            for tmpl in templateIDs:
                ...
                if view:
                    ...
                    if (
                        extras.get("registryToUse") == "theme_prime_snippet_registry"
                        and imgSrc == "s_mega_menu_category_tabs_snippet"
                    ):
                    ...
                    viewData.append(
                        {
                            ...
                            "html": Qweb._render(
                                view.id,
                                {"dr_uid": datetime.datetime.now().microsecond},
                                minimal_qcontext=True,
                            ),
                            "subtitle": snippetName,
                            "imgSrc": "/theme_prime/static/src/img/dynamic_snippet/%s.jpg"
                            % imgSrc,
    ...
    @http.route(
        "/theme_prime/get_similar_products_sidebar",
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def _get_similar_products_sidebar(self, productID, **post):
        ...
        return request.render(
            "theme_prime.similar_products_sidebar",
            {"products": product_tmpl_id.alternative_product_ids},
            headers={"Cache-Control": "no-cache"},
        )

    @http.route(
        "/theme_prime/get_tab_listing_products",
        type="json",
        auth="public",
        website=True,
    )
    def get_tab_listing_products(
    ...
    def get_brands_category_data(self, domain=None, fields=[], options={}):
        if options.get("model") in [
            ...
            if options.get("model") == "product.attribute.value":
                get_brand_count = request.env[
                    "product.template"
                ]._get_product_attrib_count(
                    attrib_values=[],
    ...
    @http.route(
        "/theme_prime/get_listing_products", type="json", auth="public", website=True
    )
    def get_listing_products(
        self, domain=None, fields=[], options={}, limit=5, **kwargs
    ):
        result = {}
        ...
        if options.get("bestseller"):
            result["bestseller"] = self._get_products(
                domain, fields, limit, "bestseller"
            )
        if options.get("newArrived"):
            result["newArrived"] = self._get_products(
                domain, fields, limit, "create_date desc"
            )
    ...
    @http.route(
        "/theme_prime/get_products_data", type="json", auth="public", website=True
    )
    ...
    @http.route(
        "/theme_prime/get_products_by_category",
        type="json",
        auth="public",
        website=True,
    )
    def get_products_by_category(self, domain, fields=[], options={}, **kwargs):
        ...
        if options.get("get_brands"):
            # get category names for snippet
            domain = [("id", "in", options.get("categoryIDs"))]
            result["categories"] = request.website._get_brands(domain).read(
                ["name", "id"]
            )
        return result

    @http.route(
        "/theme_prime/get_top_categories", type="json", auth="public", website=True
    )
    def get_top_categories(self, options={}):
        ...
        if params:
            ...
            if categoryIDs:
                ...
                category_names = {
                    i["id"]: i["name"] for i in self._get_categories(domain)
                }
                ...
                for category_id in categoryIDs:
                    ...
                    products = self._get_products(
                        domain=final_domain, fields=[], limit=1, order="list_price asc"
                    )
                    if len(products):
                        category_data["min_price"] = products[0].get("price")
                        category_data["price_public_visibility"] = (
                            price_public_visibility
                        )
    ...
    @http.route(
        ["/theme_prime/get_dialog_content"], type="json", website=True, auth="public"
    )
    def get_dialog_content(self, res_id, res_model, fields, **post):
        return (
            request.env[res_model]
            .sudo()
            .search_read([("id", "=", res_id)], fields=fields)
        )

    @http.route(
        "/theme_prime/get_categories_count", type="json", auth="public", website=True
    )
    def get_categories_count(self):
        return request.env["product.template"]._get_product_category_count(
            domain=request.env["website"].sale_product_domain()
        )

    @http.route(
        "/theme_prime/get_categories_info", type="json", auth="public", website=True
    )
    def get_categories_info(self, fields=[], options={}, **kwargs):
        categoryIDs = options.get("categoryIDs", [])
        fields = ["name", "display_name", "id"] + fields
        domain = expression.AND(
            [request.website.website_domain(), [("id", "in", categoryIDs)]]
        )
        ...
        for res_category, category in zip(result, categories):
            ...
            res_category["image_url"] = (
                "/web/image?model=product.public.category&id=%d&field=image_512"
                % (category.id)
            )
            res_category["cover_image"] = (
                "/web/image?model=product.public.category&id=%d&field=dr_category_cover_image"
                % (category.id)
            )
        return result
        ...
        if options.get("categories"):
            domain = expression.AND(
                [
                    ...
                    [("public_categ_ids", "child_of", options.get("categories"))],
                ]
            )
            brands = (
                request.env["product.template"]
                .search(domain)
                .mapped("dr_brand_value_id")[: options.get("limit", 12)]
            )
        if options.get("recordsIDs"):
            brands = request.website._get_brands(
                [("id", "in", options.get("recordsIDs"))], options.get("limit", 12)
            )
            domain = request.env["website"].sale_product_domain()
        get_attrib_count = request.env["product.template"]._get_product_attrib_count(
            attrib_values=[], domain=domain
        )
        return [
            {**brand_data, "product_count": get_attrib_count.get(brand_data["id"], 0)}
            for brand_data in brands.read(fields)
        ]

    @http.route(
        "/theme_prime/get_preview_body",
        type="http",
        auth="user",
        website=True,
        sitemap=False,
    ...
    def get_preview_body(self, **kw):
        return request.env["ir.ui.view"]._render_template(
            "theme_prime.tp_preview_body", {"mobile": kw.get("mobile")}
        )

    @http.route(
        "/theme_prime/get_megamenu_categories", type="json", auth="public", website=True
    )
    def get_megamenu_categories(
        self, options={}, limit=5, fields=["name", "id"], order="count", **kwargs
    ):
        category_ids = (
            request.env["product.public.category"]
            .browse(options.get("categoryIDs", []))
            .exists()
            .ids
        )
        final_domain = expression.AND(
            [
                request.website.website_domain(),
                [("parent_id", "child_of", category_ids)],
            ]
        )
        categories = request.env["product.public.category"].search(
            final_domain, limit=None
        )

        all_categories = []
        all_category_count = request.env[
            "product.template"
        ]._get_product_category_count(request.env["website"].sale_product_domain())
            ...
            all_categories.append(
                {
                    ...
                    "image_url": "/web/image?model=product.public.category&id=%d&field=image_512"
                    % (category.id),
                    "cover_image": "/web/image?model=product.public.category&id=%d&field=dr_category_cover_image"
                    % (category.id),
                    "dr_category_icon": "/web/image?model=product.public.category&id=%d&field=dr_category_icon"
                    % (category.id),
                    "category_label_info": category.dr_category_label_id
                    and {
                        f: category.dr_category_label_id[f]
                        for f in ["name", "background_color", "text_color"]
                    }
                    or False,
                }
            )
        ...
        parent_categories = filter(
            lambda category: category.get("id") in category_ids, all_categories
        )
        return [
            self._get_megamenu_child_categories(
                category_id, limit, all_categories, order, options
            )
            for category_id in parent_categories
        ]

    def _get_megamenu_child_categories(
        self, parent_category, limit, all_categories, order, options
    ):
        child_categories = [
            categ
            for categ in all_categories
            if categ.get("parent_id") == parent_category.get("id")
        ]
        ...
        for child_category in child_categories[:]:
            new_born_child = self._get_megamenu_child_categories(
                child_category, remain_limit, all_categories, order, options
            ).get("child")
...
class ThemeWebsite(Website):
    @http.route(
        "/website/dr_search", type="json", auth="public", website=True, sitemap=False
    )
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        results = {
            search_type: {"results": [], "results_count": 0, "parts": {}}
        ...
        options = {
            "allowFuzzy": False,
            "displayDescription": search_description,
            "displayDetail": True,
        ...
        if fuzzy_enabled:
            search_details = request.website._search_get_details(
                "products", "name asc", {**options, "allowFuzzy": True}
            )
            term = (
                request.website._search_find_fuzzy_term(search_details, raw_term)
                or raw_term
            )
        ...
        if product_limit:
            results["products"] = self.autocomplete(
        ...
        def _match_categories(trm, p_limit, match_any_word=None):
            category_result = self._match_category(
                term=trm, limit=pills_limit, parts=True, match_any_word=match_any_word
            )
            ...
            return {
                "results": [
                    {
                        "id": categ[0].id,
                        "name": self.format_result(categ[1], categ[0].ds_name),
                    }
                    for categ in category_result
                ],
                "results_count": len(category_result),
                "parts": {},
            }, p_limit
        ...
        def _match_brands(trm, p_limit, match_any_word=None):
            ...
            brand_attributes = request.website._get_brand_attributes()
            trm_domain = (
                expression.OR([[("ds_name", "ilike", tm)] for tm in splited_terms])
                if match_any_word
                else [("ds_name", "ilike", trm)]
            )
            matched_brands = request.env["product.attribute.value"].search(
                expression.AND(
                    [trm_domain, [("attribute_id", "in", brand_attributes.ids)]]
                )
            )
            return {
                "results": [
                    {
                        "id": brand.id,
                        "attribute_id": brand.attribute_id.id,
                        "name": self.format_result(splited_terms, brand.ds_name),
                    }
                    for brand in matched_brands
                ],
                "results_count": len(matched_brands),
                "parts": {},
            }
        ...
        if pills_limit == raw_pills_limit and search_config.get("search_suggestion"):
            suggest_term = " ".join(
                [w for w in term.split() if len(w) > 2]
            )  # we only suggest if string is more then 2 char
            if suggest_term and search_config.get("search_category"):
                results["categories"], pills_limit = _match_categories(
                    suggest_term, pills_limit, match_any_word=True
                )
            if suggest_term and pills_limit and search_config.get("search_brand"):
                results["brands"] = _match_brands(
                    suggest_term, pills_limit, match_any_word=True
                )

        # suggestion search
        remain_limit = max(min(search_config.get("search_limit"), 10), 5)
        if search_config.get("search_attribute") or search_config.get(
            "search_suggestion"
        ):
            ...
            for word in words:
                if matchs:
                    for match in matchs:
                        ...
                        if match_dict["remaining_words"]:
                            ...
                            regex_match = re.search(
                                re.escape(word), unmatched_record_name, re.IGNORECASE
                            )
                            if regex_match:
                                match_dict["matched_words"].append(word)
                                match_dict["unmatched_record_name"] = re.sub(
                                    re.escape(word),
                                    " ",
                                    unmatched_record_name,
                                    flags=re.I,
                                )
                            else:
                                match_dict["remaining_words"].append(word)
                    ...
                    if matchs:
                        for match in matchs:
                            matched_dicts[match] = match_dict = {
                                "match": match,
                                "matched_words": [word],
                                "remaining_words": [],
                            }
                            match_dict["unmatched_record_name"] = re.sub(
                                re.escape(match_dict["matched_words"][0]),
                                " ",
                                match.ds_name,
                                flags=re.I,
                            )
            ...
            for match_dict in match_list:
                autocomplete_data = []
                if match_dict["remaining_words"]:
                    autocomplete_data = self._get_autocomplete_data(
                        match_dict, remain_limit, search_config
                    )
                elif (
                    not search_config.get("search_category")
                    and match_dict["match"]._name == "product.public.category"
                ):
                    autocomplete_result.append(
                        self.generate_result_dict(
                            match_dict["match"], False, match_dict["matched_words"], ""
                        )
                    )
                    remain_limit -= 1
                    if not remain_limit:
                ...
                if not match_dict["remaining_words"]:
                    ...
                    autocomplete_data += self._get_suggestions_data(
                        match_dict,
                        autocomplete_result,
                        remain_limit,
                        search_config,
            ...
            suggestions_result = []
            for match_dict in match_list:
                suggestions_data = self._get_suggestions_data(
                    match_dict, autocomplete_result, min(remain_limit, 5), search_config
                )
            ...
            results["autocomplete"] = {
                "results": autocomplete_result,
                "results_count": len(autocomplete_result),
                "parts": {"name": True, "website_url": True},
            }
            results["suggestions"] = {
                "results": suggestions_result,
                "results_count": len(suggestions_result),
                "parts": {"name": True, "website_url": True},
            }
            ...
            if (
                ...
                and len(matchs) == 1
                and (
                    results["autocomplete"].get("results_count")
                ...
                if matchs[0]._name == "product.public.category":
                    ...
                    global_match = {
                        "name": f'{fixed_str} <b class="">{matchs[0].ds_name}</b>',
                        "website_url": f"/shop?category={matchs[0].id}",
                    }
                else:
                    fixed_str = _("View all products with")
                    global_match = {
                        "name": f'{fixed_str} {matchs.attribute_id.name.lower()} <b class="">{matchs.ds_name}</b>',
                        "website_url": f"/shop?&attribute_value={matchs.attribute_id.id}-{matchs.id}",
                    }

        # fill in the extra products is keyword does not match with normal products
        result_for_autocomplete = (
            results["autocomplete"]["results"] or results["suggestions"]["results"]
        )
        if (
            search_config.get("search_fill_products")
            and results["products"]["results_count"] == 0
            and result_for_autocomplete
        ):
            domain_all = []
            for autocomplete_dict in result_for_autocomplete:
                domain, catgory_id, attribute_value_od = (
                    [],
                    autocomplete_dict.get("category"),
                    autocomplete_dict.get("attribute"),
                )
                if catgory_id:
                    domain.append(("public_categ_ids", "child_of", [catgory_id]))
                if attribute_value_od:
                    domain.append(
                        ("attribute_line_ids.value_ids", "in", [attribute_value_od])
                    )
        ...
        search_result = {
            **results,
            ...
            "result_length": sum(
                [
                    results.get(r_type, {}).get("results_count", 0)
                    for r_type in search_types
                ]
            ),
        }

        if search_config.get("search_report") and search_result["result_length"] == 0:
            report_data = {"search_term": term, "device_type": kw.get("device_type")}
    ...
    def _get_autocomplete_data(self, match_dict, remain_limit, search_config):
        match, remaining_words, matched_words = (
            match_dict["match"],
            match_dict["remaining_words"],
            match_dict["matched_words"],
        )
        results = []
        if search_config.get("search_attribute") and remaining_words and match:
            for related_match, word in self.match_remaining_words(
                match, remaining_words
            ):
                results.append(
                    self.generate_result_dict(match, related_match, matched_words, word)
                )
    ...
    def _get_suggestions_data(
        ...
        if (
            search_config.get("search_suggestion") or ignore_config
        ) and remain_limit > 0:
            if match._name == "product.public.category":
                for related_match in self._category_counterpart_iterator(
                    match, search_type=["auto_suggestion"]
                ):
                    term = self.generate_result_dict(
                        match, related_match, matched_words
                    )
                ...
                domain = request.website.sale_product_domain() + [
                    ("attribute_line_ids.value_ids", "in", match.ids)
                ]
                all_related_records = (
                    request.env["product.template"]
    ...
    def _match_category(
        self, term=False, categories_ids=[], limit=None, parts=None, match_any_word=None
    ):
    ...
    def _match_attr(self, term):
        ...
        if all_active_attributes:
            matched_values = matched_values.search(
                [
                    ("ds_name", "ilike", term),
                    ("attribute_id", "in", all_active_attributes.ids),
                ]
            )
    ...
    def _website_active_attributes(self):
        all_products = (
            request.env["product.template"]
            .with_context(bin_size=True)
            .search(request.website.sale_product_domain())
        )
        return request.env["product.attribute"].search(
            [
                ("product_tmpl_ids", "in", all_products.ids),
                ("visibility", "=", "visible"),
                ("dr_search_suggestion", "!=", False),
            ]
        )
    ...
    def match_remaining_words(self, match, remaining_words):
        if match._name == "product.public.category":
            for word in remaining_words:
                for attribute_value in self._category_counterpart_iterator(match):
                    regex_match = re.search(
                        re.escape(word), attribute_value.ds_name, re.IGNORECASE
                    )
                    if regex_match:
        ...
        if match._name == "product.attribute.value":
            ...
            for word in remaining_words:
                for category in categories:
                    regex_match = re.search(
                        re.escape(word), category.ds_name, re.IGNORECASE
                    )
                    if regex_match:
                        yield category, word

    def _category_counterpart_iterator(
        self, category_dst, search_type=["auto_suggestion", "auto"]
    ):
        category = request.env["product.public.category"].browse(category_dst.id)
        attribute_values = (
            category.mapped("product_tmpl_ids.attribute_line_ids")
            .filtered(
                lambda line: line.attribute_id.dr_search_suggestion in search_type
            )
            .mapped("value_ids")
        )
        ...
        # Child category
        child_categories = (
            request.env["product.public.category"].search(
                [("parent_id", "child_of", category.id)]
            )
            - category
        )
        child_attribute_values = (
            child_categories.mapped("product_tmpl_ids.attribute_line_ids")
            .filtered(
                lambda line: line.attribute_id.dr_search_suggestion in search_type
            )
            .mapped("value_ids")
        )  # plus to maintain order
        for value in child_attribute_values:
            if value not in attribute_values:
                yield value

    def generate_result_dict(
        self, primary_match, secondary_match, matched_words, word=False
    ):
        category, attribute = (
            (primary_match, secondary_match)
            if primary_match._name == "product.public.category"
            else (secondary_match, primary_match)
        )
        attribute_str = (
            f"&attribute_value={attribute.attribute_id.id}-{attribute.id}"
            if attribute
            else ""
        )  # just for category
        return {
            "name": self.format_result(
                matched_words + (word and [word] or []),
                f"{primary_match.ds_name} {secondary_match and secondary_match.ds_name or ''}",
            ),
    ...
    def format_result(self, matched_words, value):
        ...
        if len(parts) > 1:
            ...
            html_val = request.env[("ir.qweb.field.html")].value_to_html(
    ...
    def is_search_added(self, autocomplete_result, suggestions_results, new_term):
        auto_found = len(
            [
                term
                for term in autocomplete_result
                if new_term["website_url"] == term["website_url"]
            ]
        )
        sugg_found = len(
            [
                term
                for term in suggestions_results
                if new_term["website_url"] == term["website_url"]
            ]
        )
    ...
    def _dr_has_formulate(self, search_config):  # for performance
        if search_config.get("search_attribute") or search_config.get(
            ...
            formulate_category = request.env["product.public.category"].search(
                (
                    [("dr_search_formulate", "=", True)]
                    + request.website.website_domain()
                ),
                limit=1,
            )
...
class ThemePrimeBrand(http.Controller):
    @http.route(
        "/shop/all-brands", type="http", auth="public", website=True, sitemap=True
    )
    def brands(self, search="", **args):
        is_disable_grouping = request.website._get_dr_theme_config("json_brands_page")[
            "disable_brands_grouping"
        ]
        brands = request.website._get_brands(
            [("name", "ilike", search)] if search else []
        )
        get_brand_count = request.env["product.template"]._get_product_attrib_count(
            attrib_values=[], domain=request.env["website"].sale_product_domain()
        )
...
class ThemePrimePWA(http.Controller):
    @http.route(
        "/pwa/<int:website_id>/manifest.json", type="http", auth="public", website=True
    )
    def get_pwa_manifest(self, website_id, **kargs):
        ...
        if website and website.id == website_id and website.dr_pwa_activated:
            manifest_data = {
                ...
                "icons": [
                    {
                        "src": "/web/image/website/%s/dr_pwa_icon_192/192x192"
                        % website.id,
                        "sizes": "192x192",
                        "type": "image/png",
                    },
                    {
                        "src": "/web/image/website/%s/dr_pwa_icon_512/512x512"
                        % website.id,
                        "sizes": "512x512",
                        "type": "image/png",
                    },
                ...
                manifest_data["shortcuts"] = [
                    {
                        ...
                        "icons": [
                            {
                                "src": "/web/image/dr.pwa.shortcuts/%s/icon/192x192"
                                % shortcut.id,
                                "sizes": "192x192",
                            }
                        ],
    ...
    @http.route(
        "/service_worker.js", type="http", auth="public", website=True, sitemap=False
    )
    def get_pwa_service_worker(self, **kargs):
        website = request.website
        offline_bool = "true" if website.dr_pwa_offline_page else "false"
        data = (
            file_open("theme_prime/static/src/js/pwa/service_worker.js", "rb")
            .read()
            .decode()
        )
        ...
        return request.make_response(
            data=data, headers=[("Content-Type", "text/javascript")]
        )
    ...
    def get_pwa_logo(self, **kargs):
        ...
        if not website.logo:
            ...
            response = send_file(
                ...
                mimetype=mimetype,
                last_modified=website.write_date,
            )
        return response
