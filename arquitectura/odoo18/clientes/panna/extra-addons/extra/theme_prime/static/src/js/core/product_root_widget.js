...
export default RootWidget.extend(cartMixin, CartManagerMixin, MarkupRecords, {
  ...
  _modifyElementsAfterAppend: function () {
    ...
    this.wishlistProductIDs.forEach((id) => {
      this.$('.d_add_to_wishlist_btn[data-product-product-id="' + id + '"]')
        .prop("disabled", true)
        .addClass("disabled");
    });
  ...
  _updateUserParams: function (shopConfigParams) {
    if (this.uiConfigInfo) {
      this._getMustDisabledOptions().forEach((option) => {
        ...
        if (!enabledInShop) {
          this.uiConfigInfo["activeActions"] =
            this.uiConfigInfo.activeActions.filter((x) => x !== option);
        }
      });
      ...
      this.uiConfigInfo["anyActionEnabled"] = this._anyActionEnabled(
        this._getAllActions(),
      );
    }
  },
  ...
  _onAddToWishlistClick: function (ev) {
    ...
    rpc("/theme_prime/wishlist_general", { product_id: productID }).then(
      (res) => {
        ...
        this.displayNotification({
          ...
          message: markup(
            renderToString("DroggolNotification", {
              ...
              message: _t("Added to your wishlist."),
              iconClass: "dri dri-wishlist",
            }),
          ),
          buttons: [
            {
            ...
            },
            {
              name: _t("Undo"),
              onClick: () => {
                this._removeProductFromWishlist(res.wishlist_id, productID);
              },
            },
          ],
        });
  ...
  _onSuccessResponse: function () {
    if (this.isMobile && this.uiConfigInfo && this.uiConfigInfo.mobileConfig) {
      ...
      keys.forEach((key) => {
        if (
          this.uiConfigInfo[key] &&
          this.uiConfigInfo.mobileConfig[key] === "default"
        ) {
      ...
      this.uiConfigInfo = {
        ...this.uiConfigInfo,
        ...this.uiConfigInfo.mobileConfig,
      };
    }
    this._super.apply(this, arguments);
  },
  _setClass: function () {
    ...
    if (this.deviceSizeClass <= 1) {
      ...
      if (
        ...
        this.uiConfigInfo.mobileConfig &&
        this.uiConfigInfo.mobileConfig.mode === "grid"
      ) {
        this.cardSize = 6;
      }
