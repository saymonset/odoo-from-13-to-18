    ...
    useEffect(
      () => {
        ...
        if (
          ...
          this.modalRef.el
            .querySelector(".modal-dialog")
            .classList.add("modal-fullscreen", "p-0");
        }
      },
      () => [],
    );
...
export class TpSnippetConfigDialog extends Component {
  setup() {
    ...
    if (!this.isReadOnly) {
      ...
      let {
        metadata: { title },
        name,
      } = this.tpWebsite.currentWebsite;
      this.title = title || "title";
    }
    this.videoURL =
      this.props.videoURL || "https://www.youtube.com/watch?v=h1j2L4jqzs0";
    this.defaultPricelist = false;
    this.previewState = useState({
      templateID: this.props.snippetID,
      HasRecords: false,
      history: [],
      activeStep: 1,
    ...
    onWillStart(async () => {
      if (this.registryToUse) {
        this.allSnippets = await this.componentService._fetchRecords({
          model: "ir.ui.view",
          extras: {
        ...
        if (this.isReadOnly) {
          this.allSnippets = this.allSnippets.filter((snippet) => {
            return ![
              ...
              "s_tp_mega_menu_category_snippet",
            ].includes(snippet.id);
          });
        }
        this.allSnippets.forEach((snippet) => {
          ...
          snippet["resModel"] =
            TpRecordSelector && TpRecordSelector.defaultVal
              ? TpRecordSelector.defaultVal.model
              : "product.template";
        });
        let firstSnippet = this.allSnippets.filter((snippet) => {
          return snippet.id == this.snippetID;
        })[0];
        this.previewState.templateID =
          (firstSnippet && firstSnippet.id) || false;
        this.body = (firstSnippet && firstSnippet.html) || false;
      }
      let { active_pricelist, pricelists } =
  ...
  get allSnippptsDropdown() {
    let buttonClasses = {
      menuClass: "row ps-2 pe-0 g-0 tp-snippet-dropdown",
      menuItemClass: "col-6 pe-2 position-relative",
      dropDownPlaceholder: "theme_prime.tp_snippet_dropdown_placeholder",
      buttonClasses:
        "btn d-flex justify-content-between align-items-center btn-light shadow-sm tp-rounded-border-lg fw-light w-100 mx-auto pe-2 p-0",
    };
    return {
      props: {
        ...buttonClasses,
        name: "templateID",
        value: this.previewState.templateID,
        records: this.allSnippets,
      },
      component: Components.TpDropDown.Component,
    };
  }
  get allPricelistDropdown() {
    let buttonClasses = {
      buttonClasses:
        "btn d-flex justify-content-between align-items-center btn-light shadow-sm fw-light w-100 mx-auto p-1 pe-2",
    };
    return {
      props: {
        ...buttonClasses,
        dropDownPlaceholder: "theme_prime.tp_pricelist_dropdown_placeholder",
        name: "activePricelist",
        value: this.previewState.activePricelist,
        records: this.pricelists,
      },
      component: Components.TpDropDown.Component,
    };
  }
  ...
  _checkPricelistAvailability() {
    if (
      ...
      !this.getRecordFromData(this.pricelists, this.state.TpExtraOpts.priceList)
    ) {
      this.state.TpExtraOpts["priceList"] = "*";
    }
  }
  ...
  _getConfigParameter(snippet, isForCurrentState) {
    ...
    this.$target = new DOMParser().parseFromString(
      snippetData.html,
    ...
    if (
      isForCurrentState &&
      this.previewContainer?.el?.contentDocument?.querySelector("section")
    ) {
      this.$target =
        this.previewContainer.el.contentDocument.querySelector("section");
    }
  ...
  updateSelectionComponentValue(key, value, name) {
    ...
    if (this.hasUiComponent && this._isLazyLoad) {
      ...
      let configData = toRaw(
        this.state.TpUiComponent.categoryTabsConfig.records,
      );
      ...
      this.updateUiComponentValue("categoryTabsConfig", {
  ...
  _onClickCommand(command) {
    ...
    switch (command) {
      case "undo":
        this.previewState.activeStep =
          currentStep - 1 <= 0 ? 1 : currentStep - 1;
        break;
      case "redo":
        this.previewState.activeStep =
          currentStep + 1 >= this.previewState.history.length
            ? this.previewState.history.length
            : currentStep + 1;
        break;
    }
  ...
  async updatePricelist(pricelist) {
    ...
    await this.componentService._fetchRecords({
      model: "bas.aem.j",
      extras: { activePricelist: pricelist },
    });
  }
  async save() {
    let result = { ...this.state };
    if (this.registryToUse && this.previewState.templateID) {
      let template = this.getRecordFromData(
        this.allSnippets,
        this.previewState.templateID,
      );
      result = {
        ...
        templateID: this.previewState.templateID,
      };
    }
  ...
  prepareComponents(snippetData, hardRestore) {
    ...
    Object.keys(data.components).forEach((comp) => {
      ...
      state[comp] =
        value && value.componentData && Object.keys(value.componentData).length
          ? value.componentData
          : { ...value.defaultVal };
    });
    if (!this.state) {
    ...
    Object.keys(data.components).forEach((comp) => {
      ...
      this.supportedComponents.components.push({
        ...Components[comp],
        props: {
          extras: {
            ...data.components[comp],
            isReadOnly: this.isReadOnly,
            templateID: this.previewState.activeStep,
            activePricelist: this.previewState.activePricelist,
          },
          componentData: value,
          allData: this.state,
          HasRecords: this.previewState.HasRecords,
        },
        name: comp,
      });
    });
  ...
  _onPreviewLoaded() {
    this.previewContainer.el.contentWindow.addEventListener(
      "TP_WRAPPER_READY",
      (event) => {
  ...
  _reloadPreview() {
    if (
      ...
      this.previewContainer.el.contentDocument &&
      this.previewContainer.el.contentDocument.querySelector("#tp_wrap")
    ) {
      let wrapper =
        this.previewContainer.el.contentDocument.querySelector("#tp_wrap");
  ...
  get templateIDs() {
    ...
    Object.keys(SnippetRegistry.content).forEach(function (snippetID) {
      let prefix = snippetID.includes("mega_menu")
  ...
  get HasRecords() {
    ...
    if (
      TpRecordSelector &&
      TpRecordSelector.selectionType === "manual" &&
      TpRecordSelector.recordsIDs.length
    ) {
      hasRec = true;
    }
    return hasRec;
...
TpSnippetConfigDialog.components = {
  TpConfigDialog,
  TpRecordSelector,
  TpDropDown,
};
