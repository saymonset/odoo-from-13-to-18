...
import {
  ...
  cartMixin,
  TabsMixin,
} from "@theme_prime/js/core/mixins";
...
publicWidget.registry.s_d_products_snippet = ProductRootWidget.extend(
  ...
  ProductsBlockMixins,
  {
    ...
    snippetNodeAttrs: (
      ProductRootWidget.prototype.snippetNodeAttrs || []
    ).concat(["data-selection-info"]),
...
publicWidget.registry.s_product_listing_cards_wrapper =
  ProductRootWidget.extend(ProductsBlockMixins, MarkupRecords, {
    selector:
      ".s_product_listing_cards_wrapper, .s_image_product_listing_cards_wrapper",
    ...
    snippetNodeAttrs: (
      ProductRootWidget.prototype.snippetNodeAttrs || []
    ).concat(["data-selection-info", "data-ui-config-info"]),
...
// Countdown snippet
publicWidget.registry.s_d_single_product_count_down = ProductRootWidget.extend(
  ProductsBlockMixins,
  {
    ...
    _modifyElementsAfterAppend: function () {
      ...
      this.$(".droggol_product_slider_single_product").owlCarousel({
        ...
        nav: true,
        navText: [
          '<i class="dri dri-arrow-left-l"></i>',
          '<i class="dri dri-arrow-right-l"></i>',
        ],
...
publicWidget.registry.s_product_listing_tabs_snippet = ProductRootWidget.extend(
  OwlMixin,
  MarkupRecords,
  TabsMixin,
  {
    ...
    _getOptions: function () {
      if (
        this.selectionInfo &&
        this.selectionInfo.recordsIDs &&
        this.selectionInfo.recordsIDs.length
      ) {
    ...
    _setCamelizeAttrs: function () {
      ...
      this.categories = [];
      let labels = {
        bestseller: _t("Best Sellers"),
        discount: _t("On Sale"),
        newArrived: _t("Newly Arrived"),
      };
      this.supportedTypes.forEach((type, index) => {
        this.initialType =
          !this.initialType && this.uiConfigInfo[type]
            ? type
            : this.initialType;
    ...
    _getDomain: function () {
      return this.initialType === "discount"
...
publicWidget.registry.s_category_snippet = ProductRootWidget.extend(
  ...
  {
    selector:
      ".s_d_category_snippet_wrapper, .s_products_by_brands_tabs_wrapper",
    ...
    _getDomainValues: function (categoryID) {
      ...
      return {
        domain: domain,
        options: { order: sortBy, limit: limit },
    ...
    _setDBData: function (data) {
      ...
      let categories = recordsIDs.map((categoryID) => {
        return data.categories.find((c) => c.id === categoryID);
      });
      this.categories = categories.filter((x) => !!x);
      this.fetchedCategories = this.categories.map((category) => {
        return category.id;
      });
  ...
  MarkupRecords,
  {
    ...
    initializeOwlSlider: function () {
      this.$(".droggol_product_category_slider").owlCarousel({
        ...
        nav: true,
        navText: [
          '<div class="badge text-primary"><i class="dri font-weight-bold dri-chevron-left-l"></i></div>',
          '<div class="badge text-primary"><i class="dri dri-chevron-right-l font-weight-bold"></i></div>',
        ],
        responsive: {
          0: { items: 1 },
          576: { items: 1 },
...
publicWidget.registry.s_category_brands = RootWidget.extend(
  ProductsBlockMixins,
  {
    selector: ".s_category_small, .s_brands_small",
    snippetNodeAttrs: (RootWidget.prototype.snippetNodeAttrs || []).concat([
      "data-selection-info",
    ]),
    ...
    _getImgUrl: function (id) {
      return this._getResModel() === "product.attribute.value"
        ? `/web/image/${this._getResModel()}/${id}/dr_image`
        : `/web/image/${this._getResModel()}/${id}/image_128`;
    },
    /**
     * @private
     */
    _getItemUrl: function (record) {
      return this._getResModel() === "product.attribute.value"
        ? `/shop?attribute_value=${record.attribute_id[0]}-${record.id}`
        : `/shop/category/${record.id}`;
    },
    /**
     * @private
     */
    _getBodyDetails: function (resModel) {
      let resModels = {
        "product.attribute.value": {
          title: _t("Shop By Brands"),
          url: "/shop/all-brands",
        },
        "product.public.category": {
          title: _t("Shop By Categories"),
          url: "/shop",
        },
      };
...
publicWidget.registry.s_d_single_product_cover_snippet =
  publicWidget.registry.s_single_product_snippet.extend({
...
publicWidget.registry.s_d_top_categories = RootWidget.extend(MarkupRecords, {
  ...
  _processData: function (data) {
    ...
    let res = recordsIDs.map((categoryID) => {
  ...
  ProductsBlockMixins,
  {
    ...
    _modifyElementsAfterAppend: function () {
      ...
      this.$(".droggol_product_slider_top").owlCarousel({
        ...
        nav: true,
        navText: [
          '<i class="dri h4 dri-chevron-left-l"></i>',
          '<i class="dri h4 dri-chevron-right-l"></i>',
        ],
        responsive: {
          0: { items: 1 },
          768: { items: 2 },
...
publicWidget.registry.s_two_column_card_wrapper = ProductRootWidget.extend(
  OwlMixin,
  ProductsBlockMixins,
  {
  ...
  ProductsBlockMixins,
  {
    ...
    _modifyElementsAfterAppend: function () {
      ...
      this.$(".droggol_product_slider_top").owlCarousel({
        dots: false,
        margin: 20,
        stagePadding: this.inConfirmDialog ? 0 : 5,
        rewind: true,
        nav: true,
        rtl: localization.direction === "rtl",
        navText: [
              ...
              setTimeout(function () {
                if (!uiUtils.isSmall()) {
                  var height = self.$target
                    ...
                    .find(
                      ".s_d_product_count_down .owl-item.active .tp-side-card",
                    )
                    .height();
...
publicWidget.registry.s_d_products_grid_wrapper = ProductRootWidget.extend(
  ProductsBlockMixins,
  {
...
publicWidget.registry.s_category_tabs_snippet = RootWidget.extend({
  selector:
    ".s_category_tabs_snippet_wrapper:not(.tp-side-menu), .s_tp_categories_menu",
  ...
  start: function () {
    ...
    if (this.$target.hasClass("s_tp_categories_menu")) {
      this.$target
        .find(".s_category_tabs_snippet_wrapper")
        .addClass("tp-side-menu");
  ...
  _getOptions: function () {
    let options =
      this.uiConfigInfo && this.uiConfigInfo.onlyDirectChild
        ? { onlyDirectChild: this.uiConfigInfo.onlyDirectChild }
        : {};
    return this.selectionInfo && this.selectionInfo.recordsIDs
      ? { ...options, categoryIDs: this.selectionInfo.recordsIDs }
      : false;
  },
  /**
   * @override
   */
  _getSortBy: function () {
    return this.uiConfigInfo && this.uiConfigInfo.childOrder
      ? this.uiConfigInfo.childOrder
      : "count";
  },
  /**
   * @private
  ...
  _activateCategory: function (categoryID) {
    ...
    let $submenu = $(
      this.$(".tp-category-submenu[data-submenu-id='" + categoryID + "']"),
    );
    ...
    if (!$submenu.hasClass("tp-fetched-submenu")) {
      ...
      this._setOffsetPosition(
        this.$(".tp-menu-category-tab[tp-menu-id='" + categoryID + "']"),
      );
    }
    this.$(".tp-menu-category-tab").removeClass("tp-active-category");
    this.$(".tp-menu-category-tab[tp-menu-id='" + categoryID + "']").addClass(
      "tp-active-category",
    );
  },
  ...
  getCategoryConfigData: function (categoryID) {
    if (
      ...
      this.uiConfigInfo.categoryTabsConfig.records
    ) {
  ...
  _getUIConfigData: function (categoryID) {
    let { style, limit, activeActions, background, productListing } =
      this.getCategoryConfigData(categoryID);
    return {
      productListing: productListing || "bestseller",
      background: background || false,
      style: style || "s_tp_hierarchical_category_style_1",
      limit: limit,
  ...
  _modifyElementsAfterAppend: function () {
    ...
    if (
      this.editableMode &&
      this.uiConfigInfo.categoryTabsConfig &&
      this.uiConfigInfo.categoryTabsConfig.activeRecordID
    ) {
    ...
    if (this.isSideMenu) {
      let target = this.$target
        .get(0)
        .querySelector(".s_category_tabs_snippet_wrapper");
  ...
  _processData: function (data) {
    ...
    recordsIDs.forEach((recordsID) => {
      let categoryRec = data.find((category) => {
        return category.category.id === recordsID;
      });
...
publicWidget.registry.s_tp_mega_menu_category_snippet = RootWidget.extend({
  ...
  _getLimit: function () {
    return this.uiConfigInfo.hasOwnProperty("limit")
      ? this.uiConfigInfo.limit
      : false;
  },
  _modifyElementsAfterAppend: function () {
  ...
  _getProductSelectionData: function () {
    return this.JaysonStringify({
      selectionType: "advance",
      domain_params: {
        domain: [
          ["public_categ_ids", "child_of", this.selectionInfo.recordsIDs],
        ],
        limit: 5,
        order: "bestseller",
      },
  ...
  _getUIConfigData: function () {
    ...
    return this.JaysonStringify(
      Object.assign({}, config, {
        limit: 3,
        style: "tp_product_list_cards_4",
        header: "tp_product_list_header_1",
        activeActions: ["rating", "add_to_cart", "wishlist", "quick_view"],
...
publicWidget.registry.s_category_ui_snippet = ProductRootWidget.extend(
  ProductsBlockMixins,
  {
...
publicWidget.registry.s_d_brand_snippet = RootWidget.extend({
  ...
  extraLibs: (RootWidget.prototype.extraLibs || []).concat([
    "/theme_prime/static/lib/OwlCarousel2-2.3.4/owl.carousel.js",
  ]),
  ...
  _getOptions: function () {
    // Hack
    this.recordsIDs =
      (this.selectionInfo && this.selectionInfo.recordsIDs) || [];
    this.categories = this.$target.get(0).dataset.categories;
    this.mode = (this.uiConfigInfo && this.uiConfigInfo.mode) || "slider";
    this.cardStyle =
      (this.uiConfigInfo && this.uiConfigInfo.style) || "tp_brand_card_style_1";
    return {
  ...
  _modifyElementsAfterAppend: function () {
    ...
    if (this.mode === "slider") {
      ...
      this.$(".s_d_brand_snippet > .row > *")
        .removeAttr("class")
        .addClass(this.cardStyle);
      // remove col-* classes
...
publicWidget.registry.tp_image_hotspot = publicWidget.Widget.extend(
  HotspotMixns,
  cartMixin,
  CartManagerMixin,
  MarkupRecords,
  {
    ...
    start: function () {
      ...
      if (
        this.editableMode &&
        this.hotspotType === "dynamic" &&
        this.onHotspotClick === "modal"
      ) {
    ...
    _fetchData: async function () {
      return await rpc("/theme_prime/get_products_data", {
        domain: [
          ["id", "in", [parseInt(this.$target.get(0).dataset.productId)]],
        ],
    ...
    _initPopover: function () {
      ...
      this.$target
        .popover({
          ...
          html: true,
          placement: "auto",
          content: renderToElement("theme_prime.tp_img_static_template", {
            widget: this,
            data: this._getHotspotConfig(),
          }),
        })
    ...
    _isLoaded: function () {
      return new Promise((resolve, reject) => {
        var $relatedImage = $(
          this.$target
            .closest(".tp-img-hotspot-wrapper")
            .find(".tp-img-hotspot-enable"),
        );
        // ImagesLazyLoading Odoo hack
        if ($relatedImage.get(0).naturalWidth) {
          resolve();
        }
        $relatedImage.one("load", function () {
          resolve();
        });
      });
    },
      ...
      if (this.onHotspotClick === "popover") {
        if (this.hotspotType === "dynamic") {
          ...
          if (this.productInfo && this.productInfo.has_discounted_price) {
            this.productInfo["discount"] = Math.round(
              ((this.productInfo.list_price_raw - this.productInfo.price_raw) /
                this.productInfo.list_price_raw) *
                100,
            );
          }
          this._initPopover();
