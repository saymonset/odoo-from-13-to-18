...
import { CartSidebar } from "@theme_prime/js/sidebar/sidebar";
...
export let B2bMixin = {
  _isB2bModeEnabled: function () {
    ...
    return (
      data &&
      !data.logged &&
      odoo.dr_theme_config.json_b2b_shop_config &&
      odoo.dr_theme_config.json_b2b_shop_config.dr_enable_b2b
    );
  },
  ...
  _loggedInNotification: function () {
    ...
    odoo.dr_theme_config.has_sign_up
      ? buttons.push({
          name: _t("Sign Up"),
          onClick: () => {
            window.location = "/web/signup";
          },
        })
      : buttons.push({
          name: _t("Close"),
          onClick: () => {
            $(".o_notification_manager .tp-login-notification").remove();
          },
        });
    this.displayNotification({
      className:
        "tp-notification tp-rounded-border tp-login-notification tp-bg-soft-primary o_animate",
      templateToUse: "theme_prime.NotificationGeneric",
      message: markup(
        renderToString("DroggolNotification", {
          productName: _t("Log in to place an order"),
          iconClass: "fa fa-sign-in",
          color: "primary",
          message: _t("Please log in first."),
        }),
      ),
      buttons: buttons,
    });
...
export let cartMixin = Object.assign({}, B2bMixin, {
  ...
  _addProductToCart: function (cartInfo) {
    ...
    this.call("dialog", "add", QuickViewDialog, {
      parent: this,
      ...
      autoAddCallback: (data) => {
        if (this._customCartSubmit) {
          this._customCartSubmit({
            ...data,
            product_id: cartInfo.productID,
            add_qty: 1,
          });
        }
      },
...
export let OwlMixin = {
  ...
  initializeOwlSlider: function (ppr, isTwoColLayout) {
    ...
    if (
      this.uiConfigInfo_init &&
      this.uiConfigInfo_init.mobileConfig &&
      this.uiConfigInfo_init.mobileConfig.style !== "default"
    ) {
...
export let HotspotMixns = {
  ...
  _isPublicUser: function () {
    return (
      odoo.dr_theme_config.hasOwnProperty("is_public_user") &&
      odoo.dr_theme_config.is_public_user
    );
  },
...
export let TabsMixin = {
  ...
  noDataTemplateSubString: _t(
    "Sorry, We couldn't find any products under this category",
  ),
  ...
  _activateTab: function (recordID) {
    ...
    this.$(
      ".d_s_category_cards_item[data-category-id=" + recordID + "]",
    ).removeClass("d-none");
  },
  /**
  ...
  _fetchAndAppendByCategory: function (recordID) {
    ...
    this._fetchProductsByDomain(this._getDomainValues(recordID)).then(
      (data) => {
  ...
  _renderNewProducts: function (products, recordID) {
    ...
    var $tmpl = $(
      renderToElement("d_s_category_cards_item", {
        data: products,
        widget: this,
        recordID: recordID,
      }),
  ...
  _onCategoryTabClick: function (ev) {
    ...
    if (
      !this.$(".d_s_category_cards_item[data-category-id=" + recordID + "]")
        .length
    ) {
      if (this.loaderTemplate) {
        var $template = $(renderToElement(this.loaderTemplate));
        $template
          .addClass("d_loader_default")
          .appendTo(this.$(".d_s_category_cards_container"));
      }
...
export const CartManagerMixin = Object.assign({}, B2bMixin, {
  ...
  _cartNotification: function (data) {
    this.displayNotification({
      className:
        "tp-notification tp-rounded-border tp-bg-soft-primary o_animate",
      templateToUse: "theme_prime.NotificationGeneric",
      message: markup(
        renderToString("DroggolNotification", {
          color: "primary",
  ...
  _outOfStockNotification: function (data) {
    this.displayNotification({
      className:
        "tp-notification tp-rounded-border tp-stock-notification tp-bg-soft-primary o_animate",
      templateToUse: "theme_prime.NotificationGeneric",
      message: markup(
        renderToString("DroggolNotification", {
          subIconClass: "fa fa-exclamation-circle",
          color: "danger",
          productID: data.product_id,
          productName: _t("Currently not in stock."),
          message: _t("Check again later."),
        }),
      ),
      buttons: [
        {
          name: _t("View"),
          onClick: () => {
            window.location = `/shop/${data.productTmplID}`;
          },
        },
        {
          name: _t("Close"),
          onClick: () => {
            $(".o_notification_manager .tp-stock-notification").remove();
          },
        },
      ],
    });
  ...
  _cartDialog: function (data) {
    this.call("dialog", "add", CartConfirmationDialog, {
      parent: this,
      info: data,
    });
  },
  ...
  _customCartSubmit: function (params) {
    if (Object.keys(params).includes("inStock") && params.inStock === false) {
      this._outOfStockNotification(params);
      return;
    }
    params.force_create = true;
    params.dr_cart_flow = odoo.dr_theme_config.cart_flow || "notification";
    if (Object.keys(params).includes("dr_is_combo") && params.dr_is_combo) {
      wSaleUtils.updateCartNavBar(params);
      return this._handleCartConfirmation(params.dr_cart_flow, params);
    }
    return rpc("/shop/cart/update_json", params).then(async (data) => {
