...
export const Sidebar = publicWidget.Widget.extend({
  ...
  _renderLoader: function () {
    this.$loader = $(
      renderToElement("theme_prime.Loader", {
        loadingStr: this.options.loadingStr,
        height: "100vh",
      }),
    );
...
publicWidget.registry.TpAccountInfoSidebarBtn = publicWidget.Widget.extend({
  ...
  _onClick: function (ev) {
    ev.preventDefault();
    const accountInfoSidebarEl = document.querySelector(
      ".tp-account-info-sidebar",
    );
    accountInfoSidebarEl.classList.remove("offcanvas-start", "offcanvas-end");
    accountInfoSidebarEl.classList.add(
      ev.currentTarget.dataset.position
        ? `offcanvas-${ev.currentTarget.dataset.position}`
        : "offcanvas-end",
    );
...
export const CategorySidebar = Sidebar.extend({
  ...
  _onClickCategoryLink: function (ev) {
    if (ev.currentTarget.classList.contains("back")) {
      ...
      const parentCategory = this.categories.filter(
        (category) =>
          category.id == parseInt(ev.currentTarget.dataset.parentId),
      )[0];
      if (parentCategory) {
        if (parentCategory.parent_id) {
          ...
          parentCategories = this.categories.filter((category) =>
            siblingCategories.includes(category.id),
          );
        }
      }
      ...
      const childIds = this.categories.filter(
        (category) => category.id == parentCategoryId,
      )[0].child_id;
      const childCategories = this.categories.filter((category) =>
        childIds.includes(category.id),
      );
  ...
  _renderCategories: function (categories, parentId = false) {
    ...
    const $categories = $(
      renderToElement("theme_prime.CategorySidebar", {
        categories,
        parentCategory,
        categoryCount: this.categoryCount,
        options: this.options,
        config: odoo.dr_theme_config.json_sidebar_config,
      }),
    );
    this.$el.empty();
...
publicWidget.registry.TpSimilarProductSidebarBtn = publicWidget.Widget.extend({
  ...
  _onClick: function (ev) {
    ...
    return new Sidebar(this, {
      ...
      fetchParams: {
        productID: parseInt(ev.currentTarget.dataset.productTemplateId),
      },
      position: ev.currentTarget.dataset.position || "end",
    }).show();
  },
