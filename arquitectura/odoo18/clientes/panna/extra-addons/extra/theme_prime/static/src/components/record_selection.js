...
export class TpRecordSelector extends AbstractComponent {
  setup() {
    ...
    onWillStart(async () => {
      ...
      this.records = await this.componentService._fetchRecords(params, {
      ...
      if (this.environmentModel === "product.public.category") {
        this.fetchedCategoryData = await this.componentService._fetchRecords(
          {
            options: { getCount: true, categoryIDs: this.recordsIDs },
      ...
      // magic happens here :)
      if (
        this.props.selectionType === "manual" &&
        recordsIDs.length !== this.records
      ) {
    ...
    onMounted(() => {
      if (this.recordsContainer) {
        this.sortables = new Sortable(this.recordsContainer.el, {
          animation: 150,
          handle: ".tp-sortable-handle",
          ghostClass: "tp-ghost-sortable",
          dataIdAttr: "data-record-id",
          onUpdate: (ev) => this.updateSortableList(ev),
        });
      }
    });
  ...
  updateSortableList(ev) {
    this._notifyChanges(
      "recordsIDs",
      this.sortables.toArray().map(Number).filter(Number),
    );
  }
  ...
  get componentRegistry() {
    return {
      TpSearchInput: ["TpSearchInput"],
      TpDropDown: ["selectionType", "model"],
    };
  }
  get nodeOptions() {
    return {
      TpSearchInput: {
        ...
        model: this.environmentModel,
        fieldsToMarkup: this.fieldsToMarkup,
        brand: this.isBrand,
        hasSuggestion: this.SelectionMode !== "badge",
      },
      selectionType: {
        buttonClasses:
          "btn d-flex justify-content-between align-items-center btn-light bg-white border shadow-sm fw-light w-100 btn-sm",
        records: [
          {
            id: "manual",
          ...
          { id: "advance", iconClass: "fa fa-sliders", title: _t("Advance") },
        ],
      },
      model: {
        buttonClasses:
          "btn d-flex justify-content-between align-items-center btn-light bg-white border shadow-sm fw-light w-100 btn-sm",
        records: this.models.map((key) => {
          return { id: key, title: this.resModelInfo[key].title };
        }),
      },
    };
  }
  ...
  get resModelInfo() {
    return {
      "product.template": { title: _t("Products") },
      "dr.product.label": { title: _t("Labels") },
      "product.tag": { title: _t("Tags") },
      "product.product": { title: _t("Variants") },
      "product.public.category": { title: _t("Categories") },
      "product.attribute.value": { title: _t("Brand") },
    };
  }
  performAction(action, value) {
    ...
    switch (action) {
      ...
      case "ADD_ALL":
        ...
        recordsIDs =
          action === "ADD_ALL" ? itemToAdd : recordsIDs.concat(itemToAdd);
        break;
      case "REPLACE":
        recordsIDs = value;
        break;
      ...
      case "MODEL_CHANGE":
        ...
        componentData.selectionType === "advance"
          ? (componentData["domain_params"] = {
              domain: [],
              limit: 5,
              order: "name asc",
            })
          : (componentData["recordsIDs"] = []);
      ...
      case "CHANGE":
        ...
        if (value === "advance") {
          params = {
            ...params,
            domain_params: { domain: [], limit: 5, order: "name asc" },
          };
        } else {
  ...
  async _refreshRecords(recordsIDs) {
    let params = {
      ...this.defaultParams,
      domain: this._getRecordsDomain(recordsIDs),
    };
    this.records = await this.componentService._fetchRecords(params, {
      fieldsToMarkup: this.fieldsToMarkup,
    });
    ...
    await this.componentService._fetchRecords(
      {
        model: this.environmentModel,
        recordsIDs: this.recordsIDs,
        name: this.collectionInput.el.value,
      },
      { routePath: "/theme_prime/tp_create_collection" },
    );
  ...
  get fieldsLabel() {
    return {
      image: _t("Image"),
      name: _t("Name"),
      dr_brand_value_id: _t("Brand"),
      public_categ_ids: _t("Category"),
      dr_stock_label: _t("Availability"),
    };
  }
  get getFieldsTemplates() {
    return {
      public_categ_ids: "tp_config_field_category",
      dr_brand_value_id: "tp_config_field_brand",
      name: "tp_config_field_product_name",
      dr_stock_label: "tp_config_field_product_stock_label",
    };
  }
  get recordsIDs() {
  ...
  get defaultParams() {
    return {
      ...
      fields: this.fields,
      limit: 10,
      extras: { activePricelist: this.activePricelist },
    };
  }
}
