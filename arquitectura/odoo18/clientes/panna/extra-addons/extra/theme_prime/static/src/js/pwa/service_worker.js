...
if (offlinePage) {
  self.addEventListener("fetch", (event) => {
    ...
    if (url.origin === location.origin) {
      if (
        event.clientId === "" && // Not fetched via AJAX after page load.
        ...
        (!url.href.includes("/static/src") ||
          url.href.includes("pwa_offline.png")) // Don't run for static.
      ) {
        // request.mode = navigate isn't supported in all browsers
        // so include a check for Accept: text/html header.
        if (
          (event && event.request && event.request.mode === "navigate") ||
