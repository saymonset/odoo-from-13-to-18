...
options.registry.s_tp_documents_snippet = options.Class.extend({
   ...
   */
  start: function () {
    this.$target.on("click", ".tp_add_documents", (ev) => {
  ...
  _appendDocument: function (documentInfo) {
    ...
    this.$target.find(".s_tp_documents_snippet_row").append(
      $(
        renderToElement("theme_prime.s_tp_documents_snippet_template", {
...
options.registry.TpImageHotspot = options.Class.extend({
  ...
  start() {
    this.$target.on(
      "image_changed.TpImageHotspot",
      this._onImgChanged.bind(this),
    );
    return this._super(...arguments);
  },
  /**
  ...
  toggleImgHotspot(previewMode, widgetValue, params) {
    ...
    if (widgetVal) {
      this.$target.wrap(
        "<div class='d-flex tp-img-hotspot-wrapper'><div class='position-relative tp-hotspot-container d-inline-block'></div></div>",
      );
  ...
  _computeVisibility() {
    const img = this._getImage();
    if (
      !["image/jpeg", "image/png", "image/webp"].includes(img.dataset.mimetype)
    ) {
      this._cleanHotspotNode();
      return false;
    }
  ...
  addHotspot(previewMode, widgetValue, params) {
    ...
    $wrapper.append(
      $("<span/>", {
        id: hotspotID,
        class:
          "tp_hotspot tp_hotspot_style_1 tp-hotspot-primary position-absolute",
    ...
    let values = {
      ...
      subtitleText: "Theme prime is best theme",
      buttonLink: "/",
      buttonText: "SHOP NOW",
      imageSrc: "/theme_prime/static/src/img/s_config_data.png",
    };
    ...
    if (methodName === "fetch_ecom_categories_opt" && opt) {
      if (
        this.containerEl
          ...
          .classList.contains("s_no_category_sync")
      ) {
...
options.registry.TpImageHotspotConfig = options.Class.extend({
  ...
  start: function () {
    ...
    this.$el.find('we-range[data-set-top] input[type="range"]').on(
      `input.${this.rangeWidgetTopId}`,
      throttleForAnimation((ev) => {
        this.setTop(true, $(ev.currentTarget).val());
      }, 50),
    );
    this.$el.find('we-range[data-set-left] input[type="range"]').on(
      `input.${this.rangeWidgetLeftId}`,
      throttleForAnimation((ev) => {
        this.setLeft(true, $(ev.currentTarget).val());
      }, 50),
    );
  ...
  destroy: function () {
    ...
    this.$el
      .find('we-range[data-set-top] input[type="range"]')
      .off(`.${this.rangeWidgetTopId}`);
    this.$el
      .find('we-range[data-set-left] input[type="range"]')
      .off(`.${this.rangeWidgetLeftId}`);
  },
  ...
  setHotspotType(previewMode, widgetValue, params) {
    ...
    if (
      widgetValue === "dynamic" &&
      !this.$target[0].dataset["onHotspotClick"]
    ) {
  ...
  renderHotspotPreview(previewMode, widgetValue, params) {
    ...
    this.$el
      .find('[data-name="hotspot_priview"] .fa-eye')
      .toggleClass("text-success", this.PreviewEnabled);
    this._openPopover();
  },
  /**
   * @override
  ...
  selectClass: async function (previewMode, widgetValue, params) {
    ...
    if (
      this.$target.find(".dri").length &&
      params.possibleValues.includes("tp_hotspot_style_4")
    ) {
      this.$target.find(".dri").remove();
    }
    if (
      widgetValue === "tp_hotspot_style_4" &&
      params.possibleValues.includes("tp_hotspot_style_4")
    ) {
  ...
  _computeWidgetState(methodName, params) {
    switch (methodName) {
      ...
      case "setSubtitleText":
        return (
          this.$target.get(0).dataset.subtitleText ||
          "Theme prime is best theme"
        );
      case "setTitleText":
  ...
  _getHotspotConfig: function () {
    if (this.$target.get(0).dataset.hotspotType === "static") {
      return {
        titleText: this.$target.get(0).dataset.titleText,
        subtitleText: this.$target.get(0).dataset.subtitleText,
        buttonLink: this.$target.get(0).dataset.buttonLink,
        hotspotType: this.$target.get(0).dataset.hotspotType,
        buttonText: this.$target.get(0).dataset.buttonText,
        imageSrc: this.$target.get(0).dataset.imageSrc,
      };
    }
    return {};
  },
    ...
    let SELECTOR_DATA = {
      TpRecordSelector: {
        ...
        defaultVal: {
          ...
          recordsIDs: productID ? [productID] : [],
        },
      },
    };
...
options.registry.tp_dynamic_snippet = options.Class.extend({
  ...
  async willStart() {
    const _super = this._super.bind(this);
    this.registryToUse = this.$target[0].parentElement.classList.contains(
      "o_mega_menu",
    )
      ? "theme_prime_mega_menus"
      : "theme_prime_snippet_registry";
    if (this.registryToUse === "theme_prime_snippet_registry") {
      this.switchableRelatedViews = await this._rpc.call(
        "/theme_prime/tp_search_read",
        {
          model: "ir.ui.view",
          extras: { templateIDs: ["theme_prime.s_tp_categories_menu"] },
        },
      );
    }
    return _super(...arguments);
   ...
   */
  start: function () {
    this.$target.on("click", ".tp-config-link", (ev) => {
      ev.preventDefault();
      this._openDialog();
    });
    return this._super.apply(this, arguments);
  ...
  _getCurrentSnippetId: function () {
    ...
    return snippetID === "s_tp_categories_menu"
      ? "s_mega_menu_category_tabs_snippet"
      : snippetID;
  },
  /**
   * @override
  ...
  _getConfiguratorParams: function () {
    ...
    let params = {
      registryToUse: this.registryToUse,
      videoURL: defaultValue.videoURL,
      components: {},
      snippetID: defaultValue.noSnippet ? false : snippet,
      $target: this.$target.html(),
    };
    for (let component in snippetConfig.widgets) {
      ...
      switch (component) {
        case "TpRecordSelector":
          params["components"]["TpRecordSelector"] = Object.assign(
            {},
            value,
            defaultValue,
            {
              componentData: this._getValueFromAttr(
                this.$target,
                "data-selection-info",
              ),
            },
          ...
          break;
        case "TpUiComponent":
          params["components"]["TpUiComponent"] = {
            ...value,
            ...defaultValue,
            componentData: {
              ...this._getValueFromAttr(this.$target, "data-ui-config-info"),
            },
            $target: this.$target,
          };
          ...
          break;
        case "TpExtraOpts":
          params["components"]["TpExtraOpts"] = {
            ...defaultValue,
            componentData:
              this._getValueFromAttr(this.$target, "data-extra-info") || value,
            $target: this.$target,
          };
  ...
  _setConfiguratorParams: function (data) {
    ...
    if (
      data.$target &&
      !isSameSnippet &&
      data.templateID === "s_mega_menu_category_tabs_snippet" &&
    ...
    if (data.$target && !isSameSnippet) {
      ...
      Object.values(this.$target[0].attributes).forEach(({ name }) =>
        this.$target[0].removeAttribute(name),
      );
      let vDom = new DOMParser().parseFromString(
        data.$target,
        "text/html",
      ).documentElement;
      let block = vDom.querySelector("body section");
      Object.values(block.attributes).forEach(({ name }) => {
        this.$target[0].setAttribute(name, block.getAttribute(name));
      });
  ...
  _tpModifyValuesBeforeSave: function (value) {
    if (
      Object.keys(value).includes("categoryTabsConfig") &&
      value.categoryTabsConfig.activeRecordID &&
      value.categoryTabsConfig.records.length
    ) {
      value.categoryTabsConfig.activeRecordID =
        value.categoryTabsConfig.records[0].id;
    }
    return value;
  },
  /**
  ...
  _computeWidgetVisibility: function (widgetName, params) {
    switch (widgetName) {
      case "show_variant_image": {
        return this.$target[0].classList.contains(
