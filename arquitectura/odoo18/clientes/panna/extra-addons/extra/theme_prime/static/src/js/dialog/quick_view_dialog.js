  ...
  async onWillStart() {
    const result = await rpc("/theme_prime/get_quick_view_html", {
      options: {
        ...
        variant_selector: this.props.isVariantSelector,
      },
    });
    if (result) {
      this.content = result;
      ...
      if (
        this.props.isVariantSelector &&
        ($(result).hasClass("auto-add-product") ||
          $(result).hasClass("tp-product-out-of-stock")) &&
        !$(result).hasClass("tp-combo-product")
      ) {
        this.props.autoAddCallback({
          inStock: !$(result).hasClass("tp-product-out-of-stock"),
          productTmplID: parseInt($(result).get(0).dataset.productTmplId),
        });
        this.props.close();
      }
    }
...
publicWidget.registry.d_product_quick_view = publicWidget.Widget.extend({
  selector:
    '.tp-product-quick-view-action, .tp_hotspot[data-on-hotspot-click="modal"]',
  read_events: {
  ...
  _onClick: function (ev) {
    this.call("dialog", "add", QuickViewDialog, {
      parent: this,
      productTmplId: parseInt($(ev.currentTarget).attr("data-product-id")),
    });
  },
});
