...
publicWidget.registry.WebsiteSale.include(
  Object.assign({}, CartManagerMixin, tpAddToCartMixin, {
    init() {
    ...
    _onProductReady(isOnProductPage = false) {
      ...
      let isInlineButton = this.$el
        ?.get(0)
        .classList.contains("s_add_to_cart_btn");
      if (isInlineButton) {
        this.isBuyNow = this.el?.dataset?.action === "buy_now";
      }
      let isComboProduct =
        this.el.querySelector(".tp-combo-product") ||
        this.el.classList.contains("tp-combo-product");
      ...
      let dialogOptions = {
        ...
        size: "sm",
        autoAddCallback: () => this._submitForm(),
      };
      if (isInlineButton) {
        if (!this.rootProduct.product_id) {
          dialogOptions["productTmplId"] = parseInt(
            this.rootProduct.product_template_id,
            10,
          );
        } else {
      ...
      /*  We assume is qty selector is not present the it will not have the
        ...
        */
      const variantSelectorNeeded = !this.$form.find(".js_add_cart_variants")
        .length;
    ...
    _isDefaultCartFLow: function () {
      return !["side_cart", "dialog", "notification"].includes(
        odoo.dr_theme_config.cart_flow,
      );
    },
  }),
);

publicWidget.registry.AddToCartSnippet.include(
  Object.assign({}, tpAddToCartMixin),
);
