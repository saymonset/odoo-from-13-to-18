...
publicWidget.registry.WebsiteSale.include({
  ...
  events: Object.assign(
    ...
    publicWidget.registry.WebsiteSale.prototype.events || {},
    {
      ...
      "click .tp-filter-attribute-title.collapsible":
        "_onClickFilterAttributeTitle",
  ...
  init: function () {
    ...
    this.isLazyLoad =
      odoo.dr_theme_config.json_shop_layout.layout == "prime" &&
      odoo.dr_theme_config.json_shop_filters.lazy_method;
    this.selectorToReplace = [
      ".tp-category-pill-container",
      ".tp-shop-topbar",
      ".tp-shop-row",
    ];
    ...
    $(".tp-filter-bottom-sidebar-toggle")
      .off()
      .on("click", this._onClickOpenFilterSidebar.bind(this)); // JAT: To work for bottombar
  },
  start() {
    ...
    if (this.el.querySelector(".tp-load-more-on-scroll")) {
        ...
        entries.forEach((entry) => {
          if (entry.intersectionRatio > 0) {
            this._loadMoreProducts(
              this.el
                .querySelector(".tp-load-more-on-scroll")
                .getAttribute("href"),
            );
      ...
      this.loadMoreObserver.observe(
        this.el.querySelector(".tp-load-more-on-scroll"),
      );
    }
    this.isQuickViewDialog = this.el.classList.contains(
      "tp-product-quick-view-layout",
    );
    ...
    if (zoomConfig.zoom_enabled) {
      ...
      for (const image of images) {
        ...
        const imageVals = {
          ...
          zoomFactor: zoomConfig.zoom_factor || 2,
          inlinePane: 992,
          touchDelay: 500,
        };
  ...
  _onChangeCombination: function (ev, $parent, combination) {
    ...
    const $productContainer = $parent.closest(
      "tr.js_product, .oe_website_sale, .o_product_configurator",
    );
    const $stickyAddToCart = $(
      ".tp-sticky-add-to-cart, .tp-bottom-bar-add-to-cart",
    );
    if ($stickyAddToCart.length) {
      $stickyAddToCart
        .find(".oe_currency_value")
        .text(this._priceToStr(combination.price));
      $stickyAddToCart
        .find(".product-img")
        .attr(
          "src",
          "/web/image/product.product/" + combination.product_id + "/image_128",
        );
    ...
    // Product Price Offer
    const $productPriceOfferContainer = $productContainer.find(
      ".tp-product-price-offer-container",
    );
    $productPriceOfferContainer.empty();
    if (
      combination.product_price_offer &&
      typeof combination.product_price_offer === "object"
    ) {
      $(
        renderToElement(
          "theme_prime.ProductPriceOffer",
          combination.product_price_offer,
        ),
      ).appendTo($productPriceOfferContainer);
    ...
    // Bulk Price
    const $bulkPriceContainer = $productContainer.find(
      ".tp-bulk-price-container",
    );
    ...
    if (combination.bulk_price && combination.bulk_price.length) {
      let doRenderPrices =
        combination.bulk_price.length === 1
          ? combination.bulk_price[0].qty > 1
          : true;
      if (doRenderPrices) {
        $(
          renderToElement("theme_prime.BulkPrice", {
            prices: combination.bulk_price,
            inputQty: parseInt($parent.find("input.quantity").val()),
            isQuickViewDialog: this.isQuickViewDialog,
            markup,
            parseInt,
          }),
        ).appendTo($bulkPriceContainer);
      }
    }
    // Extra Fields
  ...
  _updateProductImage: function (
    ...
    let $images = $productContainer.find(
  ...
  _onClickOpenFilterSidebar: function (ev) {
    ...
    const sidebarHtml = renderToElement("theme_prime.ShopFilterSidebar", {
      content: markup($(".tp-filters-container")[0].innerHTML),
    });
  ...
  _onClickSubmitWishlistStockNotificationForm(ev) {
    ...
    const productId = JSON.parse(
      ev.currentTarget.closest(".tp-wishlist-item").dataset.productTrackingInfo,
    ).item_id;
  ...
  _onChangeSearchAttributeValue: function (ev) {
    ...
    if (value) {
      ...
      this.el
        .querySelectorAll('li[data-search-term*="' + value.toLowerCase() + '"]')
        .forEach((el) => {
  ...
  _onClickFilterAttributeTitle: function (ev) {
    if ($(ev.currentTarget).hasClass("expanded")) {
      $(ev.currentTarget)
        .siblings(".tp-filter-attribute-collapsible-area")
        .slideUp("fast");
    } else {
      $(ev.currentTarget)
        .siblings(".tp-filter-attribute-collapsible-area")
  ...
  _onChangeAttribute: function (ev) {
    if (
      ![...ev.currentTarget.classList].some(
        (className) => this.classToIgnore.indexOf(className) !== -1,
      )
    ) {
      this._super.apply(this, arguments);
    }
  ...
  _loadMoreProducts: function (url) {
    this.$shopLoader = $(
      renderToElement("theme_prime.Loader", { height: "20vh" }),
    );
    ...
    this.keepLast
      ...
      .then((data) => {
        $(data)
          .find(".tp-product-item")
          .insertAfter(this.$(".tp-product-item:last"));
        this.$(".tp-product-pager").replaceWith(
          $(data).find(".tp-product-pager"),
        );
        this.trigger_up("widgets_start_request", {
          $target: this.$el,
        });
  ...
  _onClickActionButtonsToggler: function (ev) {
    ...
    ev.currentTarget.parentElement
      .querySelectorAll(".tp-action-toggle")
      .forEach((el) => {
  ...
  destroy: function () {
    ...
    if (
      ...
      this.loadMoreObserver.unobserve(
        this.el.querySelector(".tp-load-more-on-scroll"),
      );
    }
    this.driftImages.forEach((drift) => {
      drift.disable();
    });
    this._super.apply(this, arguments);
  },
...
publicWidget.registry.TpRangeFilter = publicWidget.Widget.extend({
  ...
  jsLibs: [
    "/theme_prime/static/lib/ion.rangeSlider-2.3.1/js/ion.rangeSlider.js",
  ],
  cssLibs: [
    "/theme_prime/static/lib/ion.rangeSlider-2.3.1/css/ion.rangeSlider.css",
  ],
  events: {
  ...
  _onChangeInput: function (ev) {
    ...
    if (parseInt(minValue) > parseInt(maxValue)) {
      this.$(".tp-validate-msg").text(
        _t("Maximum value should be greater than minimum."),
      );
      this.$(".apply").addClass("d-none");
      return false;
    }
...
publicWidget.registry.TpProductPreviewSwatches = publicWidget.Widget.extend({
  ...
  start: function () {
    this.imageEl = this.el
      .closest(this.el.dataset.parentSelector)
      .querySelector(this.el.dataset.imgSelector);
    return this._super.apply(this, arguments);
  },
...
publicWidget.registry.TpProductDetailPage = publicWidget.Widget.extend({
  ...
  start: function () {
    ...
    this.el.querySelectorAll(".tp-navigation-btn").forEach((el, index) => {
      const popoverEl = this.el.querySelector(
        `.tp-navigation-content[data-content-id=${el.dataset.contentId}]`,
      );
      if (popoverEl) {
        ...
        const popover = new Popover(el, {
          animation: true,
          template:
            '<div class="popover border shadow-sm" role="popover"><div class="popover-arrow"></div><div class="popover-body p-0"></div></div>',
    ...
    productDetailTabEl
      .querySelectorAll(":scope > ul.nav-tabs a.nav-link")
      .forEach((el) => {
        el.classList.remove("active");
      });
    productDetailTabEl
      .querySelectorAll(":scope > .tab-content > .tab-pane")
      .forEach((el) => {
        el.classList.remove("active", "show");
      });
    const firstTabEl = productDetailTabEl.querySelector(
      ":scope > ul.nav-tabs > li.nav-item:first-child > a.nav-link",
    );
    if (firstTabEl) {
  ...
  _onClickProductRating: function () {
    const ratingTabEl = this.el.querySelector(
      '[href="#tp-product-rating-tab"]',
    );
...
publicWidget.registry.TpBrandPage = publicWidget.Widget.extend({
  ...
  _onClickBrandSearchAlphabet: function (ev) {
    ...
    if (searchAlphabet === "all") {
      ...
      this.el
        .querySelector(
          '.tp-grouped-brands[data-brand="' + searchAlphabet + '"]',
        )
