...
export class TpDropDown extends Component {
  ...
  get buttonPlaceholder() {
    return (
      this.props.buttonPlaceholder || "theme_prime.tp_dropdown_placeholder"
    );
  }
  get dropDownPlaceholder() {
    return (
      this.props.dropDownPlaceholder || "theme_prime.tp_dropdown_placeholder"
    );
  }
  get menuClass() {
    return this.props.menuClass || "";
  }
  get menuItemClass() {
    return this.props.menuItemClass || "";
  }
}
...
export class TpSearchInput extends CoreComponent {
  ...
  _getCollectionDomain(term) {
    return [
      ["name", "ilike", term],
      ["dr_res_model", "=", this.defaultParams.model],
    ];
  }
  async _onSearch(event) {
    ...
    let extras = {
      brands:
        this.defaultParams.model === "product.attribute.value" && this.brand
          ? this.brand
          : false,
    };
    if (term) {
      let params = {
        ...this.defaultParams,
        domain: this._getSearchDomain(term),
        extras: extras,
        limit: 5,
      };
      this.state.records = await this.componentService._fetchRecords(params, {
        fieldsToMarkup: this.fieldsToMarkup,
      });
      this.state.collections = await this.componentService._fetchRecords({
        model: "dr.snippet.records.collection",
        domain: this._getCollectionDomain(term),
        extras: { dr_res_model: this.defaultParams.model },
      });
    } else {
  ...
  async showSuggestion() {
    this._clearAutoComplete();
    let extras = {
      ids: this.getExcludedRecordsIDs,
      brands:
        this.defaultParams.model === "product.attribute.value" && this.brand
          ? this.brand
          : false,
  ...
  _onFocusout() {
    if (
      this.searchComponent.el &&
      !this.searchComponent.el.contains(document.activeElement)
    ) {
      this._clearAutoComplete();
    }
  }
  ...
  async _onClickRemoveCollection(recordID) {
    await this.componentService._fetchRecords({
      model: "dr.snippet.records.collection",
      domain: [
        ["id", "=", recordID],
        ["dr_res_model", "=", this.defaultParams.model],
  ...
  _getSearchDomain(term) {
    if (term) {
      if (["product.template", "product.product"].includes(this.model)) {
        return [
          ["id", "not in", this.getExcludedRecordsIDs],
          "|",
          ["name", "ilike", term],
  ...
  get resModelInfo() {
    return {
      "product.template": {
        title: _t("Products"),
        template: "theme_prime.tpProductPlaceHolder",
      },
      "product.product": {
        title: _t("Variants"),
        template: "theme_prime.tpProductPlaceHolder",
      ...
      "product.attribute.value": {
        title: this.props.brand ? _t("Brands") : _t("Attributes"),
      },
...
export class TpRangeInput extends Component {
  ...
  onInput(ev) {
    let percentage =
      ((parseInt(ev.currentTarget.value) - this.minValue) /
        (this.maxValue - this.minValue)) *
      100;
    this.input.el.style.backgroundImage = `linear-gradient(90deg, #0080ff ${percentage}%, transparent ${percentage}%)`;
  }
  get maxValue() {
    return this.props.maxValue || 6;
  }
  get minValue() {
    return "minValue" in this.props ? this.props.minValue : 4;
  }
  get title() {
    return this.props.title;
  }
...
export class TpBoolean extends Component {
  ...
  get switch() {
    return this.props.switch || false;
  }
  get value() {
...
export class TpCardGrid extends AbstractComponent {
  ...
  get value() {
    return this.props.value.activeRecordID;
  }
  get activeConfig() {
    return this.getRecordFromData(
      this.props.value.records,
      this.props.value.activeRecordID,
    );
  }
  ...
  get componentRegistry() {
    return {
      TpDropDown: ["style", "productListing"],
      TpRangeInput: ["child", "limit"],
      TpBoolean: ["brand", "label", "count"],
      TpImageUpload: ["background"],
    };
  }
  ...
  get nodeOptions() {
    let buttonClasses = {
      buttonClasses:
        "btn d-flex justify-content-between align-items-center btn-light bg-white border shadow-sm fw-light w-100",
    };
    return {
      style: {
        ...buttonClasses,
        title: _t("Style"),
        records: this.styles.map((style, index) => {
          return { id: style, title: _t(`Style - ${index + 1}`) };
        }),
      },
      productListing: {
        ...buttonClasses,
        title: _t("Product Listing"),
        records: [
          {
            id: "bestseller",
            title: _t("Best Seller"),
            iconTemplate: "dri dri-bolt",
          },
          {
            iconTemplate: "fa fa-percent",
            id: "discount",
            title: _t("Discount"),
          },
        ],
      },
      limit: { title: _t("No. of items"), maxValue: 20, minValue: 0 },
      child: { title: _t("No. of Child"), maxValue: 20, minValue: 3 },
      brand: { title: _t("Brands") },
      label: { title: _t("Label") },
      count: { title: _t("Count") },
      background: { title: _t("Bg. Image") },
    };
  }
  async updateData(IDs) {
    let recordsIDs = IDs || this.recordsIDs;
    let fetchedCategoryData = await this.componentService._fetchRecords(
      {
        options: { getCount: true, categoryIDs: recordsIDs },
        fields: ["dr_category_label_id"],
      },
      { routePath: "/theme_prime/get_categories_info" },
    );
    let records = [];
    recordsIDs.forEach((resID) => {
      ...
      if (matchedRecord) {
        records.push({
          title: matchedRecord.name,
          id: resID,
          imgSrc: `/web/image/product.public.category/${resID}/image_128`,
          subtitle: `${matchedRecord.count} Products`,
        });
      }
    });
    this.state.value = records.length ? this.props.value.activeRecordID : false;
    this.state.records = records;
    this.state.activeConfig = this.getRecordFromData(
      this.props.value.records,
      this.state.value,
    );
  }
  ...
  get styles() {
    return Object.keys(
      registry.category("theme_prime_mega_menu_cards").content,
    );
  }
  _onChangeComponentValue(value, name) {
    if (name === "categories") {
      ...
      this.env.changeValue(
        { activeRecordID: value, records: toRaw(this.props.value.records) },
        "categoryTabsConfig",
      );
    } else {
      ...
      this.env.changeValue(
        {
          activeRecordID: this.state.value,
          records: toRaw(this.props.value.records),
        },
        "categoryTabsConfig",
      );
    }
...
export class TpComponentGroup extends AbstractComponent {
  ...
  get nodeOptions() {
    ...
    return {
      style: {
        ...buttonClasses,
        title: _t("Mobile Style"),
        records: [
          { id: "default", title: _t("Same as Desktop") },
          ...this.mobileStyles.map((style, index) => {
            return { id: style, title: _t(`Style - ${index + 1}`) };
          }),
        ],
      },
      mode: {
        ...
        records: [
          ...
          {
            id: "grid",
            iconTemplate: "fa fa-th-large pe-2",
            title: _t("Grid"),
          },
          {
            iconTemplate: "fa pe-2 fa-arrows-h",
            id: "slider",
            title: _t("Slider"),
          },
        ],
...
export class TpActions extends Component {
  setup() {
    ...
    if (!this.isReadOnly) {
      ...
      onWillStart(async () => {
        this.shopConfig = await this.orm.call(
          "website",
          "get_theme_prime_shop_config",
          [],
  ...
  _getAction(action) {
    let allActions = {
      colors: { icon: "theme_prime.icon_brush", label: _t("Colors") },
      count: { icon: "theme_prime.icon_hash_tag", label: _t("Count") },
      brand: { icon: "theme_prime.icon_tag", label: _t("Brands") },
      quick_view: { icon: "theme_prime.icon_eye", label: _t("QUICK VIEW") },
      add_to_cart: { icon: "theme_prime.icon_cart", label: _t("ADD TO CART") },
      category_info: { icon: "theme_prime.icon_font", label: _t("CATEGORY") },
      wishlist: { icon: "theme_prime.icon_heart", label: _t("WISHLIST") },
      comparison: { icon: "theme_prime.icon_exchange", label: _t("COMPARE") },
      rating: { icon: "theme_prime.icon_star", label: _t("RATING") },
      description_ecommerce: {
        icon: "theme_prime.icon_description",
        label: _t("DESCRIPTION"),
      },
      label: { icon: "theme_prime.icon_tag", label: _t("LABEL") },
      show_similar: { icon: "theme_prime.icon_box", label: _t("SIMILAR") },
    };
    let selectedAction = Object.keys(allActions).includes(action)
      ? allActions[action]
      : false;
    if (
      !this.isReadOnly &&
      ["rating", "wishlist", "comparison"].includes(action) &&
      !this.shopConfig[`is_${action}_active`]
    ) {
      selectedAction["disabled"] = true;
      selectedAction["title"] =
        `${action} is disabled from the shop if you want to use it please enable it from the shop`;
    }
...
export class TpDatePicker extends AbstractComponent {
    ...
    onMounted(() => {
      this.picker = this.dateTimePicker.create({
        ...
        pickerProps: {
          ...
          value: DateTime.fromSeconds(parseInt(this.state.value)),
          rounding: 0,
        },
      });
      this.picker.enable();
  ...
  _updateCounters() {
    ...
    if (dueDate.includes("-")) {
      ...
      eventTime = luxon.DateTime.fromISO(
        new Date(parseInt(dueDate) * 1000).toISOString(),
      );
    }
...
export class TpDomainComponent extends AbstractComponent {
  setup() {
    ...
    this.state = useState({
      ...
      condition:
        this.value.domain.length && this.value.domain[0] === "|" ? "or" : "and",
    });
    this._coreProps = {
      changeValue: this._onChangeComponentValue.bind(this),
    };
    this._coreCProps = {
      updateSelectionComponentValue:
        this.updateSelectionComponentValue.bind(this),
  ...
  _onChangeComponentValue(value, name) {
    if (name === "domainTemplate") {
      ...
      this.state.domainProps = this.prepareDomainValues(
        this._normalizeDomain(domain),
      );
      if (sortBy) {
  ...
  onChangeValue(ev, index, fieldType) {
    let value =
      fieldType === "integer" ? parseInt(ev.target.value) : ev.target.value;
  ...
  get fieldsList() {
    return [
      { type: "text", name: "name", label: _t("Name") },
      {
        type: "many2many",
        name: "public_categ_ids",
        label: _t("Category"),
        relationModel: "product.public.category",
      },
      {
        type: "many2one",
        name: "dr_brand_value_id",
        label: _t("Brand"),
        relationModel: "product.attribute.value",
        extras: { brands: true },
      },
      {
        type: "many2one",
        name:
          this.value.environmentModel === "product.template"
      ...
      },
      {
        ...
        name: "dr_label_id",
        label: _t("Label"),
        relationModel: "dr.product.label",
      },
      { type: "integer", name: "list_price", label: _t("Price") },
      {
        type: "many2one",
        name: "product_tag_ids",
        label: _t("Tags"),
        relationModel: "product.tag",
        is_multi_website: true,
      },
      { type: "boolean", name: "dr_has_discount", label: _t("Discount") },
    ];
  }
  get domainTemplates() {
    return [
      {
        id: 0,
        subtitle: _t("Select any template and modify as per your need."),
        title: _t("Choose Template"),
        domain: [],
        sortBy: "create_date desc",
      },
      {
        id: 1,
        subtitle: _t("Show newly arrived products based on creation date"),
        title: _t("New Arrival"),
        domain: [],
        sortBy: "create_date desc",
      },
      {
        id: 2,
        subtitle: _t("Show newly arrived products from selected categories"),
        title: _t("Category New Arrival"),
        domain: [["public_categ_ids", "in", []]],
        sortBy: "create_date desc",
      },
      {
        id: 3,
        subtitle: _t("Show newly arrived products from selected brands"),
        title: _t("Brand New Arrival"),
        domain: [["dr_brand_value_id", "in", []]],
        sortBy: "create_date desc",
      },
      {
        id: 4,
        subtitle: _t("Show newly arrived products from selected tags"),
        title: _t("Tags New Arrival"),
        domain: [["product_tag_ids", "in", []]],
        sortBy: "create_date desc",
      },
      {
        id: 5,
        subtitle: _t("Show newly arrived products from selected label."),
        title: _t("Label New Arrival"),
        domain: [["dr_label_id", "in", []]],
        sortBy: "create_date desc",
      },
      {
        id: 6,
        subtitle: _t("Show discounted products based on product pricelist"),
        title: _t("Discounted Products"),
        domain: [["dr_has_discount", "!=", false]],
        sortBy: "list_price asc",
      },
      {
        id: 7,
        subtitle: _t(
          "Show discounted products based on product pricelist from selected categories",
        ),
        title: _t("Category Discounted Products"),
        domain: [
          "&",
          ["dr_has_discount", "!=", false],
          ["public_categ_ids", "in", []],
        ],
        sortBy: "list_price asc",
      },
      {
        id: 8,
        subtitle: _t(
          "Show discounted products based on product pricelist from selected brands.",
        ),
        title: _t("Brand Discounted Products"),
        domain: [
          "&",
          ["dr_has_discount", "!=", false],
          ["dr_brand_value_id", "in", []],
        ],
        sortBy: "list_price asc",
      },
      {
        id: 9,
        subtitle: _t(
          "Show discounted products based on product pricelist from selected tags",
        ),
        title: _t("Tags Discounted Products"),
        domain: [
          "&",
          ["dr_has_discount", "!=", false],
          ["product_tag_ids", "in", []],
      ...
      },
      {
        id: 10,
        subtitle: _t(
          "Show discounted products based on product pricelist from selected label",
        ),
        title: _t("Label Discounted Products"),
        domain: [
          "&",
          ["dr_has_discount", "!=", false],
          ["dr_label_id", "in", []],
      ...
      {
        id: 11,
        subtitle: _t("Show best seller products based on last 30 days sales"),
        title: _t("Best Seller"),
        domain: [],
        sortBy: "bestseller",
      },
      {
        id: 12,
        subtitle: _t(
          "Show best seller products based on last 30 days sales from selected categories.",
        ),
        title: _t("Category Best Seller"),
        domain: [["public_categ_ids", "in", []]],
        sortBy: "bestseller",
      },
      {
        id: 13,
        subtitle: _t(
          "Show best seller products based on last 30 days sales from selected brands.",
        ),
        title: _t("Brand Best Seller"),
        domain: [["dr_brand_value_id", "in", []]],
        sortBy: "bestseller",
      },
      {
        id: 14,
        subtitle: _t(
          "Show best seller products based on last 30 days sales from selected tags.",
        ),
        title: _t("Tags Best Seller"),
        domain: [["product_tag_ids", "in", []]],
        sortBy: "bestseller",
      },
      {
        id: 15,
        subtitle: _t(
          "Show best seller products based on last 30 days sales from selected label.",
        ),
        title: _t("Label Best Seller"),
        domain: [["dr_label_id", "in", []]],
        sortBy: "bestseller",
      },
      {
        id: 16,
        subtitle: _t("Show best seller products with discount"),
        title: _t("Discounted Best Seller"),
        domain: [["dr_has_discount", "!=", false]],
        sortBy: "bestseller",
      },
      {
        id: 17,
        subtitle: _t(
          "Show best seller products with discount from the selected categories",
        ),
        title: _t("Category Discounted Best Seller"),
        domain: [
          "&",
          ["public_categ_ids", "in", []],
          ["dr_has_discount", "!=", false],
      ...
      },
      {
        id: 18,
        subtitle: _t(
          "Show best seller products with discount from the selected brands",
        ),
        title: _t("Brand Discounted Best Seller"),
        domain: [
          "&",
          ["dr_brand_value_id", "in", []],
          ["dr_has_discount", "!=", false],
      ...
      },
      {
        id: 19,
        subtitle: _t(
          "Show best seller products with discount from the selected tags",
        ),
        title: _t("Tags Discounted Best Seller"),
        domain: [
          "&",
          ["product_tag_ids", "in", []],
          ["dr_has_discount", "!=", false],
      ...
      },
      {
        id: 20,
        subtitle: _t(
          "Show best seller products with discount from the selected label",
        ),
        title: _t("Label Discounted Best Seller"),
        domain: [
          "&",
          ["dr_label_id", "in", []],
          ["dr_has_discount", "!=", false],
  ...
  _prepareRecords(index) {
    ...
    return {
      componentData: {
        ...
        recordsIDs: domain[2],
        model: this.getRecord(this.fieldsList, field, "name").relationModel,
      },
      name: index,
      extras: {
        ...
        mode: "badge",
        isBrand:
          this.getRecord(this.fieldsList, field, "name").relationModel ===
            "product.attribute.value" && field === "dr_brand_value_id"
            ? true
            : false,
      },
    };
  ...
  onChangeField(ev, index) {
    ...
    this.state.domainProps[index] = {
      domain: [fieldName, op, this.operatorInfo[fieldType].defaultVal],
      editMode: true,
      records: [],
    };
  }
  onChangeOperator(ev, index) {
    ...
    this.state.domainProps[index] = {
      domain: [domain[0], ev.target.value, domain[2]],
      editMode: true,
      records: records,
    };
  }
  ...
  get operatorInfo() {
    return {
      ...
      many2many: {
        value: {
          in: "is having",
          "not in": "is not having",
          child_of: "is having child",
        },
        defaultVal: [],
      },
      many2one: {
        value: { in: "is having", "not in": "is not having" },
        defaultVal: [],
      },
      integer: {
        value: {
  ...
  get nodeOptions() {
    return {
      limit: { title: _t("No. of items"), maxValue: 20, minValue: 4 },
      order: {
        buttonClasses:
          "btn d-flex btn-sm justify-content-between align-items-center btn-light bg-white border shadow-sm fw-light w-100",
        title: _t("Order By"),
        records: [
          { id: "list_price asc", title: _t("Price: Low to High") },
          { id: "list_price desc", title: _t("Price: High to Low") },
          { id: "name asc", title: _t("Name: A to Z") },
          { id: "name desc", title: _t("Name: Z to A") },
          { id: "create_date desc", title: _t("Newly Arrived") },
          { id: "bestseller", title: _t("Bestseller") },
...
registry
  .category("theme_components")
  .add("TpDatePicker", TpDatePicker)
  .add("TpRangeInput", TpRangeInput)
  .add("TpBoolean", TpBoolean)
  .add("TpCardGrid", TpCardGrid)
  .add("TpDropDown", TpDropDown)
  .add("TpActions", TpActions)
  .add("TpImageUpload", TpImageUpload)
  .add("TpComponentGroup", TpComponentGroup)
  .add("TpDomainComponent", TpDomainComponent)
  .add("TpSearchInput", TpSearchInput);
