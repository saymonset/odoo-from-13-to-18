...
searchBar.include(
  Object.assign({}, B2bMixin, {
    ...
    init: function () {
      ...
      this.advanceMode =
        odoo.dr_theme_config.json_product_search.advance_search;
      this.search_reports =
        odoo.dr_theme_config.json_product_search.search_report;
          ...
          if (record[fieldName]) {
            if (typeof record[fieldName] === "object") {
              for (const fieldKey of Object.keys(record[fieldName])) {
                record[fieldName][fieldKey] = markup(
                  record[fieldName][fieldKey],
                );
              }
            } else {
    ...
    _onKeydown: function (ev) {
      if (
        ...
        const focusableEls = [
          this.$input[0],
          ...[...this.$menu[0].children].filter((item) => {
            return item.classList.contains("dropdown-item");
          }),
        ];
    ...
    _onClickSearchResult: function (ev) {
      if (this.search_reports) {
        ...
        if (search_type == "product") {
          this.searchReportData["clicked_string"] = $searchResult
            .find(".h6")
            .text()
            .trim();
        } else {
