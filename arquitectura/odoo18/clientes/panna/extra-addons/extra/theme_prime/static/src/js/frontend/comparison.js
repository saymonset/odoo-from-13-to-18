...
publicWidget.registry.ProductComparison.include(
  Object.assign({}, B2bMixin, {
    ...
    start: function () {
      ...
      // });
      if (
        this.$(".tp-droggol-18-builder-snippet").length ||
        this.$target.hasClass("oe_website_sale") ||
    ...
    _onClickCompareBtn: function (ev) {
      if (
        this.productComparison.comparelist_product_ids.length <
        this.productComparison.product_compare_limit
      ) {
        this.productComparison._addNewProducts(
          parseInt($(ev.currentTarget).get(0).dataset.productProductId),
        );
      } else {
