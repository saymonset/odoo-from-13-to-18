...
class ProductTemplate(models.Model):
    ...
    dr_has_discount = fields.Boolean(
        compute="_compute_dr_has_discount", search="_search_dr_has_discount"
    )
    ...
    def _need_catch_update(self, pricelist_id, catch_date):
        ...
        item_ids_now = self._pricelist_items_for_date(
            pricelist_id, fields.Datetime.to_string(fields.Datetime.now())
        )
        if set(item_ids_catch) != set(item_ids_now):
            return True

        if (
            self.env["product.pricelist"].browse(pricelist_id).write_date
            > catch_date_obj
        ):
            return True

        product_grouped_data = self.env["product.template"].read_group(
            [("sale_ok", "=", True)], ["write_date:max"], ["sale_ok"]
        )
    ...
    def _get_product_pricelist_data(self, pricelist_id):
        discounted_product_ids, catch_date = self._get_product_pricelist_cache(
            pricelist_id
        )
    ...
    def _get_product_pricelist_cache(self, pricelist_id):
        ...
        )  # Need sudo so all products are calculated
    ...
    def _dr_process_product_data(self, product_pricelist_data, product):
        return {
            "display_name": product_pricelist_data["display_name"],
            "price": product_pricelist_data["price"],
            "id": product_pricelist_data["product_template_id"],
        }
    ...
    def _get_product_category_count(self, domain=[]):
        ...
        all_categ_data = [
            {
                "path": pc.parent_path,
                "parent_id": pc.parent_id.id,
                "product_public_category_id": pc.id,
            }
            for pc in all_categ
        ]
            ...
            for line2 in query_res:
                if line.get("parent_id"):
                    ...
                    if path_pattern in line2.get("path"):
                        result_count[line.get("product_public_category_id")] += (
                            line2.get("count")
                        )
                else:
                    path_pattern = "%s/" % line.get("product_public_category_id")
    ...
    def _get_product_attrib_count(self, attrib_values, domain=[]):
        ...
        query = SQL(
            """
                ...
                %(from_clause)s
                WHERE %(where_clause)s or product_template.id is NULL
        ...
        if attrib_values:
            ...
            attrib_value_list = dict(
                [
                    (line.get("id"), line.get("product_tmpl_ids") or [])
            ...
            for line in query_res:
                ...
                if value_id_1 in attrib_values_ids:
                    attrib_p_list[attrib_id_1] = attrib_p_list[attrib_id_1] | set(
                        product_ids_1
                    )
            ...
            for line in query_res:
                ...
                for line_2 in query_res:
                    ...
                    if (
                        value_id_2 not in attrib_vals_map.get(attrib_id_1, [])
                        and value_id_2 in attrib_values_ids
                    ):
                        attrib_product_list[value_id_1] = attrib_product_list[
                            value_id_1
                        ] & attrib_p_list.get(attrib_id_2, set())

            result_count = dict(
                [
                    (val_id, len(product_ids))
                    for val_id, product_ids in attrib_product_list.items()
                ]
            )
        else:
            result_count = dict(
                [
                    (line.get("id"), len(line.get("product_tmpl_ids") or []))
                    for line in query_res
                ]
            )
        return result_count
