...
class IrAttachment(models.Model):
    ...
    cfdi_type = fields.Selection(
        [
            ...
            ),  # customer invoice, Emisor.RFC=myself.VAT, Customer invoice
            ("SI", "Facturas de proveedor"),  # Emisor.RFC!=myself.VAT, Supplier bill
            (
                "E",
                "Notas de crédito clientes",
            ),  # customer credit note, Emisor.RFC=myself.VAT, Customer credit note
            (
                "SE",
                "Notas de crédito proveedor",
            ),  # Emisor.RFC!=myself.VAT, Supplier credit note
            ("P", "REP de clientes"),  # Emisor.RFC=myself.VAT, Customer payment receipt
            (
                "SP",
                "REP de proveedores",
            ),  # Emisor.RFC!=myself.VAT, Supplier payment receipt
            (
                "N",
                "Nominas de empleados",
            ),  # currently we shall not do anythong with this type of cfdi, Customer Payslip
            (
                "SN",
                "Nómina propia",
            ),  # currently we shall not do anythong with this type of cfdi, Supplier Payslip
    ...
    invoice_count = fields.Integer(
        compute="_compute_account_invoice_count", string="# de facturas", store=True
    )

    payment_ids = fields.One2many("account.payment", "attachment_id", "Pagos")
    payment_count = fields.Integer(
        compute="_compute_account_payment_count", string="# de pagos", store=True
    )
    ...
    def _read_group_allowed_fields(self):
        return super(IrAttachment, self)._read_group_allowed_fields() + [
            "creado_en_odoo",
            "date_cfdi",
            "nombre_tercero",
            "serie_folio",
            ...
            if ctx.get("is_fiel_attachment"):
                ...
                if datas:
                    ...
                    tfd_elements = tree.xpath(
                        "//tfd:TimbreFiscalDigital", namespaces=tfd_namespace
                    )
                    ...
                    try:
                        elements = tree.xpath(
                            "//*[re:test(local-name(), '%s','i')]" % (element_tag),
                            namespaces=ns,
                        )
                    except Exception:
    ...
    def action_view_payments(self):
        ...
        if payments and payments[0].payment_type == "outbound":
            action = (
                self.env.ref("account.action_account_payments_payable").sudo().read()[0]
            )
        else:
        ...
        if len(payments) > 1:
            ...
            action["views"] = [
                (self.env.ref("account.view_account_payment_form").sudo().id, "form")
            ]
    ...
    def action_view_invoice(self):
        ...
        if self.cfdi_type == "I":
            ...
            if len(invoices) > 1:
                ...
                action["views"] = [
                    (self.env.ref("account.view_move_form").sudo().id, "form")
                ]
    ...
    def update_status_from_ir_attachment_document(self):
        attchment_list = (
            self.env["ir.attachment"].sudo().search([("creado_en_odoo", "=", False)])
        )
        for attchment in attchment_list:
            if attchment.cfdi_uuid and attchment.cfdi_type:
                if attchment.cfdi_type == "I" or attchment.cfdi_type == "E":
                    for uu in [
                        attchment.cfdi_uuid,
                        attchment.cfdi_uuid.lower(),
                        attchment.cfdi_uuid.upper(),
                    ]:
                        customer_invoices = self.env["account.move"].search(
                            [("folio_fiscal", "=", uu)], limit=1
                        )
                ...
                if attchment.cfdi_type == "P" or attchment.cfdi_type == "SP":
                    for uu in [
                        ...
                        customer_invoices = self.env["account.payment"].search(
                            [("folio_fiscal", "=", uu)], limit=1
                        )
                        if customer_invoices:
                            customer_invoices.write({"attachment_id": attchment.id})
    ...
    def action_extract_zip(self):
        ...
        attachment = (
            ...
            .with_context(is_fiel_attachment=False)
            .create(
                {
                    "name": file_name,
                    "datas": zip_datas,
    ...
    def action_download_state(self):
        for attach in self:
            if (
                attach.cfdi_type == "I"
                or attach.cfdi_type == "E"
                or attach.cfdi_type == "P"
                or attach.cfdi_type == "N"
                or attach.cfdi_type == "T"
            ):
                try:
                    total = attach.cfdi_total if not attach.cfdi_type == "P" else 0
                    status = self.get_sat_status(
                        attach.company_id.vat,
                        attach.rfc_tercero,
                        total,
                        attach.cfdi_uuid,
                    )
                ...
                attach.cfdi_type == "SI"
                or attach.cfdi_type == "SE"
                or attach.cfdi_type == "SP"
                or attach.cfdi_type == "SN"
                or attach.cfdi_type == "ST"
            ):
                try:
    ...
    def check(self, mode, values=None):
        ...
        if self:
            ...
            self._cr.execute(
                "SELECT res_model, res_id, create_uid, public, res_field, cfdi_uuid FROM ir_attachment WHERE id IN %s",
                [tuple(self.ids)],
            )
            for (
                res_model,
    ...
    def update_extra_info_xml(self):
        for attach in self:
            ...
            monto_total = 0
            cfdi_iva = cfdi_isr = cfdi_ieps = cfdi_iva_ret = cfdi_isr_ret = (
                cfdi_ieps_ret
            ) = 0
                ...
                if todos_impuestos is not None:
                    ...
                    if traslados is not None:
                        for traslado in traslados:
                            if (
                                traslado.attrib["Impuesto"] == "002"
                                and traslado.attrib["TipoFactor"] != "Exento"
                            ):
