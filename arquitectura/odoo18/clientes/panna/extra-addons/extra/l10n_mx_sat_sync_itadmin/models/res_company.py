...
class ResCompany(models.Model):
    ...
    def verifica_solicitud_descarga(self):
        for company in self.search([("l10n_mx_esignature_ids", "!=", False)]):
            if (
                ...
                solicitud_ws_ids = self.env["solicitud.ws"].search(
                    [
                        ("cod_estatus", "=", "5000"),
                        ("state", "=", "draft"),
                ...
                if solicitud_ws_ids:
                    for solicitud_ws in solicitud_ws_ids:
                        esignature_ids = company.l10n_mx_esignature_ids
                        esignature = esignature_ids.with_user(
                            self.env.user
                        ).get_valid_certificate()
                        if not esignature:
                            continue  # raise UserError(_("No valid E-Signature found."))
                        sat_obj = SAT(
                            esignature.content, esignature.key, esignature.password
                        )
                        solicitud = {
                            "id_solicitud": solicitud_ws.id_solicitud,
                            "cod_estatus": solicitud_ws.cod_estatus,
                            "mensaje": solicitud_ws.mensaje,
                        }
                        company.save_downloaded_content(
                            esignature, sat_obj, solicitud, solicitud_ws.rfc_emisor
                        )
    ...
    def download_cfdi_invoices_api(self, start_date=False, end_Date=False):
        ...
        diff = abs(
            pytz.timezone(timezone).utcoffset(datetime.now()).total_seconds()
            / (60 * 60)
        )
        ...
        if not solicitud_ws_ids:
            solicitud = sat_obj.soap_request_download(
                token=token, date_from=date_from, date_to=date_to, rfc_receptor=True
            )
            solicitud_ws_ids = self.env["solicitud.ws"].create(
                {
            ...
            solicitud = {
                "id_solicitud": solicitud_ws_ids.id_solicitud,
                "cod_estatus": solicitud_ws_ids.cod_estatus,
                "mensaje": solicitud_ws_ids.mensaje,
            }

        self.save_downloaded_content(esignature, sat_obj, solicitud, False)

        solo_documentos_de_proveedor = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_mx_sat_sync_itadmin_ee.solo_documentos_de_proveedor")
        )
    ...
    def save_downloaded_content(
        ...
        # 5, Rechazada
        # 6, Vencida
        solicitud_ws = self.env["solicitud.ws"].search(
            [("id_solicitud", "=", solicitud["id_solicitud"])], limit=1
        )
        solicitud_ws.write(
            {
                "cod_verifica": verificacion["cod_estatus"],
        ...
        if estado_verificacion == 3:
            for paquete in verificacion["paquetes"]:
                ...
                solicitud_ws.write(
                    {
                        "cod_descarga": descarga["cod_estatus"],
                ...
                if descarga["cod_estatus"] == "5000":
                    ...
                    solicitud_ws.write(
                        {
                            "paquete_b64": descarga["paquete_b64"],
                        }
                    )
            ...
            for uuid, data in invoice_content_receptor.items():
                ...
                if b"xmlns:schemaLocation" in xml_content:
                    ...
                    self.env["bus.bus"]._sendone(
                        ...
                        {
                            "title": "Error",
                            "message": "Límite de descarga alcanzado",
                            "sticky": False,
                            "warning": True,
                        },
                    )
                    _logger.info(
                        "Ya no puedes descargar más documentos. El SAT permite descargar un máximo de 2,000 archivos por día."
                    )
                    continue
                try:
                    tree = etree.fromstring(xml_content)
        ...
        if invoice_content_emisor:
            ...
            for uuid, data in invoice_content_emisor.items():
                ...
                if cfdi_type == "P":
                    for uu in [uuid, uuid.lower(), uuid.upper()]:
                        payment_exist = payment_obj.search(
                            [("folio_fiscal", "=", uu), ("company_id", "=", self.id)],
                            limit=1,
                        )
                        if payment_exist:
                            vals.update(
