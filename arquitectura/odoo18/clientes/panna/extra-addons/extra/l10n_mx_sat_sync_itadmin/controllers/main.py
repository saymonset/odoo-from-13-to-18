...
class ActionSatSync(Action):
    ...
    def load(self, action_id, additional_context=None):
        ...
        if value and value.get("xml_id", "") in [
            "l10n_mx_sat_sync_itadmin.action_attachment_cfdi_supplier_invoices",
            "l10n_mx_sat_sync_itadmin.action_importar_xml",
        ]:
            try:
                ...
                if "company_id" not in ctx:
                    cids = request.httprequest.cookies.get(
                        "cids", str(request.env.company.id)
                    )
                    company_ids = [int(cid) for cid in cids.split(",")]
                    company_id = (
                        company_ids and company_ids[0] or request.env.company.id
                    )
