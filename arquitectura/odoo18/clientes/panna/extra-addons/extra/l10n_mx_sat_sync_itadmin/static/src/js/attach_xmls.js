...
export class IbanWidget extends CharField {
  ...
  _onButtonSave(e) {
    e.preventDefault();
    document
      .querySelectorAll(".alert-warning.dnd-alert")
    ...
    if (Object.keys(this.files).length <= 0) {
      ...
      this.env.model.root.context.active_ids
    ) {
      this.notification.add(_t("There is no files selected"), {
        type: "danger",
      });
      ...
      document.getElementById("dndfooter").querySelector("#save").disabled =
        true;
      document
        .getElementById("filescontent")
        .querySelectorAll(".xml_cont")
        .forEach((element) => {
  ...
  remove_all_xml(e) {
    ...
    const wrong_files = Object.values(files).filter(
      (file) => file.iscorrect === false,
    );
    const readfiles = {};
    ...
    wrong_files.forEach((file) => {
      if (!file.iscorrect) {
        ...
        fr.onload = function () {
          ...
          if (
            Object.keys(wrong_files).length === Object.keys(readfiles).length
          ) {
            rpc(
              "/web/dataset/call_kw/multi.file.attach.xmls.wizard/remove_wrong_file",
              {
    ...
    Object.keys(self.alerts_in_queue.alertHTML).forEach((filekey) => {
      ...
      if (
        file.alert &&
        file.alert[0] &&
        file.alert[0].innerText.includes(
          "The XML UUID</span> belong to other move",
        )
      ) {
        ...
        file.alert[0] &&
        file.alert[0].innerText.includes(
          "The move reference</span> belong to other",
        )
      ) {
        if (filekey_current === filekey) {
          remove_current_file = true;
  ...
  remove_xml(e) {
    ...
    if (type === "remove") {
      ...
      this.sendErrorToServer(
        this.alerts_in_queue.alertHTML[filekey].xml64,
        filekey,
        "check_xml",
      );
  ...
  handleFileUpload(files) {
    ...
    if (self.uploading_files) {
      ...
      Array.from(files).forEach(function (file) {
        if (file.type !== "text/xml") {
          ...
          Object.prototype.hasOwnProperty.call(self.files, file.name)
        ) {
    ...
    document
      .getElementById("xml_cont_hover")
      .addEventListener("click", self.remove_single_xml.bind(self));
  }
  ...
  sendFileToServer(files) {
    ...
    rpc(`/web/dataset/call_kw/multi.file.attach.xmls.wizard/check_xml`, {
      ...
      if (
        Object.keys(wrongfiles).length === 0 ||
  ...
  createdCorrectly(key) {
    var self = this;
    var alert = document.querySelector(
      '#filescontent div[title="' + key + '"]',
    );
  ...
  handleFileWrong(wrongfiles) {
    ...
    Object.keys(wrongfiles).forEach(function (key) {
      ...
      if ("cfdi_type" in file) {
        ...
        if (removeAllXmlBtn) {
          removeAllXmlBtn.addEventListener(
            "click",
            self.remove_all_xml.bind(self),
          );
        }
  ...
  prepareWrongAlert(key, data) {
    ...
    if (able_buttons.includes("partner") && !able_buttons.includes("remove")) {
      buttons +=
        ...
        _t(
          '<button class="dnd-alert-button" tag="partner">Create Partner</button>',
        );
    } else if (able_buttons.includes("tryagain")) {
      buttons +=
        _t(
          '<button id="remove_xml" class="dnd-alert-button remove" tag="remove" t-on-click="remove_xml">Remove XML</button>',
        ) +
        _t(
          '<button class="dnd-alert-button" tag="tryagain" id="tryagain">Try again</button>',
        );
    } else if (able_buttons.includes("remove")) {
      buttons += _t(
        '<button class="dnd-alert-button" tag="remove" t-on-click="remove_xml">Remove XML</button>',
      );
    }
    if (buttons.length > 0) {
      buttons += _t(
        '<button id="remove_all_xml" class="dnd-alert-remove-all-wrong-xml-button" style="background-color: #31708f; border: 0px solid; margin: 0 5px 0 0; color: #ffffff;" tag="remove">Remove all wrong XML</button>',
      );
    }
    return {
      errors: errors,
  ...
  wrongMsgXml(file, able_buttons) {
    ...
    var map_error = {
      unsigned: _t(
        '<div><span level="1">UUID</span> not found in the XML.</div>',
      ),
      version: _t(
        '<div><span level="1">Unable to generate moves from an XML with version 3.2.</span>You can create the move manually and then attach the xml.</div>',
      ),
      nothing: _t(
    ...
    Object.keys(file).forEach(function (ikey) {
      ...
      if (ikey === "wrong_company_i") {
        errors +=
          _t(
            '<div><span level="1">The XML Issuer RFC</span> does not match with <span level="1">your Company RFC</span>: ',
          ) +
          _t('XML Issuer RFC: <span level="2">') +
          val[0] +
          _t(', </span> Your Company RFC: <span level="2">') +
          val[1] +
          "</span></div>";
      ...
      if (
        ikey !== "wrong_company_r" &&
        ikey !== "wrong_company_i" &&
        ikey !== "partner_not_found" &&
        ikey !== "xml64" &&
      ...
      if (ikey === "partner_not_found") {
        errors +=
          _t(
            '<div><span level="1">The XML partner</span> was not found: <span level="2">',
          ) +
          val +
          "</span>.</div>";
      ...
      if (ikey === "reference_multi") {
        errors +=
          _t(
            '<div><span level="1">The XML reference</span> matches another move reference. <span level="1">Partner: </span>',
          ) +
          val[0] +
      ...
      if (ikey === "taxes") {
        errors +=
          _t(
            '<div><span level="1">Some taxes</span> do not exist: <span level="2">',
          ) +
          val.join(", ") +
          "</span>.</div>";
      }
      if (ikey === "taxes_wn_accounts") {
        errors +=
          _t(
            '<div><span level="1">Some taxes</span> do not have account assigned: <span level="2">',
          ) +
          val.join(", ") +
          "</span>.</div>";
      }
      if (ikey === "uuid_duplicated") {
        errors +=
          _t(
            '<div><span level="1">The XML UUID</span> belong to other move. <span level="1">UUID: </span>',
          ) +
          val +
          "</div>";
      }
      if (ikey === "cancelled") {
        errors +=
          _t(
            '<div><span level="1">The XML state</span> is CANCELLED in SATs system. ',
          ) +
      ...
      if (ikey === "folio") {
        errors +=
          _t(
            '<div><span level="1">The XML Folio</span> does not match with <span level="1">Partner document number</span>: ',
          ) +
          _t('XML Folio: <span level="2">') +
          val[0] +
          _t(', </span> Partner document number: <span level="2">') +
          val[1] +
          "</span></div>";
      }
      if (ikey === "amount") {
        errors +=
          _t(
            '<div><span level="1">The XML amount total</span> does not match with <span level="1">move total</span>: ',
          ) +
          _t('XML amount total: <span level="2">') +
          val[0] +
          _t(', </span> Move total: <span level="2">') +
          val[1] +
          "</span></div>";
        able_buttons.push("tryagain");
      }
      if (ikey === "amount_tax") {
        errors +=
          _t(
            '<div><span level="1">The XML tax total amount</span> does not match with <span level="1">Move tax total amount</span>: ',
          ) +
          _t('XML tax total: <span level="2">') +
          val[0] +
          _t(', </span> Move tax total: <span level="2">') +
          val[1] +
          "</span> Ref: " +
          val[2] +
          "</div>";
      ...
      if (ikey === "moves_related_not_found") {
        errors +=
          _t(
            '<div>The <span level="1">TipoDeComprobante</span> is <span level="1">"E"</span> and The XML UUIDs are not related to any move. <span level="1">UUID: </span>',
          ) +
          val +
          "</div>";
        able_buttons.push("tryagain");
      ...
      if (ikey === "no_node_related_uuids") {
        errors +=
          _t(
            '<div>The <span level="1">TipoDeComprobante</span> is <span level="1">"E"</span> and The node CFDI related is not set</span> Manually reconcile with the appropiate move. <span level="1">UUID: </span>',
          ) +
          val +
          "</div>";
      }
  ...
  sendErrorToServer(xml64, key, function_def) {
    ...
    rpc("/web/dataset/call_kw/multi.file.attach.xmls.wizard/" + function_def, {
      ...
      Object.keys(attachments).forEach(function (rkey) {
        ...
        var alertobj = document.querySelector(
      ...
      Object.keys(wrongfiles).forEach(function (rkey) {
        ...
        var footer = alertobj.querySelector(
        ...
        alertobj.innerHTML =
          alert_parts.errors + "<div>" + alert_parts.buttons + "</div>";
  ...
  removeWrongAlerts(alertobj, filekey, removefile) {
    ...
    if (removefile) {
      ...
      var fileElement = document.querySelector(
        '#filescontent div[title="' + filekey + '"]',
      );
  ...
  continueAlert(alertobj) {
    ...
    alertobj.remove();
    if (
      self.alerts_in_queue.alertHTML &&
      Object.keys(self.alerts_in_queue.alertHTML).length > 0
    ) {
      self.nextWrongAlert();
  ...
  nextWrongAlert() {
    ...
    alert.querySelector("div:last-child .index-alert").innerHTML =
      self.alerts_in_queue.total - (keys.length - 1);
  ...
  restart() {
    ...
    document
      .querySelectorAll("#alertscontent div.alert")
  ...
  correctFinalRegistry() {
    ...
    document.getElementById("alertscontent").appendChild(alert);
    alert.style.display = "block";
    document.querySelector("#dndfooter").querySelector("#show").style.display =
      "block";
  }
}
