...
class ReconcileVendorCfdiXmlBill(models.TransientModel):
    ...
    typo_de_combante = fields.Selection(
        [
    ...
    def action_reconcile(self):
        ...
        if (
            not selected_att_ids
            or self._context.get("active_model", "") != "ir.attachment"
        ):
            return
        ...
        if self.typo_de_combante == "RV":
            ...
            attachments = (
                ...
                .search(
                    [
                        ("id", "in", selected_att_ids),
                        ("creado_en_odoo", "!=", True),
                        ("cfdi_type", "=", self.typo_de_combante),
                    ]
                )
            )
        else:
            attachments = (
                ...
                .sudo()
                .search(
                    [("id", "in", selected_att_ids), ("creado_en_odoo", "!=", True)]
                )
            )
        ...
        for attachment in attachments:
            ...
            if self.typo_de_combante in ["P", "SP"]:
                ...
                if version == "4.0":
                    total = eval(
                        Complemento.get("pago20:Pagos")
                        .get("pago20:Totales")
                        .get("@MontoTotalPagos", "0.0")
                    )
                else:
                    try:
                        total = eval(
                            Complemento.get("pago10:Pagos")
                            .get("pago10:Pago")
                            .get("@Monto", "0.0")
                        )
                    except Exception:
                        for payment in Complemento.get("pago10:Pagos").get(
                            "pago10:Pago"
                        ):
            ...
            vals = {
                ...
                .sudo()
                .search([("code", "=", uso_data.get("@UsoCFDI"))])
                .id,
            }
            if self.typo_de_combante in ["P", "SP"]:
                payments = payment_obj.search(
                    [
                        ("partner_id.vat", "=", client_rfc),
                        ("amount", "=", total),
                        ("payment_type", "=", payment_type),
                    ]
                )
                ...
                invoices = invoice_obj.search(
                    [("amount_total", "=", total), ("move_type", "=", invoice_type)]
                )
                if invoices:
