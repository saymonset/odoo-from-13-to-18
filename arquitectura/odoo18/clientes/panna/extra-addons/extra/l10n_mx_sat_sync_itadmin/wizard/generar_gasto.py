...
class GenerarGastoWizard(models.TransientModel):
    ...
    def create_expense_entry(self):
        module = (
            self.env["ir.module.module"].sudo().search([("name", "=", "hr_expense")])
        )
        if module and not module.state == "installed":
            raise UserError(
                _(
                    "Debe tener el módulo de Gastos instalado para utilizar esta función."
                )
            )
        ...
        try:
            ...
            concepts = xml_tree.xpath(
                "//cfdi:Conceptos/cfdi:Concepto", namespaces=namespaces
            )
            expense_lines = []
            ...
            for concept in concepts:
                ...
                for traslado in traslados:
                    if traslado.get("Base") == concept.get("Importe", "0.00"):
                        ...
                        tax_exist = self.env["account.tax"].search(
                            [
                                ...
                                (
                                    "amount",
                                    "=",
                                    float(traslado.get("TasaOCuota", "0")) * 100,
                                ),
                        ...
                        if not tax_exist:
                            raise UserError(
                                "La factura contiene impuestos que no han sido configurados."
                        ...
                        tax_exist = self.env["account.tax"].search(
                            [
                                ...
                                (
                                    "amount",
                                    "=",
                                    float(retencion.get("TasaOCuota", "0")) * -100,
                                ),
                                ("company_id", "=", self.env.company.id),
                            ],
                            limit=1,
                ...
                self.env["hr.expense.sheet"].create(
                    {
                        ...
                        "employee_id": self.employee_id.id,
                        "expense_line_ids": [
                            (6, 0, [line.id for line in expense_lines])
                        ],
                    }
                )
                return {
