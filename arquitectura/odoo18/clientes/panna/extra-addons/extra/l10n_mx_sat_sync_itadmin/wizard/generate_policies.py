...
class GeneratePoliciesWizard(models.TransientModel):
    ...
    cuenta_de_ingreso = fields.Many2one(
        "account.account", string="Cuenta de Ingreso", required=True
    )
        ...
        for attachment in attachment_ids:
            if attachment.cfdi_type == "I":
                ...
                try:
                    ...
                    receptor = self.env["res.partner"].search(
                        [("vat", "=", receptor_rfc)], limit=1
                    )
                    if receptor:
                        receivable_account = receptor.property_account_receivable_id
                    else:
                        raise UserError(
                            "No se encontró el socio. Configure primero el socio."
                        )

                    # Create the journal entry
                    journal_entry = self.env[
                        "account.move"
                    ].create(
                        {
                        ...
                        if taxes:
                            ...
                            if taxes:
                                ...
                                for tax in taxes:
                                    ...
                                    if not tax_exist:
                                        raise UserError(
                                            "La factura contiene impuestos que no han sido configurados. Por favor configure primero los impuestos"
                                        )
                                    if tax_exist.cash_basis_transition_account_id:
                                        cash_basis_account = (
                                            tax_exist.cash_basis_transition_account_id
                                        )
                                    ...
                                    #  _logger.info('total_credit %s', total_credit)
                                        ...
                                        raise UserError(
                                            f"La factura contiene ({tax_exist.name}) impuestos que no se han configurado en una cuenta de transición de base de efectivo."
                ...
                # }
                    ...
                    raise UserError(
                        _("Invalid XML format. Please upload a valid XML file.")
                    )
                except Exception as e:
