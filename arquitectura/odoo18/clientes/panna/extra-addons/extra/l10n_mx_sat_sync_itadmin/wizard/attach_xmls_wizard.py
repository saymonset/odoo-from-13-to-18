...
class AttachXmlsWizard(models.TransientModel):
    ...
    def check_xml(self, files, **kwargs):
        ...
        if not isinstance(files, dict):
            raise UserError(
                _(
                    "Something went wrong. The parameter for XML "
        ...
        attas = attach_obj.sudo().search(
            [
                ("cfdi_uuid", "in", list(attachment_uuids.keys())),
                ("company_id", "=", company_id),
            ]
        )
        exist_uuids = dict(
            [(att.cfdi_uuid, att.id) for att in attas]
        )  # attas.mapped('cfdi_uuid')
        ...
        return {"wrongfiles": wrongfiles, "attachments": attachments}
