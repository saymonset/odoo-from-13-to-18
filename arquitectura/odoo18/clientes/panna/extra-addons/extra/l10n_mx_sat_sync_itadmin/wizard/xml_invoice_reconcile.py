...
class XMLInvoiceReconcile(models.TransientModel):
    ...
    def action_reconcile(self):
        ...
        if not invoice and not payment:
            raise UserError(
                "Seleccionar primero la factura/pago y posteriormente reconciliar con el XML."
            )
        ...
        if invoice:
            if (
                ...
                if (invoice.amount_total < (self.amount - float(diff))) or (
                    invoice.amount_total > (self.amount + float(diff))
                ):
                    raise UserError(
                        "El total de la factura no está dentro del rango permitido"
                    )
            invoice.write(
                {
        ...
        if payment:
            if (
                ...
                if abs(payment.amount_company_currency_signed) < self.amount - float(
                    diff
                ) or abs(payment.amount_company_currency_signed) > self.amount + float(
                    diff
                ):
                    raise UserError(
                        "El total de la factura no está dentro del rango permitido"
