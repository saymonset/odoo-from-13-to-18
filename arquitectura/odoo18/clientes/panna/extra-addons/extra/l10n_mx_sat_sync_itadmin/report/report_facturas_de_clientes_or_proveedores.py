...
class Reportfacturas_de_clientes(models.AbstractModel):
    ...
    def get_tax_amount_by_percent(self, lines):
        ...
        for line in lines:
            if (
                not hasattr(line, "Impuestos")
                or not hasattr(line.Impuestos, "Traslados")
                or not hasattr(line.Impuestos.Traslados, "Traslado")
            ):
                continue
            ...
            if (
                not hasattr(line, "Impuestos")
                or not hasattr(line.Impuestos, "Retenciones")
                or not hasattr(line.Impuestos.Retenciones, "Retencion")
            ):
                continue
    ...
    def get_tax_amount(self, line):
        ...
        if (
            hasattr(line, "Impuestos")
            and hasattr(line.Impuestos, "Traslados")
            and hasattr(line.Impuestos.Traslados, "Traslado")
        ):
            for tax in line.Impuestos.Traslados.Traslado:
                tx = float(tax.get("TasaOCuota", "0.0")) * 100
                tx_per_str = str(tx) + "%"
        ...
        if (
            hasattr(line, "Impuestos")
            and hasattr(line.Impuestos, "Retenciones")
            and hasattr(line.Impuestos.Retenciones, "Retencion")
        ):
            for tax in line.Impuestos.Retenciones.Retencion:
                tx = float(tax.get("TasaOCuota", "0.0")) * 100
                tx_per_str = "-" + str(tx) + "%"
    ...
    def l10n_mx_edi_amount_to_text(self, currency, amount_total):
        for doc in self:
            currency_rec = doc.env["res.currency"].search(
            ...
            words = (
                currency_rec.with_context(lang=doc.env.user.lang or "es_ES")
