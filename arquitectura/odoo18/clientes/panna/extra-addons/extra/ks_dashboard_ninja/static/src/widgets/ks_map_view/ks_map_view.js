...
export class KsMapPreview extends Component {
  ...
  _Ks_render() {
    ...
    var graphTextElements =
      self.mapContainerRef.el.querySelectorAll("div.graph_text");
      ...
      if (rec.ks_data_calculation_type !== "query") {
        if (rec.ks_model_id) {
          if (
            ...
            self.mapContainerRef.el?.appendChild(
  ...
  async get_map_data(rec) {
    if (this.mapContainerRef.el.querySelectorAll(".graph_text").length) {
      this.mapContainerRef.el
        ?.querySelectorAll(".graph_text")
        .forEach(function (el) {
          el.remove();
        });
    }
    ...
    if (chart_data.groupByIds?.length) {
      ...
      this.mapContainerRef.el.innerHTML += renderToString(
        "ksNoItemChartView",
        {},
      );
    }
...
registry
  .category("fields")
  .add("ks_dashboard_map_preview", ks_map_preview_field);
