...
export class KsKeywordSelection extends Component {
  ...
  setup() {
    ...
    onWillStart(async () => {
      this.ks_data_model = await rpc(
        "/web/dataset/call_kw/ks_dashboard_ninja.arti_int/ks_get_keywords",
        {
    ...
    if (value.length) {
      ...
      if (this.ks_data_model) {
        this.ks_data_model.forEach((item) => {
          if (
            item.value.toUpperCase().indexOf(ks_value) > -1 &&
            item.value !== value
          ) {
  ...
  _onResponseSelect(ev) {
    ...
    document
      .querySelectorAll("#ks_keywords_container .createAI-card")
...
registry
  .category("fields")
  .add("ks_keyword_selection", KsKeywordSelectionfield);
