...
export class KschatwithAI extends Component {
  setup() {
    ...
    this.name_title =
      this.user.split(" ").length > 1
        ? this.user.split(" ")[0].charAt(0) + this.user.split(" ")[1].charAt(0)
        : this.user.split(" ")[0].charAt(0);
    this.state = useState({
      messages: [
        {
          sender: "ai",
          text: "Hello I am AI Assistant, How may i help you ?",
          frame: false,
        },
      ],
    });
    ...
    onPatched(() => {
      document.querySelector(".chat-sec").scrollTop =
        document.querySelector(".chat-sec").scrollHeight;
    });
  ...
  ks_send_request(ev) {
    ...
    if (user_question.length > 1 && this.send_request) {
      ...
      this.state.messages = [
        ...this.state.messages,
        user_obj,
        { sender: "ai", text: "loading", frame: false },
      ];
      ...
      rpc("/web/dataset/call_kw/ks_dashboard_ninja.arti_int/ks_gen_chat_res", {
        ...
        if (result["Answer"]) {
          ...
          let frame =
            JSON.parse(result["Dataframe"]).length > 5
              ? JSON.parse(result["Dataframe"])
              : false;
          ...
          self.state.messages.push({
            sender: "ai",
            text: "AI unable to Generate Response",
          });
          this.send_request = true;
        }
      });
      ...
      let res_obj = {
        sender: "ai",
        text: "Either you have not asked any question or AI Unable to generate response",
        frame: false,
      };
