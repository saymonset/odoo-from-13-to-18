...
patch(KsDashboardNinja.prototype, {
  ...
  ks_init_domain_data_index() {
    ...
    to_insert.forEach((x) => {
      ...
      if (x["categ"] in temp_data) {
        temp_data[x["categ"]]["domain"] = temp_data[x["categ"]][
          "domain"
        ].concat(x["domain"]);
  ...
  onKsDnDynamicFilterSelect(ev) {
    ...
    if (this.isFavFilter) {
      self.ks_dashboard_data.ks_dashboard_domain_data = {};
      self.header.el
        ?.querySelector(".ks_fav_filters_checked")
        ?.classList.remove("ks_fav_filters_checked", "global-active");
    }
    ...
    if (
      Object.keys(self.ks_dashboard_data.ks_dashboard_domain_data).length !== 0
    ) {
  ...
  _ksAppendDynamicFilter(filterId) {
    ...
    this.ks_dashboard_data.ks_dashboard_pre_domain_filter[filterId].active =
      true;

    var action = "add_dynamic_filter";

    var categ =
      this.ks_dashboard_data.ks_dashboard_pre_domain_filter[filterId].categ;
    var params = {
      model:
        this.ks_dashboard_data.ks_dashboard_pre_domain_filter[filterId].model,
      model_name:
  ...
  _ksUpdateAddDomainIndexData(action, categ, params) {
    ...
    var self = this;
    self.header.el
      ?.querySelector(".custom-filter-tab")
    ...
    const filterAppliedContainer = document.querySelector(
    ...
    if (domain_data) {
      var domain_index = domain_data?.ks_domain_index_data?.find((x) => {
  ...
  _ksUpdateRemoveDomainIndexData(action, categ, params) {
    ...
    if (filters_to_update.length < 1) {
      if (domain_data.ks_domain_index_data.length > 1) {
        ...
        if (
          !Object.keys(self.ks_dashboard_data.ks_dashboard_domain_data).length
        ) {
          const filterAppliedContainer = document.querySelector(
            ".ks_dn_filter_applied_container",
          );
    ...
    if (
      !Object.values(
        ...
        return x.active === true && x.model === model;
      }).length
    )
  ...
  ksOnRemoveFilterFromSearchPanel(ev) {
    ...
    if (search_section.getAttribute("kscateg") != "0") {
      ...
      var action = "remove_dynamic_filter";
      var selected_pre_define_filter = document.querySelectorAll(
      ...
      selected_pre_define_filter.forEach(function (element) {
        ...
        self.ks_dashboard_data.ks_dashboard_pre_domain_filter[filterId].active =
          false;
      });
      ...
      var domain_data = self.ks_dashboard_data.ks_dashboard_domain_data[model];
      domain_data.ks_domain_index_data.forEach(
        (ks_domain_index_data, index) => {
      ...
      if (domain_data.ks_domain_index_data.length === 0) {
        ...
        if (
          ...
          const filterAppliedContainers = document.querySelectorAll(
            ".ks_dn_filter_applied_container",
          );
  ...
  ksGetParamsForItemFetch(item_id) {
    ...
    if (model1 in self.ks_dashboard_data.ks_model_item_relation) {
      if (
        self.ks_dashboard_data.ks_model_item_relation[model1].indexOf(item_id) <
        0
      )
    ...
    if (model2 in self.ks_dashboard_data.ks_model_item_relation) {
      if (
        self.ks_dashboard_data.ks_model_item_relation[model2].indexOf(item_id) <
        0
      )
    ...
    var ks_domain_1 =
      (self.ks_dashboard_data.ks_dashboard_domain_data[model1] &&
        self.ks_dashboard_data.ks_dashboard_domain_data[model1]["domain"]) ||
      [];
    var ks_domain_2 =
      (self.ks_dashboard_data.ks_dashboard_domain_data[model2] &&
        self.ks_dashboard_data.ks_dashboard_domain_data[model2]["domain"]) ||
      [];
  ...
  ks_render_custom_filter(show_remove_option) {
    let table_row_id =
      document.querySelectorAll(
        "#ks_dn_custom_filters_container .ks_dn_custom_filter_input_container_section",
      ).length + 1;
    ...
    if (container)
      Object.values(container.children).forEach((childContainer) =>
        newRow.append(childContainer),
      );
    ...
    var operator_input = renderToElement("ks_dn_custom_domain_input_operator", {
      ...
      ksOnCustomFilterOperatorSelect:
        this.ksOnCustomFilterOperatorSelect.bind(this),
    });
    ...
    const filtersContainer = document.getElementById(
      "ks_dn_custom_filters_container",
    );
  ...
  async ks_render_filter_options(ev) {
    ...
    var filter_id = ev.currentTarget
      .closest(".o_generator_menu_value_td")
      .parentElement.querySelector(".custom_filter_current_value_section")
      .getAttribute("data-index");
    var ks_current_operator_value = ev.currentTarget
      .closest(".o_generator_menu_value_td")
      .parentElement.querySelector(".operator_current_value_section")
    ...
    if (
      !ev.target.parentElement.classList.contains("awesomplete") &&
      ks_current_operator_value != "in"
    ) {
  ...
  _ksRenderCustomFilterInputSection(
    relation,
    operator_type,
    field_type,
    special_data,
    ...
    switch (field_type) {
      ...
      case "selection":
        if (!operator_type) return false;
          ...
          value_input = renderToFragment(
            "ks_dn_custom_domain_input_selection",
            {
              selection_input: special_data["select_options"] || [],
              show_remove_option: show_remove_option,
              ksOnCustomFilterConditionRemove:
                this.ksOnCustomFilterConditionRemove.bind(this),
              onCustomFilterSelectionFieldSelect:
                this.onCustomFilterSelectionFieldSelect.bind(this),
              self: self,
              this: this,
              relation: relation,
  ...
  onCustomFilterSelectionFieldSelect(ev) {
    ...
    document.querySelector(
      "#ks_dn_custom_filters_container " +
        targetRowId +
        " .o_generator_menu_value_td .o_generator_menu_value",
    ).textContent = changedValue;
    document.querySelector(
      "#ks_dn_custom_filters_container " +
        targetRowId +
        " .o_generator_menu_value_td .o_generator_menu_value",
    ).dataset.value = valueAttribute;
  },
  ...
  _ksRenderDateTimeFilterInput(operator, field_type, show_remove_option) {
    ...
    if (field_type == "date") {
      if (datetimePicker) {
        datetimePicker.value = formatDate(DateTime.now(), {
          format: "yyyy-MM-dd",
        });
      }
    } else {
      if (datetimePicker) {
        datetimePicker.value = new Date(
          DateTime.now() + new Date().getTimezoneOffset() * -60 * 1000,
        )
          .toISOString()
          .slice(0, 19);
      }
  ...
  ksOnCustomFilterApply(ev) {
    ...
    this.isFavFilter = false;
    document
      .querySelectorAll(".ks_dn_custom_filter_input_container_section")
      .forEach((filter_container) => {
        var field_id = filter_container
          .querySelector(".custom_filter_current_value_section")
          .getAttribute("data-index");
        var field_select =
          this.ks_dashboard_data.ks_dashboard_custom_domain_filter[field_id];
        var field_type = field_select.type;
        var domainValue = [];
        var domainArray = [];
        var operatorIndex = filter_container
          .querySelector(".operator_current_value_section")
          .getAttribute("data-index");
        ...
        if (["date", "datetime"].includes(field_type)) {
          var dateValue = [];
          filter_container
            .querySelectorAll(".o_generator_menu_value_td .o_datepicker")
            .forEach((input_val) => {
              var a = input_val.value;
              if (field_type === "datetime") {
                var b = formatDateTime(DateTime.fromISO(a), {
                  format: "yyyy-MM-dd HH:mm:ss",
                });
                var c = formatDateTime(DateTime.fromISO(a), {
                  format: "dd/MM/yyyy HH:mm:ss",
                });
              } else {
                var b = formatDate(DateTime.fromFormat(a, "yyyy-MM-dd"), {
                  format: "yyyy-MM-dd",
                });
                var c = formatDate(DateTime.fromFormat(a, "yyyy-MM-dd"), {
                  format: "dd/MM/yyyy",
                });
              }
          ...
          let domainValueText = filter_container.querySelectorA(
            ".o_generator_menu_value_td .o_generator_menu_value",
          ).textContent;
          domainValue = [domainValueText];
          label = label + " " + domainValueText;
        } else if (field_type === "boolean") {
          let domainValueBool = filter_container.querySelector(
            ".ks_operator_option_selector_td .operator_current_value_section",
          ).textContent;
          ...
          if (operator === "in") {
            var ks_input_value = filter_container
              .querySelector(".o_generator_menu_value_td textarea")
              .value.split(",");
            ks_input_value.pop();
            ks_input_value = ks_input_value.map((x) => x.trim());
            var ks_filter_options = this.ks_custom_filter_option[field_id];
            const ks_domain_array = ks_filter_options
              ?.filter((item) => ks_input_value.includes(item[1]))
              .map((value) => value[0]);
            ...
            label =
              label +
              " " +
              filter_container.querySelector(
                ".o_generator_menu_value_td textarea",
              ).value;
          } else {
            let inputvalues;
            if (operator === "between") {
              inputvalues = filter_container.querySelectorAll(
                ".o_generator_menu_value_td input",
              );
            ...
            if (operator === "between") {
              domainValue = [
                domain.ast?.value[1]?.value[2]?.value,
                domain.ast?.value[2]?.value[2]?.value,
              ];
        ...
        if (field_select.model in model_domain) {
          model_domain[field_select.model]["domain"] =
            model_domain[field_select.model]["domain"].concat(domainArray);
          model_domain[field_select.model]["domain"].unshift("|");
          model_domain[field_select.model]["label"] =
            model_domain[field_select.model]["label"] + " or " + label;
        } else {
  ...
  ksAddCustomDomain(model_domain) {
    ...
    Object.entries(model_domain).map(([model, val]) => {
      ...
      if (
        domain_data["domain"][0] !== undefined &&
        domain_data["domain"].length != 0
      ) {
        var storedData = this.getObjectFromCookie(
          "FilterOrderData" + self.ks_dashboard_id,
        );
  ...
  ksOnCustomFilterFieldSelect(ev) {
    var self = this;
    let targetRowId = "#" + ev.target.dataset?.trId;
    let displayed_filter_container = document.querySelector(
      targetRowId + " .custom_filter_current_value_section",
    );
    if (displayed_filter_container) {
      displayed_filter_container.textContent = ev.currentTarget.text;
      displayed_filter_container.setAttribute(
        "data-index",
        ev.currentTarget.dataset?.value,
      );
    }
    var parent_container = document.getElementById("ks_custom_filter_table");
    var operatorOptionSelector = parent_container.querySelector(
      targetRowId + " .ks_operator_option_selector_td",
    );
    if (operatorOptionSelector) {
      operatorOptionSelector.remove();
    }
    var generatorMenuValue = parent_container.querySelector(
      targetRowId + " .o_generator_menu_value_td",
    );
    if (generatorMenuValue) {
      generatorMenuValue.remove();
    }
    var customFilterDeleteBtn = parent_container.querySelector(
      targetRowId + " .customFilterDeleteBtn",
    );
    ...
    var field_select =
      self.ks_dashboard_data.ks_dashboard_custom_domain_filter[field_id];
    ...
    let isShowDeleteBtn =
      document.getElementById("ks_dn_custom_filters_container").children
        .length <= 1 || ev.target.dataset?.trId === "div-1"
        ? false
        : true;
    var value_input = self._ksRenderCustomFilterInputSection(
      relation,
      operator_type?.symbol,
      ks_field_type[field_type],
      field_select.special_data,
      isShowDeleteBtn,
      ev.target.dataset?.trId,
    );
    if (value_input) {
      let targetRow = document.querySelector(targetRowId);
      Object.values(value_input.children).forEach((value) =>
        targetRow.appendChild(value),
      );
    }
  },
  ...
  ksOnCustomFilterOperatorSelect(ev) {
    var parent_container = document.getElementById("ks_custom_filter_table");
    let targetRowId = "#" + ev.target.dataset?.trId;
    let displayed_operator_container = document.querySelector(
      targetRowId + " .operator_current_value_section",
    );
    if (displayed_operator_container) {
      displayed_operator_container.textContent = ev.currentTarget.text;
      displayed_operator_container.setAttribute(
        "data-index",
    ...
    var customFilterSection = parent_container.querySelector(
      targetRowId + " .custom_filter_current_value_section",
    );
    var field_id = customFilterSection
      ? customFilterSection.getAttribute("data-index")
      : null;
    var field_select =
      this.ks_dashboard_data.ks_dashboard_custom_domain_filter[field_id];
    var relation = field_select?.relation;
    ...
    const elementToRemove = parent_container.querySelector(
      targetRowId + " .o_generator_menu_value_td",
    );
    if (elementToRemove) {
      elementToRemove.remove();
    }
    const removeDelete = parent_container.querySelector(
      targetRowId + " .customFilterDeleteBtn",
    );
    if (removeDelete) {
      removeDelete.remove();
    ...
    let isShowDeleteBtn =
      document.getElementById("ks_dn_custom_filters_container")?.children
        ...
        ? false
        : true;
    var value_td = this._ksRenderCustomFilterInputSection(
      relation,
      operator_type,
      ks_field_type[field_type],
      field_select?.special_data,
    ...
    if (value_td) {
      let targetRow = document.querySelector(targetRowId);
      Object.values(value_td.children).forEach((value) =>
        targetRow.appendChild(value),
      );
    }
  },
  ...
  searchPredefinedFilter(ev) {
    ...
    if (ev.currentTarget.value !== "") {
      searchedPredefinedFilters = Object.values(
        this.state.ks_dn_pre_defined_filters,
      ).filter(
        (filter) =>
          filter.name.toLowerCase().includes(searchName.toLowerCase()) ||
          filter.type === "separator",
      );
      while (
        searchedPredefinedFilters.length &&
        searchedPredefinedFilters[searchedPredefinedFilters.length - 1].type ===
          "separator"
      )
        searchedPredefinedFilters.pop();
      while (
        searchedPredefinedFilters.length &&
        searchedPredefinedFilters[0].type === "separator"
      )
        searchedPredefinedFilters.shift();
    } else {
      searchedPredefinedFilters = this.state.ks_dn_pre_defined_filters;
    }

    let filterSection = ev.currentTarget
      .closest(".ks_dn_pre_filter_menu")
      .querySelector(".predefined_filters_section");
    this.attachSearchFilter(filterSection, searchedPredefinedFilters);
  },

  predefinedSearchFocusout(ev) {
    let input = ev.currentTarget.querySelector(
      ".dropdown-menu.show .predefinedFilterSearchInput",
    );
    if (input) input.value = "";
    let filterSection = ev.currentTarget.querySelector(
      ".dropdown-menu.show .predefined_filters_section",
    );
    this.attachSearchFilter(
      filterSection,
      Object.values(this.state.ks_dn_pre_defined_filters),
    );
  },
  ...
  ks_save_favourite(ev, dialogCloseCallback) {
    ...
    var ks_is_fav_filter_shared =
      document.getElementById("favFilterShareBool").checked;
    if (!ks_filter_name.length) {
      ...
      var ks_saved_fav_filters = Object.keys(
        self.ks_dashboard_data.ks_dashboard_favourite_filter,
      );
      const favourite = ks_saved_fav_filters.find(
        (item) => item == ks_filter_name,
      );
      if (favourite?.length) {
        ...
        var ks_filter_to_save = JSON.stringify(
          self.ks_dashboard_data.ks_dashboard_domain_data,
        );
        rpc(
          "/web/dataset/call_kw/ks_dashboard_ninja.favourite_filters/create",
          {
            ...
            args: [
              {
                ...
                ks_access_id: ks_is_fav_filter_shared
                  ? false
                  : user.context.uid,
              },
            ],
          ...
          var ks_filter_obj = {
            id: result,
            filter: JSON.parse(
              JSON.stringify(self.ks_dashboard_data.ks_dashboard_domain_data),
            ),
            name: ks_filter_name,
            ks_access_id: ks_is_fav_filter_shared ? false : user.context.uid,
          };
          self.ks_dashboard_data.ks_dashboard_favourite_filter[ks_filter_name] =
            ks_filter_obj;
          //                    self.state.stateToggle = !self.state.stateToggle
          var filterContainer = renderToElement(
            "dn_favourite_filter_dropdown",
            {
              ks_favourite_filters:
                self.ks_dashboard_data.ks_dashboard_favourite_filter,
              onksfavfilterselected: self.onksfavfilterselected.bind(self),
              ks_delete_favourite_filter:
                self.ks_delete_favourite_filter.bind(self),
              self: self,
              ks_dashboard_data: self.ks_dashboard_data,
            },
          );
          var favFilterDropdowns = document.querySelectorAll(
            "#favFilterMain .favFilterDropdown",
          );
          favFilterDropdowns.forEach(function (dropdown) {
            dropdown.remove();
          });
          document
            .getElementById("favFilterMain")
            ?.replaceWith(filterContainer);
  ...
  ks_delete_favourite_filter(ev) {
    ...
    var ks_filter_domain =
      self.ks_dashboard_data.ks_dashboard_favourite_filter[
        ks_filter_name_to_del
      ].filter;
    var ks_access_id =
      self.ks_dashboard_data.ks_dashboard_favourite_filter[
        ks_filter_name_to_del
    ...
    const ks_items_to_update_remove =
      self.ks_dashboard_data.ks_dashboard_items_ids.filter(
        (item) =>
          ks_remove_filter_models.includes(
            self.ks_dashboard_data.ks_item_model_relation[item][0],
          ) ||
          ks_remove_filter_models.includes(
            self.ks_dashboard_data.ks_item_model_relation[item][1],
          ),
      );
    if (ks_access_id) {
      self.dialogService.add(ConfirmationDialog, {
        ...
        confirm: () => {
          self.ks_delete_fav_filter(
            ks_filter_name_to_del,
            ks_filter_id_to_del,
            ks_items_to_update_remove,
          );
        },
      });
    } else {
  ...
  ks_delete_fav_filter(
    ...
    rpc("/web/dataset/call_kw/ks_dashboard_ninja.favourite_filters/unlink", {
      ...
      delete self.ks_dashboard_data.ks_dashboard_favourite_filter[
        ks_filter_name_to_del
      ];
      ...
      Object.keys(self.ks_dashboard_data.ks_dashboard_domain_data).forEach(
        (model) => {
          self.state.pre_defined_filter =
  ...
  ksFavFilterFacetRemove(ev) {
    ...
    if (FilterModels.length) {
      ...
      FilterModels.forEach((model) => {
        let item_ids =
          this.ks_dashboard_data.ks_dashboard_favourite_filter[
            this.activeFavFilterName
          ].filter[model]?.item_ids;
        if (item_ids)
  ...
  onksfavfilterselected(ev) {
    ...
    var ks_filters_to_remove = document.querySelectorAll(
      ".ks_dn_filter_applied_container .ks_dn_filter_section_container .o_searchview_facet",
    );
    ...
    ks_filters_to_remove.forEach(function (filter, item) {
      ...
      if (!self.ks_pre_define_filters_model.includes(ks_filter_model)) {
        ...
        var ks_filters = Object.keys(
          self.ks_dashboard_data.ks_dashboard_favourite_filter[favName].filter,
        );
        self.ks_pre_define_filters_model.push(ks_filter_model);
        if (!ks_filters.includes(ks_filter_model)) {
          self._ksUpdateRemoveDomain(
            self.ks_dashboard_data.ks_dashboard_domain_data[ks_filter_model],
          );
        }
      }
    });
    ...
    //            facet.remove();
    //        });
    var dynamicFilters = document.querySelectorAll(
    ...
    var elementsToRemove = document.querySelectorAll(
      ".ks_dn_filter_applied_container .ks_dn_filter_section_container",
    );
    ...
    var ks_filter_to_uncheck = currentTarget?.querySelector(
      ".ks_fav_filters_checked",
    );
    let item_for_update = [];
    if (ks_filter_to_uncheck) {
      ...
      var ks_remove_filter_domain =
        self.ks_dashboard_data.ks_dashboard_favourite_filter[
          ks_remove_filter_name
        ].filter;
      ...
      if (
        ks_items_to_update_remove.length &&
        ks_filter_to_uncheck.getAttribute("fav-name") ===
          ev.currentTarget?.getAttribute("fav-name")
      ) {
        let domain_data = {
          item_ids: [],
        };
      ...
      var checkedElements = parentElement.querySelectorAll(
        ".ks_fav_filters_checked",
      );
    ...
    if (
      ...
      var ks_applied_filter_domain =
        self.ks_dashboard_data.ks_dashboard_favourite_filter[
          ks_applied_filter_name
        ].filter;
      var ks_applied_filter_models = Object.keys(ks_applied_filter_domain);
      let ks_items_to_update =
        self.ks_dashboard_data.ks_dashboard_items_ids.filter(
          (item) =>
            ks_applied_filter_models.includes(
              self.ks_dashboard_data.ks_item_model_relation[item][0],
            ) ||
            ks_applied_filter_models.includes(
              self.ks_dashboard_data.ks_item_model_relation[item][1],
      ...
      if (item_for_update.length !== 0) {
        ks_items_to_update = [
          ...new Set([...ks_items_to_update, ...item_for_update]),
        ];
      }
      ...
      if (ks_items_to_update.length) {
        ...
        if (item_for_update.length !== 0) {
          domain_data.item_ids = [
            ...new Set([...domain_data["item_ids"], ...item_for_update]),
          ];
        }
        self.state.pre_defined_filter = { ...domain_data };
      }
  ...
  filterTableDropdownShow(e) {
    ...
    if (targetElement) {
      let dropdownMenu =
        targetElement.querySelector(".dropdown-menu") ||
        targetElement.querySelector("ul");
      var dropdownToggle =
        targetElement.querySelector(".dropdown-toggle") ||
        targetElement.querySelector(".ks_input_filter_options");

      if (dropdownMenu && dropdownToggle) {
        if (dropdownMenu.id?.includes("awesomplete_list"))
          dropdownMenu.classList.add("awesomplete_list_ul");
        ...
        dropdownMenu.style.top =
          targetRect.top + window.scrollY + targetElement.offsetHeight + "px";
