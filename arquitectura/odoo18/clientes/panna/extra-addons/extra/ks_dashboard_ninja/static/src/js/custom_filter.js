...
export class CustomFilter extends Component {
  ...
  setup() {
    ...
    useBus(this.env.bus, "Clear:Custom-Filter", (ev) =>
      this.clearRows(ev.detail),
    );
    useBus(this.env.bus, "Clear:Custom-Filter-Facets", (ev) =>
      this.clearFacets(ev.detail),
    );
  }
  ...
  async loadFieldDefs_and_Labels(filters_list) {
    ...
    for (const filter of filters_list) {
      filter_labels.push({
        filter_name: filter.name + " ( " + filter.model_name + " ) ",
        filter_id: filter.id,
      });
      if (typeof filter.field_name === "string") {
        promises.push(
          this.loadFieldInfo(filter.model, filter.field_name).then(
            ({ fieldDef }) => {
  ...
  getValueInfo(filter_row) {
    const fieldDef =
      this.fieldDefs?.[filter_row.id] || this.filter_info[filter_row.id];
    let valueInfo = getValueEditorInfo(fieldDef, filter_row.operator);
    if (
      valueInfo.component?.name === "Select" ||
      valueInfo.component?.name === "List"
    ) {
  ...
  onUpdateOperator(operator, row_index) {
    ...
    this.state.filtersRowList[row_index].value = getDefaultValue(
      fieldDef,
      operator,
      this.state.filtersRowList[row_index].value,
    );
  }
  ...
  async applyFilters(ev) {
    ...
    for (const filter_row of filterRowList) {
      ...
      models_domain[model].domain.children.push(
    ...
    await Promise.all(
      Object.entries(models_domain).map(async ([model, model_domain]) => {
        model_domain.label = await this.getDomainTreeDescription(
          model,
          model_domain.domain,
        );
        model_domain.domain = new Domain(
          domainFromTree(model_domain.domain),
        ).toList();
      }),
    );

    for (const [model, model_domain] of Object.entries(models_domain)) {
      this.customDomainFacets[model] = this.customDomainFacets[model] || {
        labels: [],
        model_name: model_domain.model_name,
      };
