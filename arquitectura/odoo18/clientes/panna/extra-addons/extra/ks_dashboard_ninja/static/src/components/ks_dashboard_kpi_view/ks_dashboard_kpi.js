...
export class Ksdashboardkpiview extends Component {
  ...
  setup() {
    ...
    this.classname =
      " encapsulated-kpi-tile ks_dashboard_kpi ks_dashboard_kpi_dashboard ks_dashboard_custom_srollbar ks_dashboarditem_id ks_dashboard_item_hover ks_db_item_preview_color_picker grid-stack-item-content";
  ...
  _update_view() {
    if (!this.kpi_data[1]) {
      if (
        this.field.ks_target_view === "Progress Bar" &&
        this.field.ks_goal_enable
      ) {
        if (this.ks_kpi.el?.querySelector("#ks_progressbar")) {
          this.ks_kpi.el.querySelector("#ks_progressbar").value = parseInt(
            this.target_deviation,
          );
        }
      }

      if (this.field.ks_goal_enable) {
        const targetDeviationElement =
          this.ks_kpi.el?.querySelector(".target_deviation");
        if (targetDeviationElement)
          targetDeviationElement.style.color =
            this.state.item_info_kpi1.target_arrow === "up" ? "green" : "red";
      }
      ...
      if (
        this.field.ks_previous_period &&
        String(this.state.item_info_kpi1.previous_period_data) &&
        ks_valid_date_selection.indexOf(this.ks_date_filter_selection) >= 0
      ) {
        const preDeviationElement =
          this.ks_kpi.el?.querySelector(".pre_deviation");
        if (preDeviationElement)
          preDeviationElement.style.color =
            this.state.item_info_kpi1.pre_arrow === "up" ? "green" : "red";
      }

      const ksTargetPreviousElement = this.ks_kpi.el?.querySelector(
        ".ks_target_previous",
      );
      ...
      if (this.field.ks_data_comparison === "Sum") {
        if (this.ks_kpi.el?.querySelector(".target_deviation")) {
          ...
          if (targetDeviationElement)
            targetDeviationElement.style.color =
              this.state.item_info_kpi2.ks_color;
        }
        if (this.field.ks_target_view === "Progress Bar") {
          const progressBarElement =
            this.ks_kpi.el?.querySelector("#ks_progressbar");
          const targetDeviation =
            this.ks_target_deviation === Infinity ||
            this.ks_target_deviation === -Infinity
              ? 0
              : this.ks_target_deviation;
  ...
  ksSum(count_1, count_2, item_info, field, target_1, kpi_data) {
    ...
    if (field.ks_multiplier_active) {
      item_info["count"] = globalfunction._onKsGlobalFormatter(
        count * field.ks_multiplier,
        field.ks_data_format,
        field.ks_precision_digits,
      );
      item_info["count_tooltip"] = formatFloat(count * field.ks_multiplier, {
      ...
      item_info["count_tooltip"] = formatFloat(
        parseFloat(count),
        Float64Array,
        { digits: [0, field.ks_precision_digits] },
      );
    ...
    if (ks_selection === "monetary") {
      ...
      var ks_data = globalfunction.ks_monetary(
        item_info["count"],
        ks_currency_id,
      );
    ...
    if (this.target_deviation !== Infinity)
      item_info.target_deviation = formatInteger(this.target_deviation) + "%";
    else {
    ...
    item_info.target_progress_deviation =
      formatInteger(target_progress_deviation) + "%";
    return item_info;
  }
  ...
  prepareKpiData() {
    ...
    this.ks_rgba_background_color = self._ks_get_rgba_format(
    ...
    if (field.ks_goal_enable) {
      ...
      if (deviation !== Infinity)
        deviation = deviation ? formatInteger(deviation) + "%" : 0 + "%";
    }
    if (
    ...
    this.item["ksIsDashboardManager"] =
    ...
    this.target_deviation = parseInt(item_info.target_progress_deviation)
    ...
    item_info.target_progress_deviation = parseInt(
      ...
      ? formatInteger(parseInt(item_info.target_progress_deviation))
      : "0";
    if (field.ks_multiplier_active) {
      item_info["count_1"] = globalfunction._onKsGlobalFormatter(
    ...
    if (kpi_data[0].target) {
      item_info["target"] = globalfunction._onKsGlobalFormatter(
        kpi_data[0].target,
        field.ks_data_format,
        field.ks_precision_digits,
      );
    ...
    if (field.ks_unit) {
      ...
      if (ks_selection === "monetary") {
        ...
        var ks_data = globalfunction._onKsGlobalFormatter(
          ks_record_count,
          field.ks_data_format,
          field.ks_precision_digits,
        );
        ...
        item_info["count_1"] =
          ks_field +
          " " +
          globalfunction._onKsGlobalFormatter(
            ks_record_count,
            field.ks_data_format,
            field.ks_precision_digits,
          );
      ...
      switch (this.field.ks_data_comparison) {
        case "None":
          if (field.ks_multiplier_active) {
            ...
            var count =
              String(
                globalfunction.ksNumFormatter(count_1 * field.ks_multiplier, 1),
              ) +
              "/" +
              String(
                globalfunction.ksNumFormatter(count_2 * field.ks_multiplier, 1),
              );
            var data1 = globalfunction._onKsGlobalFormatter(
              count_1 * field.ks_multiplier,
              field.ks_data_format,
              field.ks_precision_digits,
            );
            var data2 = globalfunction._onKsGlobalFormatter(
            ...
            var count =
              String(globalfunction.ksNumFormatter(count_1, 1)) +
              "/" +
              String(globalfunction.ksNumFormatter(count_2, 1));
            var data1 = globalfunction._onKsGlobalFormatter(
              count_1,
              field.ks_data_format,
              field.ks_precision_digits,
          ...
          break;
        case "Sum":
          this.state.item_info_kpi2 = this.ksSum(
            count_1,
            count_2,
            item_info,
            field,
          ...
          break;
        case "Percentage":
          this.state.item_info_kpi2 = this.ksPercentage(
            count_1,
            count_2,
            field,
            item_info,
        ...
        case "Ratio":
          ...
          if (this.item.ks_data_format == "exact") {
            if (count_1 && count_2) {
              ...
              item_info["count"] =
                formatFloat(count_1 / gcd, Float64Array, {
                  digits: [0, field.ks_precision_digits],
                }) +
                ":" +
                formatFloat(count_2 / gcd, Float64Array, {
                  digits: [0, field.ks_precision_digits],
                });
            } else {
            ...
            if (count_1 && count_2) {
              item_info["count_tooltip"] = count_1 / gcd + ":" + count_2 / gcd;
              item_info["count"] =
                globalfunction.ksNumFormatter(count_1 / gcd, 1) +
                ":" +
                globalfunction.ksNumFormatter(count_2 / gcd, 1);
            } else {
              item_info["count_tooltip"] = count_1 + ":" + count_2;
              item_info["count"] =
                globalfunction.ksNumFormatter(count_1, 1) +
                ":" +
                globalfunction.ksNumFormatter(count_2, 1);
            }
          }
          item_info["target_enable"] = false;
          this.state.item_info_kpi2 = item_info;
