...
export class Ksdashboardtile extends Component {
  ...
  setup() {
    ...
    if (this.ks_ai_analysis) {
      ...
      this.ks_inner_container_class =
        "grid-stack-item-content ks_ai_dashboard_item";
    } else {
      this.ks_container_class = "grid-stack-item";
      this.ks_inner_container_class =
        "encapsulated-kpi-tile grid-stack-item-content";
    }
    ...
    onMounted(() => {
      if (this.ks_ai_analysis) {
        const dashboardItems = this.ks_tile.el.querySelectorAll(
          ".ks_dashboarditem_id",
        );
  ...
  prepare_item() {
      ...
      if (this.item.ks_unit) {
        ...
        if (ks_selection === "monetary") {
          ...
          var ks_data = globalfunction._onKsGlobalFormatter(
            ks_record_count,
            this.item.ks_data_format,
            this.item.ks_precision_digits,
          );
        ...
        var data_count = globalfunction._onKsGlobalFormatter(
          ks_record_count,
          this.item.ks_data_format,
          this.item.ks_precision_digits,
        );
    ...
    if (this.item.ks_icon_select == "Custom") {
      if (this.item.ks_icon[0]) {
        ks_icon_url =
          "data:image/" +
          (self.file_type_magic_word[this.item.ks_icon[0]] || "png") +
          ";base64," +
          this.item.ks_icon;
      } else {
        ks_icon_url = false;
    ...
    this.ks_rgba_background_color = self._ks_get_rgba_format(
      this.item.ks_background_color,
    );
    this.ks_rgba_font_color = self._ks_get_rgba_format(this.item.ks_font_color);
    this.ks_rgba_default_icon_color = self._ks_get_rgba_format(
      this.item.ks_default_icon_color,
    );
    this.ks_rgba_button_color = self._ks_get_rgba_format(
      this.item.ks_button_color,
    );
    ...
    this.style_main_body = this._ksMainBodyStyle(
      this.ks_rgba_background_color,
      this.ks_rgba_font_color,
      this.item,
    ).background_style;
  }

  get style_image_body_l2() {
    return this._ksMainBodyStyle(
      this.ks_rgba_background_color,
      this.ks_rgba_font_color,
      this.item,
    ).style_image_body_l2;
  ...
  _ksMainBodyStyle(ks_rgba_background_color, ks_rgba_font_color, tile) {
    var background_style =
      ...
      ks_rgba_font_color +
      ";";
    var ks_rgba_dark_background_color_l2 = this._ks_get_rgba_format(
      this.ks_get_dark_color(
        tile.ks_background_color.split(",")[0],
        tile.ks_background_color.split(",")[1],
        -10,
      ),
    );
    var style_image_body_l2 =
