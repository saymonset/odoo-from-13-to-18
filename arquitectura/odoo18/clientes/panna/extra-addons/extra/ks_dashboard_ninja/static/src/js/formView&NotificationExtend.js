...
patch(FormController.prototype, {
  setup() {
    ...
    onMounted(() => {
      ...
      let cpDiscardButton = this.rootRef.el?.querySelector(
        ".o_form_button_cancel",
      );

      if (
        this.rootRef.el &&
        this.props.resModel.startsWith("ks_dashboard_ninja.")
      ) {
        ...
        if (this.props.resModel === "ks_dashboard_ninja.item" && cpCogMenu)
          cpCogMenu.remove();
      }
    });
...
patch(FormLabel.prototype, {
  setup() {
    ...
    onMounted(() => {
      ...
      if (
        tooltip &&
        this.env.model?.config?.resModel.startsWith(
          "ks_dashboard_ninja." ||
            this.env.services.action?.currentController?.action?.tag ===
              "ks_dashboard_ninja",
        )
      )
        tooltip.innerHTML =
    ...
    onMounted(() => {
      let notificationContainer =
        ...
        document.querySelectorAll(
          ".o-main-components-container .o_notification",
        );
      if (
