...
export class CustomDialog extends Component {
  setup() {
    ...
    this.state = useState({
      ...
      messages: [
        { sender: "ai", text: "Hello I am AI Assistant, How may i help you ?" },
      ],
      confirm_notification: false,
      switch_layout: false,
    });
  ...
  switch_bar_chart(new_item) {
    ...
    const chartCards = this.graphRef.el?.querySelectorAll(
      ".ks_chart_card_body",
    );
    chartCards?.forEach((card) => card.remove());
    if (new_item == "ks_funnel_chart") {
      Ksdashboardgraph.prototype.ksrenderfunnelchart.call(
        this,
        this.graphRef.el,
        this.ks_item,
      );
    } else {
      Ksdashboardgraph.prototype.ks_render_graphs.call(
        this,
        this.graphRef.el,
        this.ks_item,
