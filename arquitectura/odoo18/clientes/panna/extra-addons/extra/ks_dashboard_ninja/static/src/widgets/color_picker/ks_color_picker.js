...
export class KsColorPicker extends Component {
  ...
  get value() {
    return {
      ks_color:
        this.props.record.data[this.props.name].split(",")[0] || "#376CAE",
      ks_opacity:
        this.props.record.data[this.props.name].split(",")[1] || "0.99",
    };
  }

  _ksOnColorChange(ev) {
    var new_value = ev.currentTarget.value.concat(
  ...
  _ksOnOpacityChange(ev) {
    var new_value = this.props.record.data[this.props.name]
      .split(",")[0]
  ...
  _ksOnOpacityInputNew(ev) {
    ...
    const percentage =
      ((newOpacity - ev.currentTarget.min) /
        (ev.currentTarget.max - ev.currentTarget.min)) *
      100;
  ...
  _ksOnOpacityInput(ev) {
    ...
    if (this.props.name === "ks_background_color") {
      var ksDbItemPreviewColorPicker = document.querySelector(
        ".ks_db_item_preview_color_picker",
      );
      color = window.getComputedStyle(
        ksDbItemPreviewColorPicker,
      ).backgroundColor;
      ksDbItemPreviewColorPicker.style.backgroundColor =
        self.get_color_opacity_value(color, value);
      ...
      ksDbItemPreviewL2.style.backgroundColor = self.get_color_opacity_value(
        color,
        value,
      );
    } else if (this.props.name === "ks_default_icon_color") {
      var ksDashboardIconColorPickerSpan = document.querySelector(
        ".ks_dashboard_icon_color_picker > span",
      );
      color = window.getComputedStyle(ksDashboardIconColorPickerSpan).color;
      ksDashboardIconColorPickerSpan.style.color = self.get_color_opacity_value(
        color,
        value,
      );
...
registry
  .category("fields")
  .add("Ks_dashboard_color_picker_owl", ksColorPickerField);
