...
export class KSDashboardNinjaOverview extends Component {
  setup() {
    ...
    this.overviewTilesNames = [
      ["All Dashboards", "one"],
      ["All Charts", "two"],
      ["Total Maps", "three"],
      ["Bookmarked Dashboards", "four"],
      ["All Lists", "five"],
    ];
    this.state = useState({
    ...
    onWillStart(async () => {
      await rpc(
        "/web/dataset/call_kw/ks_dashboard_ninja.board/ks_update_menu_id",
        {
      ...
      this.data = await rpc(
        "/web/dataset/call_kw/ks_dashboard_ninja.board/fetch_dashboard_overview",
        {
          model: "ks_dashboard_ninja.board",
  ...
  get filteredDashboards() {
    if (this.state.filter === "Bookmarked") {
      let filteredData = Object.entries(this.data.dashboardsInfo).filter(
        (dashboard) => dashboard[1].is_bookmarked,
      );
  ...
  async updateBookmark(ev) {
    ...
    let unBookmarkSvg = this.ks_overview.el.querySelector(
      `#unBookmark${dashboardId}`,
    );
    let bookmarkSvg = this.ks_overview.el.querySelector(
      `#bookmark${dashboardId}`,
    );
    ...
    this.data.dashboardsInfo[dashboardId].is_bookmarked =
      !this.data.dashboardsInfo[dashboardId].is_bookmarked;
