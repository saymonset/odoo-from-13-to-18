...
export class Ksdashboardgraph extends Component {
  setup() {
    ...
    onWillUpdateProps(async (nextprops) => {
      if (nextprops.ksdatefilter != "none") {
        await this.ksFetchUpdateItem(
          nextprops.item.id,
          nextprops.dashboard_data.ks_dashboard_id,
          nextprops.dashboard_data.context,
        );
      }
      ...
      if (update_interval && !this.env.inDialog) {
        const interval = setInterval(() => {
          this.ksFetchUpdateItem(
            this.item.id,
            this.ks_dashboard_id,
            this.props.dashboard_data.context,
          );
        }, update_interval);
    ...
    if (
      this.props?.item?.ks_dashboard_item_type === "ks_list_view" &&
      this.env.inDialog
    )
      useBus(this.env.bus, `TV:List_Load_More_${this.props?.item?.id}`, (ev) =>
        this.getDomainParams(ev),
      );
  }
            ...
            if (date) {
              if (list_view_data.fields_type[index] === "date") {
                (list_view_data.data_rows[j]["data"][index] =
                  luxon.DateTime.fromJSDate(date).format(date_format)),
                  {},
                  { timezone: false };
              } else {
                (list_view_data.data_rows[j]["data"][index] =
  ...
  async ksFetchUpdateItem(item_id, dash_id, context, domain) {
    ...
    if (!domain) {
      if (
        this.__owl__.parent.component.hasOwnProperty(
          "ksGetParamsForItemFetch",
        ) &&
        this.__owl__.parent.component?.ksGetParamsForItemFetch(
          item_id,
        ) instanceof Function
      ) {
        domain =
          this.__owl__.parent.component?.ksGetParamsForItemFetch(item_id);
      } else {
        domain = {};
      }
    }
    await rpc("/web/dataset/call_kw/ks_dashboard_ninja.board/ks_fetch_item", {
      ...
      args: [[parseInt(item_id)], dash_id, domain],
      kwargs: { context },
    }).then(
      function (new_item_data) {
        if (new_item_data[item_id].ks_list_view_data) {
          new_item_data[item_id].ks_list_view_data = self.renderListViewData(
        ...
        this.__owl__.parent.component.ks_dashboard_data.ks_item_data[
          this.item.id
        ] = new_item_data[item_id];
        if (this.item.ks_dashboard_item_type == "ks_funnel_chart") {
          this.ks_gridstack_container.el
          ...
          if (this.intial_count < this.item.ks_pagination_limit) {
            this.ks_list_view.el
              ?.querySelectorAll(".ks_load_next")
              .forEach((element) => {
  ...
  _update_view() {
    if (this.item.ks_dashboard_item_type == "ks_list_view") {
      ...
      if (
        this.item.ks_data_calculation_type === "query" ||
        this.item.ks_list_view_type === "ungrouped"
      ) {
        document
          .querySelectorAll(".ks_list_canvas_click")
          ?.forEach((element) => {
      ...
      if (this.item.ks_data_calculation_type === "query") {
        this.ks_gridstack_container.el
          ?.querySelectorAll(".ks_dashboard_item_chart_info")
          ?.forEach((element) => {
            element?.classList.add("d-none");
          });
      }
      ...
      this.ks_gridstack_container.el
        ?.querySelectorAll(".ks_dashboard_item_button_container")
        ?.forEach((element) => {
  ...
  update_list_view() {
    if (this.item.ks_dashboard_item_type == "ks_list_view") {
      ...
      if (this.item.ks_pagination_limit == 0) {
        this.ks_list_view.el
          ?.querySelectorAll(".ks_pager_name")
          ?.forEach((element) => {
            element?.classList.add("d-none");
          });
      }
  ...
  prepare_list() {
    ...
    this.calculation_type =
      this.ks_dashboard_data.ks_item_data[
        this.item_id
          ...
          for (var j = 0; j < list_view_data.data_rows.length; j++) {
            ...
            if (date) {
              if (list_view_data.fields_type[index] === "date") {
                ...
                let parsedDate = parseDateTime(date, {
    ...
    if (list_view_data) {
      for (var i = 0; i < list_view_data.data_rows.length; i++) {
        for (var j = 0; j < list_view_data.data_rows[0]["data"].length; j++) {
          if (
            typeof list_view_data.data_rows[i].data[j] === "number" ||
            list_view_data.data_rows[i].data[j]
          ) {
            if (typeof list_view_data.data_rows[i].data[j] === "number") {
              list_view_data.data_rows[i].data[j] = formatFloat(
                list_view_data.data_rows[i].data[j],
                Float64Array,
                { digits: [0, self.item.ks_precision_digits] },
              );
            }
          } else {
            list_view_data.data_rows[i].data[j] = "";
    ...
    this.ks_pager = true;
    this.tmpl_list_type =
      self.ks_dashboard_data.ks_item_data[this.item_id].ks_list_view_type;
  ...
  ks_render_graphs(ks_gridstack_container, item, zooming_enabled = false) {
    var self = this;
    var chartCardBodies = ks_gridstack_container?.querySelectorAll(
      ".ks_chart_card_body",
    );
    if (chartCardBodies?.length > 0) {
      ...
      var dashboardItemContainer = ks_gridstack_container?.querySelector(
        ".ks_dashboarditem_chart_container",
      );
      if (dashboardItemContainer) {
        dashboardItemContainer.appendChild(newCardBody);
      }
      var graph_render = ks_gridstack_container?.querySelectorAll(
        ".ks_chart_card_body",
      );
    }
      ...
      if (ks_data.length && ks_labels.length) {
        ...
        switch (chart_type) {
          ...
          case "ks_bullet_chart":
            if (
              this.props?.dashboard_data?.zooming_enabled ||
              zooming_enabled
            ) {
        ...
        if (
          chart_type == "ks_radar_view" ||
          chart_type == "ks_radialBar_chart" ||
          chart_type == "ks_flower_view" ||
          chart_type == "ks_polarArea_chart"
        ) {
          root.rtl = true;
          var legend = chart.children.push(
            am5.Legend.new(root, {
        ...
        ks_gridstack_container
          .querySelector(".ks_li_" + item.ks_chart_item_color)
          ?.classList.add("ks_date_filter_selected");
      } else {
        var ksChartCardBody = ks_gridstack_container?.querySelector(
          ".ks_chart_card_body",
        );
        if (ksChartCardBody) {
  ...
  ksrenderfunnelchart(ks_gridstack_container, item) {
    var self = this;
    if (
      ks_gridstack_container?.querySelectorAll(".ks_chart_card_body")?.length
    ) {
      var funnelRender = ks_gridstack_container?.querySelectorAll(
        ".ks_chart_card_body",
      );
    } else {
      var dashboardItemContainer = ks_gridstack_container?.querySelector(
    ...
    if (funnel_data["labels"] && funnel_data["datasets"]) {
      ...
      if (keyValueArray.length) {
        ...
        const ksChartCardBody = ks_gridstack_container.querySelector(
          ".ks_chart_card_body",
        );
        ksChartCardBody.innerHTML += renderToString("ksNoItemChartView", {});
      }
  ...
  onChartCanvasClick(evt) {
    ...
    if (self.ks_dashboard_data.ks_item_data[item_id].max_sequnce) {
      ...
      if (lastSeq !== sequence) {
        rpc(
          "/web/dataset/call_kw/ks_dashboard_ninja.item/ks_fetch_drill_down_data",
          {
          ...
          if (result.ks_list_view_data) {
            ...
            self.ks_dashboard_data.ks_item_data[item_id].sequnce =
              result.sequence;
            self.ks_dashboard_data.ks_item_data[item_id].ks_list_view_data =
            ...
            var gridItem = document.querySelector(
              ".ks_dashboard_main_content .grid-stack-item[gs-id='" +
                item_id +
                "']",
            );
            gridItem
              ?.querySelector(".ks_chart_heading")
              ?.classList.add("d-none");
            gridItem
              ?.querySelector(".ks_list_view_heading")
              ?.classList.add("d-none");
            ...
            gridItem
              ?.querySelector(".ks_dashboard_item_action_export")
              ?.classList.add("d-none");
            gridItem
              ?.querySelector(".ks_dashboard_quick_edit_action_popup")
              ?.classList.remove("d-sm-block");

            var item_data = self.ks_dashboard_data.ks_item_data[item_id];
            var list_view_data = JSON.parse(item_data.ks_list_view_data);

            var $container = renderToElement(
              "ks_dashboard_ninja.ks_new_list_view_table",
              {
                list_view_data,
            ...
            self.ks_dashboard_data.ks_item_data[item_id].ks_chart_data =
            ...
            if (self.ks_dashboard_data.ks_item_data[item_id].domains) {
              self.ks_dashboard_data.ks_item_data[item_id].domains[
                result.sequence
              ] = JSON.parse(result.ks_chart_data).previous_domain;
            } else {
              self.ks_dashboard_data.ks_item_data[item_id].domains = {};
              self.ks_dashboard_data.ks_item_data[item_id].domains[
                result.sequence
            ...
            gridItem?.querySelector(id_name)?.classList.remove("d-none");
            gridItem
              ?.querySelector(".ks_dashboard_item_chart_info")
              ?.classList.remove("d-none");
            gridItem
              ?.querySelector(".ks_dashboard_color_option")
              ?.classList.remove("d-none");
            gridItem?.querySelector(".ks_search_plus")?.classList.add("d-none");
            gridItem
              ?.querySelector(".ks_search_minus")
      ...
      if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce) {
        rpc(
          ...
          if (result.ks_chart_data) {
            ...
            var gridItem = dashboardContent?.querySelector(
              `.grid-stack-item[gs-id="${item_id}"]`,
            );
            ...
            if (gridItem) {
              ...
              var drillUp = children?.querySelector(
              ...
              var chartInfo = children?.querySelector(
                ".ks_dashboard_item_chart_info",
              );
              if (chartInfo) chartInfo?.classList.remove("d-none");

              var colorOption = children?.querySelector(
                ".ks_dashboard_color_option",
              );
              if (colorOption) colorOption?.classList.remove("d-none");

              var quickEditPopup = children?.querySelector(
                ".ks_dashboard_quick_edit_action_popup",
              );
              if (quickEditPopup)
                quickEditPopup?.classList.remove("d-sm-block");

              var moreAction = children?.querySelector(
                ".ks_dashboard_more_action",
              );
            ...
            if (gridItem) {
              ...
              var drillUp = firstChild?.querySelector(
                ".ks_dashboard_item_drill_up",
              );
              if (drillUp) {
              ...
              var chartInfo = firstChild?.querySelector(
                ".ks_dashboard_item_chart_info",
              );
              if (chartInfo) {
              ...
              var colorOption = firstChild?.querySelector(
                ".ks_dashboard_color_option",
              );
              if (colorOption) {
              ...
              var quickEditPopup = firstChild?.querySelector(
                ".ks_dashboard_quick_edit_action_popup",
              );
              if (quickEditPopup) {
                quickEditPopup.classList.remove("d-sm-block");
              ...
              var moreAction = firstChild?.querySelector(
                ".ks_dashboard_more_action",
              );
              if (moreAction) {
      ...
      if ("domains" in item_data) {
        ...
        if (sequnce >= 0) {
          rpc(
            ...
            var dashboardMainContent = document.querySelector(
              ".ks_dashboard_main_content",
            );
            var gridStackItem = dashboardMainContent.querySelector(
              `.grid-stack-item[gs-id="${item_id}"]`,
            );
            ...
            const drillUpElement = firstChild.querySelector(
              ".ks_dashboard_item_drill_up",
            );
            if (drillUpElement) {
            ...
            if (result.ks_chart_data) {
              ...
              const chartInfoElement = firstChild.querySelector(
                ".ks_dashboard_item_chart_info",
              );
              if (chartInfoElement) {
                chartInfoElement.classList.remove("d-none");
              }
              const colorOptionElement = firstChild.querySelector(
                ".ks_dashboard_color_option",
              );
          ...
          const listViewHeading = gridStackItem.querySelector(
          ...
          const quickEditActionPopup = firstChild.querySelector(
            ".ks_dashboard_quick_edit_action_popup",
          );
          if (quickEditActionPopup) {
            quickEditActionPopup.classList.add("d-sm-block");
          }
          const moreActionElement = firstChild.querySelector(
            ".ks_dashboard_more_action",
          );
          ...
          const exportActionElement = firstChild.querySelector(
            ".ks_dashboard_item_action_export",
          );
          ...
          const searchMinusElement =
            firstChild.querySelector(".ks_search_minus");
        ...
        if (!domain) {
          let dashboardMainContent = document.querySelector(
            ".ks_dashboard_main_content",
          );
          let gridStackItem = dashboardMainContent?.querySelector(
            `.grid-stack-item[gs-id="${item_id}"]`,
          );
          let firstChild = gridStackItem?.children[0];
          let drillUpElement = firstChild?.querySelector(
            ".ks_dashboard_item_drill_up",
          );
          drillUpElement?.classList.add("d-none");
        }
  ...
  ksFetchChartItem(id) {
    ...
    return rpc("/web/dataset/call_kw/ks_dashboard_ninja.board/ks_fetch_item", {
      model: "ks_dashboard_ninja.board",
      method: "ks_fetch_item",
      args: [[item_data.id], self.ks_dashboard_data.ks_dashboard_id, {}],
      kwargs: {},
    }).then(
      function (new_item_data) {
        self.ks_dashboard_data.ks_item_data[id] = new_item_data[id];

        var gridStackItem = document?.querySelector(
          `.ks_dashboard_main_content .grid-stack-item[gs-id="${id}"]`,
        );
        if (gridStackItem) {
  ...
  async ksrendermapview(ks_map_view_tmpl, item) {
    ...
    if (ks_map_view_tmpl?.querySelectorAll(".ks_chart_card_body").length > 0) {
      ...
      var chartContainer = ks_map_view_tmpl?.querySelector(
        ".ks_dashboarditem_chart_container",
      );
      if (chartContainer) {
    ...
    if (map_data.groupByIds?.length) {
      ...
      if (ks_data.length && ks_labels.length) {
        ...
        const ksChartCardBody = ks_map_view_tmpl?.querySelector(
          ".ks_chart_card_body",
        );
        ksChartCardBody.innerHTML += renderToString("ksNoItemChartView", {});
      }
  ...
  ksRenderChartColorOptions(e) {
    ...
    if (!currentTargetParent.classList.contains("ks_date_filter_selected")) {
      ...
      var selectedFilter = currentTargetGrandParent.querySelector(
      ...
      rpc("/web/dataset/call_kw/ks_dashboard_ninja.item/write", {
        ...
        self.ks_dashboard_data.ks_item_data[itemId]["ks_chart_item_color"] =
          e.target.dataset.chartColor;
        var gridStackItem = document.querySelector(
  ...
  ksLoadMoreRecords(e) {
    ...
    if (
      ...
      this.env.bus.trigger("GET:ParamsForItemFetch", {
        item_id: parseInt(itemId),
        isCarouselParentClass: true,
      });
      params = this.domainParams;
    } else
      params = self.__owl__.parent.component.ksGetParamsForItemFetch(
        parseInt(itemId),
      );
    rpc(
      "/web/dataset/call_kw/ks_dashboard_ninja.board/ks_get_list_view_data_offset",
      {
        model: "ks_dashboard_ninja.board",
        ...
        args: [
          ...
          parseInt(self.ks_dashboard_data.ks_dashboard_id),
          params,
        ],
      ...
      var item_data = self.ks_dashboard_data.ks_item_data[itemId];
      var item_view = document.querySelector(
        ".ks_dashboard_main_content .grid-stack-item[gs-id='" +
          item_data.id +
          "']",
      );
      ...
      var loadPreviousButton =
        e.target.parentElement.querySelector(".ks_load_previous");
      loadPreviousButton.classList.remove("ks_event_offer_list");

      if (
        result.next_offset < parseInt(result.offset) + (offset - 1) ||
  ...
  ksLoadPreviousRecords(e) {
    ...
    var offset =
      self.ks_dashboard_data.ks_item_data[itemId].ks_pagination_limit;
    var ks_offset =
      parseInt(e.target.parentElement.dataset.prevOffset) - (offset + 1);
    ...
    rpc(
      ...
      var loadNextButton =
        e.target.parentElement.querySelector(".ks_load_next");
  ...
  ksOnListItemInfoClick(e) {
    ...
    if (e.currentTarget.dataset.listViewType === "ungrouped") {
      ...
      if (e.currentTarget.dataset.listType === "date_type") {
        ...
        action["domain"] = domain;
        action["context"]["search_default_" + e.currentTarget.dataset.groupby] =
