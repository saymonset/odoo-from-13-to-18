...
export class KsAIDashboardNinja extends Component {
  setup() {
    ...
    this.generate_dialog = this.props.action.context.generate_dialog
      ? true
      : false;
  ...
  grid_initiate() {
    ...
    Object.values(
      ks_element.querySelectorAll(".ks_dashboarditem_chart_container"),
    ).map((item) => {
      item.addEventListener("click", this.onkschartcontainerclick.bind(this));
    });
    Object.values(
      ks_element.querySelectorAll(".ks_dashboard_kpi_dashboard"),
    ).map((item) => {
      item.addEventListener("click", this.onkschartcontainerclick.bind(this));
    });
    document
      .querySelectorAll(".modal-body .ks_dashboard_item_button_container")
      ?.forEach((element) => {
        element.remove();
      });
    ...
    document
      .querySelectorAll(".modal-header .btn-close")
      ?.forEach((element) => {
        element.remove();
      });
    document
      .querySelectorAll(".modal-footer .o-default-button")
      ?.forEach((element) => {
        element.remove();
      });
  ...
  ksRenderDashboard() {
    ...
    if (Object.keys(self.ks_dashboard_data.ks_item_data).length === 0) {
      ...
      self.header?.el
        .querySelectorAll(".ks_dashboard_edit_layout")
        .forEach((layout) => {
  ...
  onKsDuplicateItemClick(e) {
    ...
    var dashboard_id = parentElement.querySelector(
      ".ks_dashboard_select",
    ).value;
    var dashboard_name = parentElement.querySelector(
      ".ks_dashboard_select option:checked",
    ).textContent;
  ...
  onkschartcontainerclick(ev) {
    if (ev.currentTarget.classList.contains("ks_dashboard_kpi_dashboard")) {
      if (
        ...
        this.ksSelectedgraphid.push(
          parseInt(ev.currentTarget.parentElement.id),
        );
      } else {
        ev.currentTarget
          .querySelector(".select-btn")
          .classList.remove("active");
        const index = this.ksSelectedgraphid.indexOf(
          parseInt(ev.currentTarget?.parentElement.id),
        );
  ...
  onselectallitems() {
    ...
    document
      .querySelectorAll(".modal-body .ks_dashboarditem_chart_container")
      .forEach((item) => {
        item.querySelector(".select-btn").classList.add("active");
        this.ksSelectedgraphid.push(parseInt(item.parentElement.id));
      });
