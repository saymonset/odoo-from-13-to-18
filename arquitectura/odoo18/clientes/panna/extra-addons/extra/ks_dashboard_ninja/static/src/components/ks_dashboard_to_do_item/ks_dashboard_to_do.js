...
import {
  Todoeditdialog,
  addtododialog,
} from "@ks_dashboard_ninja/components/ks_dashboard_to_do_item/editdialog";
...
export class Ksdashboardtodo extends Component {
  ...
  ksFetchUpdateItem(item_id) {
    ...
    return rpc("/web/dataset/call_kw/ks_dashboard_ninja.board/ks_fetch_item", {
      ...
      args: [[parseInt(item_id)], self.ks_dashboard_data.ks_dashboard_id, {}],
    ...
    rpc("/web/dataset/call_kw/ks_to.do.description/write", {
      ...
      self.ksFetchUpdateItem(ks_item_id).then(function () {
        ...
        const addButton = document.querySelector(
          `.header_add_btn[data-item-id="${ks_item_id}"]`,
        );
        if (addButton) {
    ...
    this.dialogService.add(ConfirmationDialog, {
      ...
      confirm: () => {
        rpc("/web/dataset/call_kw/ks_to.do.description/unlink", {
          ...
          self.ksFetchUpdateItem(ks_item_id).then(function () {
            const tabsToDeactivate = document.querySelectorAll(
              `.ks_li_tab[data-item-id="${ks_item_id}"]`,
            );
            if (tabsToDeactivate) {
            ...
            const sectionsToDeactivate = document.querySelectorAll(
              `.ks_tab_section[data-item-id="${ks_item_id}"]`,
            );
  ...
  _onCreateTask(content, ks_section_id, ks_item_id) {
    ...
    rpc("/web/dataset/call_kw/ks_to.do.description/create", {
      ...
      self.ksFetchUpdateItem(ks_item_id).then(function () {
        document
          .querySelectorAll(`.ks_li_tab[data-item-id="${ks_item_id}"]`)
          .forEach((tab) => {
        ...
        document
          .querySelectorAll(`.ks_tab_section[data-item-id="${ks_item_id}"]`)
          .forEach((section) => {
            section.classList.remove("active");
            section.classList.remove("show");
          });
  ...
  ksOnToDoClick(ev) {
    ...
    const parentSiblings =
      ev.currentTarget?.parentNode?.parentNode?.parentNode?.children;
