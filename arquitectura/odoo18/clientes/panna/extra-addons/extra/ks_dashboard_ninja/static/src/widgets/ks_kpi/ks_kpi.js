...
class KsKpiPreview extends Component {
  ...
  ksSum(count_1, count_2, item_info, field, target_1, kpi_data) {
    ...
    if (field.ks_multiplier_active) {
      ...
      item_info["count_tooltip"] = formatFloat(count, {
        digits: [0, field.ks_precision_digits],
      });
    }
    ...
    if (target_deviation !== Infinity)
      item_info.target_deviation = formatInteger(target_deviation) + "%";
    else {
  ...
  get value() {
    ...
    if (
      ...
      ks_valid_date_selection.indexOf(field.ks_date_filter_selection) >= 0
    ) {
        ...
        case "Ratio":
          ...
          if (field.ks_data_format == "exact") {
            if (count_1 && count_2) {
              ...
              item_info["count"] =
                formatFloat(count_1 / gcd, {
                  digits: [0, field.ks_precision_digits],
                }) +
                ":" +
                formatFloat(count_2 / gcd, {
                  digits: [0, field.ks_precision_digits],
                });
            } else {
  ...
  ks_changes() {
    ...
    if (field.ks_kpi_data) {
      ...
      if (
        ...
        var target_progress_deviation = String(
          Math.round((count / target_1) * 100),
        );
      } else if (
        field.ks_data_comparison === "Percentage" &&
        field.ks_model_id_2 &&
        field.ks_target_view == "Progress Bar"
      ) {
        var count = parseInt((count_1 / count_2) * 100);
      ...
      if (field.ks_goal_enable) {
        ...
        if (acheive) {
          this.ks_kpi_ref.el
            ?.querySelectorAll(".target_deviation")
            .forEach((element) => {
      ...
      if (
        this.ks_kpi_ref.el &&
        this.ks_kpi_ref.el?.querySelectorAll(".row").children?.length !== 2
      ) {
        var filter_children =
          this.ks_kpi_ref.el?.querySelectorAll(".row").children;
      ...
      if (
        ...
        if (pre_acheive) {
          this.ks_kpi_ref.el
            ?.querySelectorAll(".pre_deviation")
            .forEach((element) => {
              element.style.color = "green";
            });
        } else {
      ...
      if (
        ...
        field.ks_standard_goal_value &&
        field.ks_model_id_2
      ) {
        this.ks_kpi_ref.el.querySelector("#ks_progressbar").value = parseInt(
...
registry
  .category("fields")
  .add("ks_dashboard_kpi_owlpreview", KsKpiPreviewfield);
