...
export class Ksiconselectordialog extends Component {
  ...
  ks_icon_container_list(e) {
    var self = this;
    this.ks_modal.el
      .querySelectorAll(".ks_icon_container_list")
      .forEach((selected_icon) => {
  ...
  ks_fa_icon_search(e) {
    ...
    if (this.ks_search.el.querySelectorAll(".ks_fa_search_icon").length > 0) {
      this.ks_search.el
        .querySelectorAll(".ks_fa_search_icon")
        .forEach(function (el) {
          el.remove();
        });
    }
    var ks_fa_icon_name = this.ks_search.el.querySelectorAll(
      ".ks_modal_icon_input",
    )[0].value;
    ...
    ks_fa_icon_render.classList.add(
    ...
    spanElement.classList.add(
      "fa",
      "fa-" + ks_fa_icon_name.toLowerCase(),
      "fa-4x",
    );
