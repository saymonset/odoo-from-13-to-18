...
export class KsDashboardNinja extends Component {
  setup() {
    ...
    this.isReloadOnFirstCreate = this.props.action?.params
      ?.isReloadOnFirstCreate
      ? true
      : false;
    ...
    this.explain_ai_whole =
      this.props.action.params.explain_ai_whole === undefined ? true : false;
    ...
    onMounted(() => {
      ...
      var filter_date_data = this.getObjectFromCookie(
        "FilterDateData" + this.ks_dashboard_id,
      );

      if (filter_date_data != null) {
        this.header.el
          .querySelector(".ks_date_filter_selected")
          ?.classList.remove("ks_date_filter_selected");
        document.getElementById("ks_date_filter_selection").textContent =
          this.ks_date_filter_selections[filter_date_data];
        ...
        if (filter_date_data === "l_custom") {
          var custom_range = this.getObjectFromCookie(
            "custom_range" + this.ks_dashboard_id,
          );
          if (custom_range) {
            try {
              this.state.ksDateFilterStartDate = parseDateTime(
                custom_range["start_date"],
                self.datetime_format,
              );
              this.state.ksDateFilterEndDate = parseDateTime(
                custom_range["end_date"],
                self.datetime_format,
              );
            } catch (error) {
          ...
          this.header.el
            ?.querySelector(".ks_date_input_fields")
            ?.classList.remove("ks_hide");
          document
            .querySelector(".ks_date_filter_dropdown")
            ?.classList.add("ks_btn_first_child_radius");
        }
      }

      if (
        this.getObjectFromCookie("FilterDateData" + this.ks_dashboard_id) ||
        this.getObjectFromCookie("FilterOrderData" + this.ks_dashboard_id)
      ) {
        var pre_defined_filters = document.querySelectorAll(
          ".dn_filter_click_event_selector",
        );
        var filters_applied = this.ks_dashboard_data.ks_dashboard_domain_data;
        var pre_defined_filters_data =
            ...
            for (const key in pre_defined_filters_data) {
              if (key === element.dataset.filterId) {
                for (const key1 in filters_applied) {
                  for (const domain_index in filters_applied[key1]
                    .ks_domain_index_data) {
                    if (
                      filters_applied[key1].ks_domain_index_data[domain_index]
                        .categ
                    ) {
                      if (
                        filters_applied[key1].ks_domain_index_data[domain_index]
                          .categ === pre_defined_filters_data[key].categ
                      ) {
                        if (
                          filters_applied[key1].ks_domain_index_data[
                            domain_index
                          ].label.includes(pre_defined_filters_data[key].name)
                        ) {
                          if (
      ...
      document
        .querySelector(".table-custom")
        ?.addEventListener("show.bs.dropdown", this.filterTableDropdownShow);
      document
        .querySelector(".table-custom")
        ?.addEventListener("hide.bs.dropdown", this.filterTableDropdownHide);
      document.querySelector(".modal-footer button")?.classList.add("d-none");
      let filterFacetCountTag =
        this.header.el?.querySelector(".filters-amount");
      if (filterFacetCountTag)
        filterFacetCountTag.textContent =
          this.header.el?.querySelectorAll(".selcted-opt").length;
    });
    ...
    onRendered(() => {
      ...
      if (
        this.env.services.menu.getCurrentApp()?.xmlid !==
      ...
      if (this.isReloadOnFirstCreate) {
        ...
        if (
          ...
          this.props.action.params.isReloadOnFirstCreate
        )
          this.props.action.params.isReloadOnFirstCreate = false;
        this.env.services.menu.reload();
        this.notification.add(
          _t("Your Personal Dashboard has been successfully created"),
          {
    ...
    useBus(this.env.bus, "GET:ParamsForItemFetch", (ev) =>
      this.ksGetParamsForItemFetch(ev.detail),
    );
  }
  ...
  willStart() {
    var self = this;
    var def;
    var storedData = this.getObjectFromCookie(
      "FilterOrderData" + self.ks_dashboard_id,
    );
    var storedPredefinedData = this.getObjectFromCookie(
      "PredefinedData" + self.ks_dashboard_id,
    );
    if (this.reload_menu_option.reload && this.reload_menu_option.menu_id) {
      def = this.getParent().actionService.ksDnReloadMenu(
        this.reload_menu_option.menu_id,
      );
    }
    return Promise.all([
      def,
      loadBundle("ks_dashboard_ninja.ks_dashboard_lib"),
    ]).then(function () {
        ...
        return self.ks_fetch_items_data().then(function () {
          if (storedData != null) {
            ...
            Object.keys(
              ...
              if (
                self.ks_dashboard_data.ks_dashboard_domain_data[model]
                  .ks_domain_index_data.length !=
                self.ks_dashboard_data.ks_dashboard_domain_data[
                  model
                ].ks_domain_index_data?.filter?.((x) => {
                  return x.isCustomFilter;
                }).length
              ) {
            ...
            if (storedPredefinedData) {
              var filters_to_update = Object.values(
  ...
  dashboardImageUpdate() {
    ...
    if (
      !document.querySelector(".ks_dashboard_main_content")?.childNodes.length
    ) {
  ...
  grid_initiate() {
    ...
    if (gridstackContainer) {
      ...
      if (this.ks_dashboard_data.ks_gridstack_config) {
        this.gridstackConfig = JSON.parse(
          this.ks_dashboard_data.ks_gridstack_config,
        );
      }
      for (var i = 0; i < item.length; i++) {
        ...
        if (ks_preview && !this.ks_dashboard_data.ks_ai_explain_dash) {
          if (item[i].id in self.gridstackConfig) {
            var min_width = graphs.includes(item[i].ks_dashboard_item_type)
              ? 3
              : 2;
            self.grid.addWidget(ks_preview, {
              x: self.gridstackConfig[item[i].id].x,
              y: self.gridstackConfig[item[i].id].y,
              w: self.gridstackConfig[item[i].id].w,
              h: self.gridstackConfig[item[i].id].h,
              autoPosition: false,
              minW: min_width,
              maxW: null,
              minH: 3,
          ...
          Object.values(
            ks_element.querySelectorAll(".ks_dashboard_item_button_container"),
          ).map((item) => {
            item?.classList.add("d-none");
            item?.classList.remove("d-md-flex");
          });
          Object.values(ks_element.querySelectorAll(".ks_pager_name")).map(
            (item) => {
              item?.classList.add("d-none");
            },
          );
    ...
    Object.values(ks_element.querySelectorAll(".ks_duplicate_item")).map(
      (item) => {
        item.addEventListener("click", this.onKsDuplicateItemClick.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_move_item")).map((item) => {
      item.addEventListener("click", this.onKsMoveItemClick.bind(this));
    });
    Object.values(ks_element.querySelectorAll(".ks_dashboard_item_delete")).map(
      (item) => {
        item.addEventListener("click", this._onKsDeleteItemClick.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_chart_xls_csv_export")).map(
      (item) => {
        item.addEventListener("click", this.ksChartExportXlsCsv.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_chart_pdf_export")).map(
      (item) => {
        item.addEventListener("click", this.ksChartExportPdf.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_chart_image_export")).map(
      (item) => {
        item.addEventListener("click", this.ksChartExportimage.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_chart_json_export")).map(
      (item) => {
        item.addEventListener("click", this.ksItemExportJson.bind(this));
      },
    );
    Object.values(
      ks_element.querySelectorAll(".ks_dashboard_quick_edit_action_popup"),
    ).map((item) => {
      item.addEventListener("click", this.onEditItemTypeClick.bind(this));
    });
    Object.values(
      ks_element.querySelectorAll(".ks_dashboard_item_customize"),
    ).map((item) => {
      item.addEventListener("click", this.onEditItemTypeClick.bind(this));
    });
    Object.values(
      ks_element.querySelectorAll(".ks_dashboard_menu_container"),
    ).map((item) => {
      item.addEventListener("click", this.stoppropagation.bind(this));
    });
    Object.values(ks_element.querySelectorAll(".ks_item_description")).map(
      (item) => {
        item.addEventListener("click", this.stoppropagation.bind(this));
      },
    );
    Object.values(ks_element.querySelectorAll(".ks_item_click")).map((item) => {
      item.addEventListener("click", this._onKsItemClick.bind(this));
    });
    Object.values(ks_element.querySelectorAll(".ks_dashboard_item_action")).map(
      (item) => {
        item.addEventListener(
          "click",
          this.ks_dashboard_item_action.bind(this),
        );
      },
    );
    Object.values(
      ks_element.querySelectorAll(".ks_dashboard_item_chart_info"),
    ).map((item) => {
      item.addEventListener("click", this.onChartMoreInfoClick.bind(this));
    });
    ...
    document
      .querySelector(".predefined-filters")
      ?.addEventListener(
        "hide.bs.dropdown",
        this.predefinedSearchFocusout.bind(this),
      );
  ...
  getContext() {
    ...
    if (
      self.dn_state["user_context"]["ksDateFilterSelection"] !== undefined &&
    ...
    var ks_new_obj = {
      ...
      ...{ allowed_company_ids: this.env.services.company.activeCompanyIds },
    };
            ...
            if (date) {
              if (list_view_data.fields_type[index] === "date") {
                list_view_data.data_rows[j]["data"][index] =
                  luxon.DateTime.fromJSDate(new Date(date + " UTC")).toFormat?.(
                    this.date_format,
                  );
              } else {
                list_view_data.data_rows[j]["data"][index] =
  ...
  ks_fetch_data() {
    var self = this;
    return rpc(
      "/web/dataset/call_kw/ks_dashboard_ninja.board/ks_fetch_dashboard_data",
      {
        model: "ks_dashboard_ninja.board",
      ...
      self.ks_dashboard_data.ks_ai_explain_dash = self.props.action.params
        .explainWithAi
        ? true
        : false;
      self.ks_dashboard_data["ks_dashboard_id"] =
        self.props.action.params.ks_dashboard_id;
      self.ks_dashboard_data["context"] = self.getContext();
      self.ks_dashboard_data["ks_ai_dashboard"] = false;
      self.ks_favourite_filters = JSON.parse(
        JSON.stringify(self.ks_dashboard_data.ks_dashboard_favourite_filter),
      );
      if (self.dn_state["domain_data"] != undefined) {
        Object.values(
          ...
          if (self.dn_state["domain_data"][x["model"]] != undefined) {
            if (
              self.dn_state["domain_data"][x["model"]][
                "ks_domain_index_data"
              ][0]["label"].indexOf(x["name"]) == -1
            ) {
              self.ks_dashboard_data.ks_dashboard_pre_domain_filter[
                x["id"]
              ].active = false;
  ...
  ks_fetch_items_data() {
    ...
    self.ks_dashboard_data.ks_dashboard_items_ids.forEach(function (item_id) {
      items_promises.push(
        rpc("/web/dataset/call_kw/ks_dashboard_ninja.board/ks_fetch_item", {
          model: "ks_dashboard_ninja.board",
          ...
          if (result[item_id].ks_list_view_data) {
            result[item_id].ks_list_view_data = self.renderListViewData(
              result[item_id],
            );
          }
          ...
          self.state.ks_show_create_layout_option =
            Object.keys(self.ks_dashboard_data.ks_item_data).length > 0 &&
    ...
    (self.state.ks_dashboard_name = self.ks_dashboard_data.name),
      ...
      (self.state.ks_dashboard_manager =
        self.ks_dashboard_data.ks_dashboard_manager),
      ...
      (self.state.ks_show_layout =
        self.ks_dashboard_data.ks_dashboard_manager &&
        self.ks_dashboard_data.ks_child_boards &&
        true),
      (self.state.ks_selected_board_id =
      ...
      (self.state.ks_dn_pre_defined_filters = Object.values(
        self.ks_dashboard_data.ks_dashboard_pre_domain_filter,
      ).sort(function (a, b) {
        return a.sequence - b.sequence;
      })),
      (self.state.ks_dashboard_item_length =
        self.ks_dashboard_data.ks_dashboard_items_ids.length);
    self.state.update = false;
    (self.state.ksDateFilterSelection =
      self.state.ksDateFilterSelection == false
        ? "none"
        : self.state.ksDateFilterSelection),
  ...
  ksRenderDashboard() {
    ...
    if (Object.keys(self.ks_dashboard_data.ks_item_data).length === 0) {
      self.header.el
        .querySelector(".ks_dashboard_link")
        ?.classList.add("d-none");
      self.header.el
        .querySelector(".ks_dashboard_edit_layout")
        ?.classList.add("ks_hide");
    }
    if (!self.state.ks_dashboard_manager) {
      self.header.el
        .querySelector("#ks_dashboard_title")
        ?.classList.add("ks_user");
    }
  ...
  ksRenderDashboardMainContent() {
    ...
    if (
      isMobileOS() &&
      document.querySelector("#ks_dn_layout_button")?.firstElementChild
    ) {
      document
        .querySelector(".ks_am_element")
        ?.append(
          document.querySelector("#ks_dn_layout_button")?.firstElementChild
            .innerText,
        );
      self.header.el
        .querySelector("#ks_dn_layout_button")
        ?.classList.add("ks_hide");
    }
  ...
  loadDashboardData(date = false) {
    var self = this;
    self.header.el
      .querySelector(".apply-dashboard-date-filter")
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector(".clear-dashboard-date-filter")
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector(".ks_dashboard_top_settings")
      ?.classList.add("d-none");
  ...
  _KsGetDateValues() {
    ...
    self.header.el
      .querySelector("#" + date_filter_selected)
      ?.classList.add("ks_date_filter_selected", "global-filter");
    self.header.el.querySelector("#ks_date_filter_selection").textContent =
      self.ks_date_filter_selections[date_filter_selected];
  ...
  _ksOnDateFilterMenuSelect(e) {
    if (e.target.id !== "ks_date_selector_container") {
      ...
      self.header.el
        .querySelector(".custom-date-range-selector")
        ?.classList.add("dn_hide");
      ...
      if (e.target.parentElement.id !== "l_custom") {
        ...
        document
          .querySelector(".ks_date_filter_dropdown")
          ?.classList.remove("ks_btn_first_child_radius");
        ...
        self.header.el
          .querySelector(".custom_date_filter_section")
          ?.classList.remove("ks_hide");
      ...
      if (self.state.ksDateFilterSelection === "l_none") {
        ...
        this.setObjectInCookie(
          "FilterDateData" + self.ks_dashboard_id,
          self.state.ksDateFilterSelection,
          1,
        );
      }
  ...
  _onKsApplyDateFilter(e) {
    ...
    self.header.el
      .querySelector(".ks_dashboard_item_drill_up")
      ?.classList.add("d-none");
    this.header.el
      .querySelectorAll(".custom-date-range-selector")
    ...
    if (start_date === "Invalid date") {
      ...
      self.header.el
        .querySelector(".ks_date_filter_selected")
        ?.getAttribute("id") !== "l_custom"
    ) {
      self.ksDateFilterSelection = self.header.el
        ?.querySelector(".ks_date_filter_selected")
        ?.getAttribute("id");
      var res = {};
      ...
      self.header.el
        ...
        ?.classList.remove("d-none");
      self.header.el
        .querySelectorAll(".custom_date_filter_section")
        .forEach(function (element) {
          element.classList.add("ks_hide");
        });

      self.header.el
        .querySelectorAll(".ks_date_input_fields")
        .forEach(function (element) {
          element.classList.add("ks_hide");
        });
      ...
      if (start_date && end_date) {
        if (
          parseDateTime(start_date, self.datetime_format) <=
          parseDateTime(end_date, self.datetime_format)
        ) {
          this.setObjectInCookie(
            "custom_range" + self.ks_dashboard_id,
            { start_date: start_date, end_date: end_date },
            1,
          );
          self.state.ksDateFilterStartDate = parseDateTime(
            start_date,
            self.datetime_format,
          );
          self.state.ksDateFilterEndDate = parseDateTime(
            end_date,
            self.datetime_format,
          );
          var start_date = formatDateTime(
            parseDateTime(start_date, self.datetime_format),
            { format: "yyyy-MM-dd HH:mm:ss" },
          );
          var end_date = formatDateTime(
            parseDateTime(end_date, self.datetime_format),
            { format: "yyyy-MM-dd HH:mm:ss" },
          );
          if (start_date === "Invalid date" || end_date === "Invalid date") {
            ...
            self.state.ksDateFilterSelection = self.header.el
              ?.querySelector(".ks_date_filter_selected")
              ?.getAttribute("id");
          ...
          this.notification.add(
            _t("Start date should be less than end date."),
            {
              type: "warning",
            },
          );
  ...
  _onKsClearDateValues(ks_l_none = false) {
    ...
    self.state.custom_filter = {};
    self.header.el
      .querySelector(".ks_dashboard_top_settings.new-features")
      ?.classList.remove("d-none");
  ...
  onChartMoreInfoClick(e) {
    ...
    var groupBy =
      item_data.ks_chart_groupby_type === "date_type"
        ...
        if (!item_data.ks_is_client_action) {
          ...
          for (var i = 0; i < action.views.length; i++)
            action.views[i][1].includes("tree")
              ? (action.views[i][1] = action.views[i][1].replace(
                  "tree",
                  "list",
                ))
              : action.views[i][1];
  ...
  onKsDuplicateItemClick(e) {
    var self = this;
    var ks_item_id = e.target
      .closest(".ks_dashboarditem_id")
      .getAttribute("id");
    var ks_selected = e.target
      .closest(".ks_dashboarditem_id")
      .querySelector(".ks_dashboard_select");
    ...
    if (self.ksAllowItemClick) {
      ...
      if (e.target.title != "Customize Item") {
        ...
        if (
          ...
          item_data.ks_data_calculation_type != "query"
        ) {
          if (item_data.action) {
            if (!item_data.ks_is_client_action) {
              var action = Object.assign({}, item_data.action);
  ...
  removeitems() {
    var self = this;
    var ks_items = Object.values(
      self.main_body.el.querySelectorAll(".grid-stack-item"),
    );
  ...
  _ksSetCurrentLayoutClick() {
    var self = this;
    this.ks_dashboard_data.ks_selected_board_id = self.header.el
      .querySelector("#ks_dashboard_layout_dropdown_container")
      .querySelector(".ks_layout_selected").dataset.ks_layout_id;
    self.header.el
      .querySelector(
        ".ks_dashboard_top_menu-new .ks_dashboard_top_settings.new-features",
      )
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector(".ks_dashboard_top_menu-new .ks_am_content_element")
      ?.classList.remove("ks_hide");
    this.header.el
      .querySelectorAll(".ks_dashboard_top_settings.new-features")
      .forEach(function (element) {
        element.classList.remove("dn_hide");
      });
    self.header.el
      .querySelector(".ks_dashboard_layout_edit_mode_settings")
      ?.classList.add("ks_hide");
  ...
  _ksSetDiscardCurrentLayoutClick() {
    ...
    this.header.el
      ...
      ?.classList.add("ks_hide");
    this.header.el
      .querySelector(".ks_dashboard_top_menu-new .ks_dashboard_edit_layout")
      ?.classList.remove("ks_hide");
    this.header.el
      .querySelectorAll(".ks_dashboard_top_settings.new-features")
      .forEach(function (element) {
  ...
  ks_get_current_gridstack_config() {
    ...
    if (
      ...
      var items =
        document.querySelector(".grid-stack").gridstack.el.gridstack.engine
          .nodes;
    }
    var grid_config = {};
  ...
  async ksChartExportXlsCsv(e) {
    ...
    if (
      this.ks_dashboard_data.ks_item_data[chart_id].ks_dashboard_item_type ===
      "ks_list_view"
    ) {
      ...
      var data = {
        header: name,
        chart_data:
          typeof this.ks_dashboard_data.ks_item_data[chart_id]
            .ks_list_view_data === "string"
            ? this.ks_dashboard_data.ks_item_data[chart_id].ks_list_view_data
            : JSON.stringify(
                this.ks_dashboard_data.ks_item_data[chart_id].ks_list_view_data,
              ),
        ks_item_id: chart_id,
  ...
  ksChartExportPdf(e) {
    ...
    var name = this.ks_dashboard_data.ks_item_data[chart_id].name;
    var base64_image;
    base64_image = e.target
      .closest(".grid-stack-item")
      .querySelector(".ks_chart_card_body");
    var ks_height = base64_image.offsetHeight;
    html2canvas(base64_image, { useCORS: true, allowTaint: false }).then(
      function (canvas) {
  ...
  async updateBookmark(ev) {
    ...
    let updatedBookmarks = await rpc(
      "/web/dataset/call_kw/ks_dashboard_ninja.board/update_bookmarks",
      {
        model: "ks_dashboard_ninja.board",
  ...
  ksOnLayoutSelection(layout_id) {
    ...
    Object.entries(this.gridstackConfig).forEach((x, y) => {
      self.grid.update(document.getElementById(x[0]), {
        x: x[1]["x"],
        y: x[1]["y"],
        w: x[1]["w"],
        h: x[1]["h"],
    ...
    self.header.el
      ...
      ?.classList.remove("ks_layout_selected");
    self.header.el
      .querySelector(
        "li.ks_dashboard_layout_event[data-ks_layout_id='" + layout_id + "']",
      )
      ?.classList.add("ks_layout_selected");
    self.header.el.querySelector(
      "#ks_dn_layout_button span:first-child",
    ).textContent = selected_layout_name;
    self.header.el
      .querySelector(".ks_dashboard_top_menu-new .ks_dashboard_edit_layout")
      ?.classList.add("ks_hide");
    self.header.el
  ...
  _onKsSaveLayoutClick() {
    ...
    self.header.el
      .querySelector(".filters_section")
      ?.classList.remove("ks_hide");
    var dashboard_title = self.header.el.querySelector(
      "#ks_dashboard_title_input",
    ).value;
    self.header.el
      .querySelectorAll(".hide-in-edit")
      .forEach(function (element) {
        element.classList.remove("dn_hide");
      });

    self.header.el
      .querySelectorAll(".hide-in-edit.custom-date-range-selector")
      .forEach(function (element) {
        element.classList.add("dn_hide");
      });
  ...
  ks_update_child_board_value(dashboard_title, res_id, grid_config) {
    ...
    self.ks_dashboard_data.ks_child_boards = Object.assign(
      update_data,
      self.ks_dashboard_data.ks_child_boards,
    );
  }
  ...
  _ksRenderActiveMode() {
    ...
    if (self.ks_dashboard_data.ks_child_boards) {
      ...
      self.header.el
        .querySelector("#ks_dashboard_title .ks_am_element")
        ?.replaceWith(layout_container);
      self.header.el
        .querySelector("#ks_dashboard_title_label")
        ?.replaceWith(layout_container);
    }
    if (self.header.el.querySelector("#ks_dashboard_title_label")) {
      self.header.el.querySelector("#ks_dashboard_title_label").textContent =
        self.ks_dashboard_data.name;
    }
    ...
    self.header.el
      .querySelector(".ks_dashboard_print_pdf")
      ?.classList.remove("ks_hide");
    if (self.ks_dashboard_data.ks_item_data)
      self.header.el
        .querySelector(".ks_am_content_element")
        ?.classList.remove("ks_hide");

    self.header.el
      .querySelector(".ks_item_not_click")
      ?.classList.add("ks_item_click");
    self.header.el
      .querySelector(".ks_item_not_click")
      ?.classList.remove("ks_item_not_click");
    self.header.el
      .querySelector(".ks_dashboard_item")
      ?.classList.add("ks_dashboard_item_header_hover");
    self.header.el
      .querySelector(".ks_dashboard_item_header")
      ?.classList.add("ks_dashboard_item_header_hover");

    self.header.el
      .querySelector(".ks_dashboard_item_l2")
      ?.classList.add("ks_dashboard_item_header_hover");
    self.header.el
      .querySelector(".ks_dashboard_item_header_l2")
      ?.classList.add("ks_dashboard_item_header_hover");

    //      For layout 5
    self.header.el
      .querySelector(".ks_dashboard_item_l5")
      ?.classList.add("ks_dashboard_item_header_hover");

    self.header.el
      .querySelector(".ks_dashboard_item_button_container")
      ?.classList.add("ks_dashboard_item_header_hover");
    ...
    self.header.el
      ...
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector(".ks_dashboard_edit_mode_settings")
      ?.classList.add("ks_hide");
    self.header.el
      .querySelector("#ks_dashboard_layout_edit")
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector("#ks_import_item")
      ?.classList.remove("ks_hide");

    self.header.el
      .querySelector(".ks_start_tv_dashboard")
      ?.classList.remove("ks_hide");
    self.header.el
      .querySelector(".ks_chart_container")
      ?.classList.remove("ks_item_not_click", "ks_item_click");
    self.header.el
      .querySelector(".ks_list_view_container")
      ?.classList.remove("ks_item_click");
  ...
  _ksRenderEditMode() {
    ...
    self.header.el.querySelector("#ks_dashboard_title_input").value =
      self.ks_dashboard_data.name;
    ...
    self.header.el
      .querySelector(".ks_item_click")
      ?.classList.add("ks_item_not_click");
    self.header.el
      .querySelector(".ks_item_click")
      ?.classList.remove("ks_item_click");
    self.header.el
      .querySelector(".ks_dashboard_item")
      ?.classList.remove("ks_dashboard_item_header_hover");
    self.header.el
      .querySelector(".ks_dashboard_item_header")
      ?.classList.remove("ks_dashboard_item_header_hover");
    self.header.el
  ...
  ksImportItem(e) {
    ...
    fileReader.readAsText(
      self.header.el.querySelector(".ks_input_import_item_button").files[0],
    );
  }
  ...
  ksOnDashboardDeleteClick(ev) {
    ...
    self.dialogService.add(ConfirmationDialog, {
      ...
      confirm: () => {
        rpc(
          "/web/dataset/call_kw/ks.dashboard.delete.wizard/ks_delete_record",
          {
  ...
  ksOnDashboardDuplicateClick(ev) {
    ...
    var dashboard_id = this.ks_dashboard_id;
    rpc(
      "/web/dataset/call_kw/ks.dashboard.duplicate.wizard/DuplicateDashBoard",
      {
  ...
  kscreateaiitem(ev) {
    ...
    self.dialogService.add(FormViewDialog, {
      ...
      onRecordSaved: () => {
        this.notification.add(
          _t("Your charts have been successfully Generated with AI."),
          {
            type: "success",
          },
        );
  ...
  ks_gen_ai_analysis(ev) {
    ...
    if (ks_items.length > 0) {
      ks_items.map((item) => {
        if (!item.ks_ai_analysis) {
          ks_items_explain.push({
            name: item.name,
            id: item.id,
            ks_chart_data: item.ks_chart_data
              ? {
                  ...JSON.parse(item.ks_chart_data),
                  ...{ domains: [], previous_domain: [] },
                }
              : item.ks_chart_data,
            ks_list_view_data:
              typeof item.ks_list_view_data === "string"
                ? JSON.parse(item.ks_list_view_data)
                : item.ks_list_view_data,
      ...
      this.dialogService.add(ConfirmationDialog, {
        body: _t(
          "Do you agree that AI should be used to produce the explanation? It will take a few minutes to finish the process?",
        ),
        ...
        confirm: () => {
          rpc(
            "/web/dataset/call_kw/ks_dashboard_ninja.arti_int/ks_generate_analysis",
            {
            ...
            if (result) {
              self.actionService.doAction(
                ...
                },
                {
                  onClose: () => {
                    return rpc(
                      "/web/dataset/call_kw/ks_dashboard_ninja.arti_int/ks_switch_default_dashboard",
                      {
                        model: "ks_dashboard_ninja.arti_int",
  ...
  async speak_once(ev, item) {
    ...
    ev.currentTarget.parentElement
      .querySelector(".voice-cricle")
      .classList.toggle("d-none");
    ev.currentTarget.parentElement
      .querySelector(".comp-gif")
      .classList.toggle("d-none");
    ...
    if (!this.ks_speeches.length) {
      if (
        ks_audio.querySelector("audio").getAttribute("src") &&
        ks_audio.querySelector("audio").paused
      ) {
        ...
        ks_audio.querySelector(".comp-gif")?.classList.remove("d-none");
        ks_audio.querySelector(".voice-cricle")?.classList.add("d-none");
        this.ks_speeches.push(
          rpc(
            "/web/dataset/call_kw/ks_dashboard_ninja.arti_int/ks_generatetext_to_speech",
            {
              model: "ks_dashboard_ninja.arti_int",
            ...
            function (result) {
              if (result) {
                ...
                ks_audio.querySelector("audio").src =
  ...
  showDateFilterRange(ev) {
    ...
    this.header.el
      .querySelectorAll(".filters_section")
      .forEach(function (element) {
        element.classList.add("ks_hide");
      });
