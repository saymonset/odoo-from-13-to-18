...
export class KsGraphPreview extends Component {
  ...
  _Ks_render() {
    ...
    var graphTextElements = self.graphref.el.querySelectorAll("div.graph_text");
    ...
    if (
      ...
      rec.ks_dashboard_item_type !== "ks_to_do"
    ) {
      if (rec.ks_data_calculation_type !== "query") {
        if (rec.ks_model_id) {
          if (
            ...
            self.graphref.el.appendChild(
              this.createGraphTextElement(
                "Select Group by date to create chart based on date groupby",
              ),
            );
          } else if (
            rec.ks_dashboard_item_type !== "ks_scatter_chart" &&
            ...
            rec.ks_dashboard_item_type !== "ks_scatter_chart" &&
            rec.ks_chart_data_count_type !== "count" &&
            (rec.ks_chart_measure_field.count === 0 ||
              !rec.ks_chart_relation_groupby)
          ) {
            self.graphref.el.appendChild(
              this.createGraphTextElement(
            ...
            self.graphref.el.appendChild(
              this.createGraphTextElement("Select Chart Data Count Type"),
            );
            ...
            if (rec.ks_scatter_measure_x_id && rec.ks_chart_measure_field) {
              ...
              self.graphref.el.appendChild(
                this.createGraphTextElement("Please Choose Measures"),
              );
            }
          } else {
            this.get_chart(rec);
          ...
          self.graphref.el.appendChild(
            this.createGraphTextElement("Select a Model first."),
          );
        }
      } else if (
        ...
        self.graphref.el.appendChild(
          this.createGraphTextElement("Please run the appropriate Query"),
        );
      }
    }
  }
        ...
        switch (chart_type) {
          case "ks_bar_chart":
            ...
            for (let k = 0; k < ks_data.length; k++) {
              ...
              if (
                self.props.record.data.ks_bar_chart_stacked == true &&
                ks_data[k].type != "line"
              ) {
              ...
              if (
                self.props.record.data.ks_chart_measure_field_2 &&
                ks_data[k].type == "line"
              ) {
                var tooltip = am5.Tooltip.new(this.root, {
                  textAlign: "center",
          ...
          case "ks_horizontalBar_chart":
            ...
            for (let k = 0; k < ks_data.length; k++) {
              ...
              if (
                ...
                ks_data[k].type != "line" &&
                self.props.record.data.ks_dashboard_item_type ==
          ...
          case "ks_doughnut_chart":
            var series = [];
            if (
              rec.ks_semi_circle_chart == true &&
              (rec.ks_dashboard_item_type == "ks_pie_chart" ||
                rec.ks_dashboard_item_type == "ks_doughnut_chart")
            ) {
            ...
            for (let item of series) {
              ...
              if (self.props.record.data.ks_show_data_value == true) {
                item.labels.template.setAll({
                  text:
                    self.props.record.data.ks_data_label_type == "value"
                      ? "{value}"
                      : "{valuePercentTotal}%",
                  inside: true,
          ...
          case "ks_radialBar_chart":
            ...
            if (rec.ks_dashboard_item_type == "ks_polarArea_chart") {
              for (let k = 0; k < ks_data.length; k++) {
                ...
                series.set(
                  "stroke",
                  self.root.interfaceColors.get("background"),
                );
              ...
              for (let k = 0; k < ks_data.length; k++) {
                ...
                var series = chart.series.push(
                  am5radar.RadarLineSeries.new(self.root, {
                    name: `${ks_data[k].label}`,
                    xAxis: xAxis,
                    yAxis: yAxis,
                    valueYField: `${ks_data[k].label}`,
...
registry
  .category("fields")
  .add("ks_dashboard_graph_preview", KsGraphPreviewfield);
