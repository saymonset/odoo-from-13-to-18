...
export function dnNavBarAddClasses() {
  ...
  document
    ...
    ?.classList.add("navbar", "navbar-expand-md");
  document.querySelector(".ks-mobile-burger-menu")?.classList.remove("d-none");
  document
    .querySelector(".mobile-burger-menu-default")
...
export function dnNavBarRemoveClasses() {
  ...
  document
    .querySelector(".o_main_navbar")
    ?.classList.remove("navbar", "navbar-expand-md");
...
patch(MenuDropdown.prototype, {
  setup() {
    ...
    useEffect(
      () => {
        if (
          this.props?.slots?.toggler?.__ctx?.section?.childrenTree?.length >= 9
        ) {
  ...
  addIcons() {
    ...
    if (
      ...
      dropdownItems.forEach((dropdownItem) => {
        let dropdownName =
          menuName !== "My Dashboards"
            ? dropdownItem.textContent
            : "templateIcon";
        let icon =
          "<span class='me-2 dash-dd-icons'>" +
          dnMenuIcons[menuName][dropdownName] +
          "</span>";
        ...
        if (
          currentActiveMenu &&
          parseInt(dropdownItem.dataset.section) === currentActiveMenu
        )
...
patch(NavBar.prototype, {
  setup() {
    super.setup();
    onMounted(() => {
      let apps_menu_icon = this.root.el?.querySelector(
  ...
  async adapt() {
    if (
      this.currentApp?.xmlid === "ks_dashboard_ninja.board_menu_root" ||
      this.actionService?.currentController?.action.tag === "ks_dashboard_ninja"
    ) {
  ...
  setup() {
    ...
    onPatched(() => {
      if (
        this?.env.services.menu.getCurrentApp?.()?.xmlid ===
          "ks_dashboard_ninja.board_menu_root" ||
        this.info?.componentProps?.action?.tag === "ks_dashboard_ninja"
      ) {
        if (!document.querySelector("body").classList.contains("ks_body_class"))
          dnNavBarAddClasses();
      } else if (
