...
class KsDashboardNInjaAI(models.TransientModel):
    ...
    ks_type = fields.Selection(
        [("ks_model", "Model"), ("ks_keyword", "Keywords")],
        string="Ks AI Type",
        default="ks_model",
    )

    ks_import_model_id = fields.Many2one(
        "ir.model",
        string="Model ID",
    ...
    ks_import_model = fields.Many2one(
        "ir.model",
        string="Model",
        domain="[('access_ids','!=',False),('transient','=',False),"
    ...
    def _compute_show_model(self):
        if self.ks_input_keywords and self.ks_type == "ks_keyword":
            api_key = (
                ...
                .sudo()
                .get_param("ks_dashboard_ninja.dn_api_key")
            )
            url = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param("ks_dashboard_ninja.url")
            )
            if api_key and url:
                json_data = {
                    "name": api_key,
    ...
    def ks_do_action(self):
        ...
        if self.ks_import_model_id:
            ...
            ks_filtered_fields = {
                key: val
                for key, val in ks_fields.items()
                if val["type"] not in ["many2many", "one2many", "binary"]
                and val["name"] != "id"
                and val["name"] != "sequence"
                and val["store"] == True
            }
            ks_fields_name = {
                val["name"]: val["type"] for val in ks_filtered_fields.values()
            }
                ...
                if ks_ai_response.status_code == 200:
                    ...
                    ks_create_record = self.env["ks_dashboard_ninja.board"].create(
                        {
                            ...
                            "ks_dashboard_default_template": self.env.ref(
                                "ks_dashboard_ninja.ks_blank", False
                            ).id,
                            "ks_dashboard_top_menu_id": self.env["ir.ui.menu"]
                            .search([("name", "=", "My Dashboards")])[0]
                            .id,
                        }
                    )
                    ks_dash_id = ks_create_record.id

                    ks_result = self.env["ks_dashboard_ninja.item"].create_ai_dash(
                        ks_ai_response, ks_dash_id, ks_model_name
                    )
                    context = {
                        "ks_dash_id": self._context["ks_dashboard_id"],
                        "ks_dash_name": self.env["ks_dashboard_ninja.board"]
                        .search([("id", "=", self._context["ks_dashboard_id"])])
                        .name,
                    ...
                    if ks_result == "success":
                        return {
                            ...
                            "params": {
                                "ks_dashboard_id": ks_create_record.id,
                        ...
                        raise ValidationError(
                            _(
                                "Items didn't render because AI provides invalid response for this model.Please try again"
                            )
                        )
                else:
                    raise ValidationError(
                        _("AI Responds with the following status:- %s")
                        % ks_ai_response.text
                    )
            else:
                raise ValidationError(
                    _("Please enter URL and API Key in General Settings")
                )
        else:
            ...
            if json.loads(ks_response.text) != False and ks_response.status_code == 200:
                ...
                if api_key and url:
                    ...
                    if ks_ai_response.status_code == 200:
                        ...
                        if (
                            self.env["ir.model"]
                            .search([("model", "=", ks_model_name)])
                            .id
                            or self.env["ir.model"]
                            .search([("name", "=", ks_model_name)])
                            .id
                        ):
                            if (
                                self.env["ir.model"]
                            ...
                            ].create_ai_dash(ks_ai_response, ks_dash_id, ks_model_name)
                            if ks_result == "success":
                                return {
                                    "type": "ir.actions.client",
                                    "tag": "reload",
                                ...
                                raise ValidationError(
                                    _("Items didn't render, please try again!")
                                )
                        else:
                            raise ValidationError(
                                _("%s model does not exist.Please install")
                                % ks_model_name
                            )
                    else:
                        raise ValidationError(
    ...
    def ks_generate_analysis(self, ks_items_explain, ks_rest_items, dashboard_id):
        if ks_items_explain:
            ...
            words = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param("ks_dashboard_ninja.ks_analysis_word_length")
            )
                ...
                if api_key and url:
                    ...
                    if ks_response.status_code == 200 and json.loads(ks_response.text):
                        ...
                        if item["analysis"] or item["insights"]:
                            try:
                                self.env["ks_dashboard_ninja.item"].browse(
                                    item["id"]
                                ).write(
                                    {
                                        "ks_ai_analysis": item["analysis"]
                                        + "ks_gap"
                                        + item["insights"]
                                    }
                                )
            ...
            if len(result):
                ...
                raise ValidationError(
                    _("AI Responds with the wrong analysis. Please try again ")
                )
        elif ks_rest_items:
            if self.env.context.get("explain_items_with_ai", False):
                self.env["ks_dashboard_ninja.board"].browse(dashboard_id).write(
    ...
    def ks_generatetext_to_speech(self, item_id):
        if item_id:
            try:
                ks_text = (
                    self.env["ks_dashboard_ninja.item"].browse(item_id).ks_ai_analysis
                )
                if ks_text:
                    language = "en"
    ...
    def ks_gen_chat_res(self, **kwargs):
        ...
        url = (
            self.env["ir.config_parameter"].sudo().get_param("ks_dashboard_ninja.url")
            + "/api/v1/get_sql_query"
        )
        data = {
    ...
    def ks_gen_dataframe(self, ks_query, question):
        ...
        if not all([host, user, port, password, db]):
            ...
            ks_fixed_url = (
                ...
                .sudo()
                .get_param("ks_dashboard_ninja.url")
                + "/api/v1/get_fixed_query"
            )
            try:
                df = pd.read_sql(ks_query, sql_uri)
            except Exception as e:
                ks_query_data = {"query": ks_query, "error": e}
