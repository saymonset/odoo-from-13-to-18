...
class KsDashboardNinjaBoardItemAction(models.Model):
    ...
    ks_computed_group_access = fields.Many2many(
        "res.groups", compute="_compute_ks_computed_group_access", store=True
    )

    @api.depends("ks_dashboard_ninja_id.ks_dashboard_group_access")
    def _compute_ks_computed_group_access(self):
        for record in self:
            record.ks_computed_group_access = (
                record.ks_dashboard_ninja_id.ks_dashboard_group_access
            )
