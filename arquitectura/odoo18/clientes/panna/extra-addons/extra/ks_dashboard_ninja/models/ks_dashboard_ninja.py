...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_dashboard_top_menu_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref(
            "ks_dashboard_ninja.dashboards_menu_root", False
        ),
    )
    ...
    ks_dashboard_favourite_filters_ids = fields.One2many(
        "ks_dashboard_ninja.favourite_filters",
        "ks_dashboard_board_id",
        string="Dashboard Favourite Filters",
    )
    ...
    def _default_dashboard_image(self):
        return image_data_uri(
            encodebytes(
                file_open(
                    "ks_dashboard_ninja/static/images/dashboardOverview/defaultDashboard.png",
                    "rb",
                ).read()
            )
        )

    ks_dn_dashboard_image = fields.Binary(
        "Dashboard Image", default=_default_dashboard_image, attachment=False
    )
        ...
        for record in records:
            if record.ks_dashboard_top_menu_id and record.ks_dashboard_menu_name:
                action_id = {
                    ...
                    "params": {
                        "ks_dashboard_id": record.id,
                        "ks_dashboard_name": record.ks_dashboard_menu_name,
                    },
                }
                ...
                group_ids = (
                    record.ks_dashboard_group_access.ids
                    if record.ks_dashboard_group_access
                    else []
                )
            ...
            if (
                ...
                for item_data in template_data:
                    if (
                        record.ks_dashboard_default_template.ks_template_type
                        == "ks_custom"
                    ):
                        dashboard_item = (
                        ...
                        ks_gridstack_config[dashboard_item.id] = template_data[
                            item_data
                        ]
                    else:
    ...
    def ks_update_menu_id_old_db(self):
        ...
        for rec in ks_records:
            if (rec.name == "My Dashboard" and rec.ks_dashboard_state == "Locked") or (
                rec.name
                in ["Template1 Dashboard", "Template2 Dashboard", "Template3 Dashboard"]
                and rec.ks_dashboard_top_menu_id.name == "My Dashboard"
            ):
                ...
                if not rec.ks_dashboard_menu_id:
                    action_id = {
                        ...
                        "tag": "ks_dashboard_ninja",
                        "params": {
                            "ks_dashboard_id": rec.id,
                            "ks_dashboard_name": rec.ks_dashboard_menu_name,
                        },
                    }
                    rec.ks_dashboard_client_action_id = (
                        self.env["ir.actions.client"].sudo().create(action_id)
                    )

                    rec.ks_dashboard_menu_id = (
                        self.env["ir.ui.menu"]
                        .sudo()
                        .create(
                            {
                                ...
                                "action": "ir.actions.client,"
                                + str(rec.ks_dashboard_client_action_id.id),
                                "groups_id": rec.ks_dashboard_group_access.ids
                                if rec.ks_dashboard_group_access
                                else False,
                                "sequence": rec.ks_dashboard_menu_sequence
        ...
        if self.env["ks_dashboard_ninja.child_board"].search(
            ...
            dashboard_data["ks_child_boards"] = {
                "ks_default": [
                    ...
                    default_grid_id[0].ks_gridstack_config,
                ]
            }
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        if rec.ks_actions:
            ...
            context.update({"resId": ks_client_action.id})
    ...
    def fetch_dashboard_overview(self, **kwargs):
        ...
        dashboards = ks_board_model.search_read(
            domain=[], fields=["name", "is_bookmarked", "ks_dn_dashboard_image"]
        )
        bookmarked_dashboards = ks_board_model.search_count(
            [("is_bookmarked", "=", True)]
        )
        chart_count = 0
        map_count = ks_board_item_model.search_count(
        ...
        list_view_count = ks_board_item_model.search_count(
            [
                ...
                ("ks_dashboard_ninja_board_id", "!=", False),
            ]
        )
        chart_list = [
            "ks_bar_chart",
        ...
        for dashboard in dashboards:
            charts = ks_board_item_model.search_count(
                [
                    ...
                    ("ks_dashboard_item_type", "in", chart_list),
                ]
            )
            chart_count += charts
        ...
        return {
            ...
            "isManager": self.env.user.has_group(
                "ks_dashboard_ninja.ks_dashboard_ninja_group_manager"
            ),
        }
    ...
    def update_bookmarks(self, *args):
        ...
        return [
            self.env["ks_dashboard_ninja.board"].search_count(
                [("is_bookmarked", "=", True)]
            ),
            self.is_bookmarked,
        ]
            ...
            ks_list_view_group_fields_list = self.env["ir.model.fields"].search(
                [
                    (
                        "id",
                        "in",
                        ks_many2many_field_ordering.get(
    ...
    def ks_dashboard_export(self, ks_dashboard_ids, **kwargs):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            dashboard_data = self.ks_prepare_export_data_vals(
                ks_dashboard_rec, grid_conf=grid_conf
            )
            if selecred_rec:
                ...
                dashboard_data["ks_gridstack_config"] = (
                    ks_dashboard_rec.ks_child_dashboard_ids[0].ks_gridstack_config
                )
    ...
    def ks_import_dashboard(self, file, menu_id):
        ...
        if (
            ...
            for i in range(len(ks_dashboard_data)):
                if "ks_set_interval" in ks_dashboard_data[
                    i
                ].keys() and ks_dashboard_data[i].get("ks_item_data", False):
                    # del ks_dashboard_data[i]['ks_set_interval']
                    for j in range(
                        len(ks_dashboard_data[i].get("ks_item_data", False))
                    ):
                        if (
                            "ks_update_items_data"
                            in ks_dashboard_data[i].get("ks_item_data", False)[j].keys()
                        ):
                            del ks_dashboard_data[i].get("ks_item_data", False)[j][
                                "ks_update_items_data"
                            ]
                        if (
                            "ks_auto_update_type"
        ...
        for data in ks_dashboard_data:
            ...
            for id_index, id in enumerate(item_ids):
                if data["ks_gridstack_config"] and str(id) in ks_gridstack_config:
                    ks_grid_stack_config[str(item_new_ids[id_index])] = (
    ...
    def ks_prepare_item(self, item):
        try:
            ...
            ks_many2many_field_ordering = (
                item["ks_many2many_field_ordering"]
                if item.get("ks_many2many_field_ordering", False)
                else False
            )
            ...
            if ks_many2many_field_ordering:
                ...
                ks_list_view_fields_name = ks_many2many_field_ordering.get(
                    "ks_list_view_fields_name", False
                )
                ks_chart_measure_field_name = ks_many2many_field_ordering.get(
                    "ks_chart_measure_field_name", False
                )
                ks_chart_measure_field_2_name = ks_many2many_field_ordering.get(
                    "ks_chart_measure_field_2_name", False
                )
            ...
            if (
                ...
                and len(ks_list_view_group_fields_name) > 0
            ):
            ...
            if item["ks_chart_relation_sub_groupby"]:
                ...
                if ks_chart_relation_sub_groupby:
                    item["ks_chart_relation_sub_groupby"] = (
                        ks_chart_relation_sub_groupby.id
                    )
                else:
                    item["ks_chart_relation_sub_groupby"] = False

            if (
                item["ks_dashboard_item_type"] == "ks_scatter_chart"
                and item["ks_scatter_measure_x_id"]
            ):
            ...
            if item["ks_model_id_2"]:
                ...
                ks_model_id_2 = (
                    self.env["ir.model"]
                    .search([("model", "=", item["ks_model_id_2"])])
                    .id
                )
    ...
    def ks_prepare_dashboard_custom_domain(self):
        ...
        for rec in custom_filter_ids:
            data[str(rec.id)] = {
                ...
                "relation": rec.ks_domain_field_id.relation
                if rec.ks_domain_field_id.ttype in ["many2many", "many2one", "one2many"]
                else False,
                "special_data": {},
            }
    ...
    def ks_prepare_dashboard_favourite_filter(self):
        data = {}
        ks_favourite_filter_ids = self.env[
            "ks_dashboard_ninja.favourite_filters"
        ].search(
            [
                ["id", "in", self.ks_dashboard_favourite_filters_ids.ids],
                "|",
                ["ks_access_id", "=", self.env.user.id],
                ["ks_access_id", "=", 0],
            ],
            order="create_date",
        )
