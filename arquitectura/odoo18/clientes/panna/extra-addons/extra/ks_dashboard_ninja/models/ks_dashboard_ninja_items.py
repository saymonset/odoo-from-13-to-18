...
class KsDashboardNinjaItems(models.Model):
    ...
    zoom_enabled = fields.Boolean(
        string="Zoom enabled?", compute="compute_zoom_enabled"
    )
    ...
    def compute_zoom_enabled(self):
        for rec in self:
            rec.zoom_enabled = (
                ...
                .sudo()
                .get_param("ks_dashboard_ninja.enable_chart_zoom")
            )
    ...
    ks_icon_select = fields.Selection(
        [
            ("Default", "Default"),
            ("Custom", "Custom"),
        ],
        string="Icon Option",
        default=("Default"),
        help="Choose the Icon option. ",
    )
    ks_font_color = fields.Char(
        default="#000000,0.99", string="Font Color", help="Select the font color. "
    )
    ...
    ks_email_to_ids = fields.Many2many(
        "ks_dashboard_ninja.kpi_mail", string="Email Address"
    )
    ...
    ks_target_view = fields.Selection(
        [
            ("Number", "Number"),
    ...
    ks_data_comparison = fields.Selection(
        [
            ("None", "None"),
            ("Sum", "Sum"),
            ("Ratio", "Ratio"),
    ...
    ks_currency_id = fields.Many2one(
        "res.currency",
        string="Currency",
    ...
    ks_hide_legend = fields.Boolean(
        "Show Legend", help="Hide all legend from the chart item", default=False
    )
    ks_radial_legend = fields.Boolean(
        "Show Radial Legend", help="Hide all legend from the chart item", default=False
    )
    ...
    ks_scatter_measure_x_id = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
        "('ttype','=','integer'),('ttype','=','float'),"
    ...
    ks_scatter_measure_y_id = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),'|','|',"
        "('ttype','=','integer'),('ttype','=','float'),"
    ...
    ks_scatter_field_id = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),('ttype','!=','binary'),"
        "('ttype','!=','many2many'), ('ttype','!=','one2many')]",
    ...
    ks_funnel_record_field = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('store','=',True),'|',"
        "'|',('ttype','=','integer'),('ttype','=','float'),"
        "('ttype','=','monetary')]",
    ...
    ks_map_record_field = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('store','=',True),'|',"
        "'|',('ttype','=','integer'),('ttype','=','float'),"
        "('ttype','=','monetary')]",
    ...
    ks_map_chart_relation_groupby = fields.Many2one(
        "ir.model.fields",
        domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
        "('store','=',True),('ttype','!=','binary'),"
    ...
    ks_flower_view_preview = fields.Char(
        string="Flower Preview", default="Flower Preview"
    )
    ...
    ks_group_by_lines = fields.One2many(
        "ks.dashboard.group.by", "ks_dashboard_group_by_id", string="Group By Lines"
    )
    ks_csv_group_by_lines = fields.One2many(
        "ks.dashboard.csv.group.by",
        "ks_dashboard_csv_group_by_id",
        string="CSV Group By Lines",
    )
    ...
    def create_ai_dash(self, data, ks_dash_id, model):
        try:
            ...
            for item in data:
                ...
                if item["chart_type"].lower() in [
                    ...
                    ks_measure_id = self.env["ir.model.fields"].search(
                        [
                            ("name", "=", item["aggregations"][0]["field"]),
                            ("model", "=", model),
                        ]
                    )
                    if ks_measure_id and ks_measure_id["ttype"] in [
                        "integer",
                        "float",
                        "monetary",
                    ]:
                    ...
                    if ks_record_id:
                        ...
                        if (
                            ks_record_id["ttype"] == "datetime"
                            or ks_record_id["ttype"] == "date"
                        ):
                    ...
                    if item["aggregations"][0]["type"] == "avg":
                        ...
                        value["ks_chart_data_count_type"] = item["aggregations"][0][
                            "type"
                        ]

                    value["ks_dashboard_item_type"] = chart_switch.get(
                        item["chart_type"], False
                    )
                    ...
                    if ks_measure_id and ks_measure_id["ttype"] in [
                        ...
                        ks_measure_field_ids.append(ks_measure_id.id)
                        value["ks_list_view_group_fields"] = [
                            (6, 0, ks_measure_field_ids)
                        ]
            ...
            raise ValidationError(
                _("Getting invalid response from AI, please try again")
            )
    ...
    def read_csv(self):
        if self.ks_csv_field:
            if " " in self.filename or "_" in self.filename:
                ...
                raise ValidationError(
                    "Please add filename which contain Spaces and Underscore in there name only."
                )
        else:
            if self.ks_model_id:
            ...
            if " " in self.filename or "_" in self.filename:
                try:
                    ...
                    if file_type[1] == ".xlsx":
                        ...
                        raise ValueError(
                            "Unsupported file format. Only XLSX and XLS are supported."
                        )
    ...
    def data_sync(self):
        ...
        for rec in data:
            self.write({"ks_group_by_lines": [(0, 0, {"name": rec.name})]})

    def csv_data_sync(self):
        data = self.env["ks.dashboard.csv.new"].search([])
        for rec in data:
            self.write({"ks_csv_group_by_lines": [(0, 0, {"name": rec.name})]})
    ...
    def create_table(self):
        ...
        if records:
            for rec in records:
                ...
                if not rec.ttype:
                    raise ValidationError(
                        "Please Enter the type under Column Data Type Tab"
                    )
                values.update(
                    {"name": rec.name.lower().replace(" ", "_"), "type": rec.ttype}
                )
                dict.append(values)
            ...
            for row_no in range(df.shape[0]):
                    ...
                    if "Date" in field or "Deadline" in field:
                        if line[value] != "NaT" and line[value] != False:
                            if self.ks_group_by_lines[value].ttype == "datetime":
                                ...
                                formatted_utc_datetime = utc_datetime.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                        ...
                        while value < len(line):
                            if line[value] != "nan" and line[value] != False:
                                if "." in line[value]:
                                    if "," in line[value]:
                                        if (
                                            self.ks_group_by_lines[value].ttype
                                            == "char"
                                        ):
                                    ...
                                    self.ks_group_by_lines[value].ttype == "integer"
                                    and line[value] != "nan"
                                ):
                                    try:
                                        values.update(
                                    ...
                                    self.ks_group_by_lines[value].ttype == "float"
                                    and line[value] != "nan"
                                ):
                                    try:
                                        values.update(
                ...
                try:
                    if values.keys():
                        data_values = dict(
                            [
                                ("x_" + key.lower().replace(" ", "_"), values[key])
                                for key in values
                                if values[key] != "Null"
                            ]
                        )
                    ...
                    raise ValidationError(
                        "found error while Table creation {}".format(e)
                    )
                ...
                for line in csv_reader:
                    ...
                    for field in fields:
                        ...
                        if "Date" in field or "Deadline" in field:
                            if line[value]:
                                if (
                                    self.ks_csv_group_by_lines[value].ttype
                                    == "datetime"
                                ):
                                    user_datetime_str = line[value]
                                    local_datetime = datetime.strptime(
                                    ...
                                    utc_datetime = localized_datetime.astimezone(
                                        pytz.utc
                                    )
                                    formatted_utc_datetime = utc_datetime.strftime(
                                        "%Y-%m-%d %H:%M:%S"
                    ...
                    try:
                        ...
                        raise ValidationError(
                            "found error while Table creation error {}".format(e)
                        )
    ...
    def check_target(self):
        ...
        for res in sales_target:
            if res.ks_record_count >= res.ks_standard_goal_value:
                ...
                action_id = (
                    res.ks_dashboard_ninja_board_id.ks_dashboard_menu_id.action.id
                    if res.ks_dashboard_ninja_board_id.ks_dashboard_menu_id.action
                    and res.ks_dashboard_ninja_board_id.ks_dashboard_menu_id.action.id
                    else menu_record.action.id
                )
                for partner in res.ks_email_to_ids:
                    recipient_emails.append(partner.name)
                kpi_mail = self.env["mail.mail"].create(
                    {
                        "body_html": "<p>Congratulations! The Target of "
                        + str(res.ks_standard_goal_value)
                        + " for "
                        + res.name
                        + " is achieved!!</p>"
    ...
    def copy_data(self, default=None):
        ...
        if "ks_multiplier_lines" not in default:
            default["ks_multiplier_lines"] = [
                (0, 0, line.copy_data()[0]) for line in self.ks_multiplier_lines
            ]
        ...
        if ks_many2many_field_ordering:
            ...
            ks_list_view_group_fields = ks_many2many_field_ordering.get(
                "ks_list_view_group_fields", False
            )
            ks_list_view_fields = ks_many2many_field_ordering.get(
                "ks_list_view_fields", False
            )
            ks_chart_measure_field = ks_many2many_field_ordering.get(
                "ks_chart_measure_field", False
            )
            ks_chart_measure_field_2 = ks_many2many_field_ordering.get(
    ...
    def create(self, values):
        ...
        for i in range(len(values)):
            if not values[i].get("ks_many2many_field_ordering", False):
                ...
                if (
                    values[i].get("ks_list_view_group_fields", False)
                    and len(values[i]["ks_list_view_group_fields"][0][2]) > 0
                ):
                ...
                if (
                    values[i].get("ks_list_view_fields", False)
                    and len(values[i]["ks_list_view_fields"][0][2]) > 0
                ):
                ...
                if (
                    values[i].get("ks_chart_measure_field", False)
                    and len(values[i]["ks_chart_measure_field"][0][2]) > 0
                ):
                ...
                if (
                    values[i].get("ks_chart_measure_field_2", False)
                    and len(values[i]["ks_chart_measure_field_2"][0][2]) > 0
                ):
                ...
                ks_many2many_field_ordering = {
                    "ks_list_view_fields": values[i]["ks_list_view_fields"][0][2]
                    if values[i].get("ks_list_view_fields", False)
                    else [],
                    "ks_list_view_fields_name": ks_list_view_fields_name,
                    "ks_list_view_group_fields": values[i]["ks_list_view_group_fields"][
                        0
                    ][2]
                    if values[i].get("ks_list_view_group_fields", False)
                    else [],
                    "ks_list_view_group_fields_name": ks_list_view_group_fields_name,
                    "ks_chart_measure_field": values[i]["ks_chart_measure_field"][0][2]
                    if values[i].get("ks_chart_measure_field", False)
                    else [],
                    "ks_chart_measure_field_name": ks_chart_measure_field_name,
                    "ks_chart_measure_field_2": values[i]["ks_chart_measure_field_2"][
                        0
                    ][2]
                    if values[i].get("ks_chart_measure_field_2", False)
                    else [],
                    "ks_chart_measure_field_2_name": ks_chart_measure_field_2_name,
                }
                values[i]["ks_many2many_field_ordering"] = json.dumps(
                    ks_many2many_field_ordering
                )
    ...
    def ks_list_view_fields_onchange(self):
        ...
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_list_view_fields"] = (
                rec.ks_list_view_fields.ids
            )
            ks_many2many_field_ordering["ks_list_view_fields_name"] = [
                x.name for x in rec.ks_list_view_fields
            ]
    ...
    def ks_list_view_group_fields_onchange(self):
        ...
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_list_view_group_fields"] = (
                rec.ks_list_view_group_fields.ids
            )
    ...
    def ks_chart_measure_field_onchange(self):
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_chart_measure_field"] = (
                rec.ks_chart_measure_field.ids
            )
            ks_many2many_field_ordering["ks_chart_measure_field_name"] = [
                x.name for x in rec.ks_chart_measure_field
            ]
            rec.ks_many2many_field_ordering = json.dumps(ks_many2many_field_ordering)
    ...
    def ks_chart_measure_field_2_onchange(self):
        ...
        for rec in self:
            ...
            ks_many2many_field_ordering["ks_chart_measure_field_2"] = (
                rec.ks_chart_measure_field_2.ids
            )
    ...
    def layout_four_font_change(self):
        if self.ks_dashboard_item_theme != "white":
            if self.ks_layout == "layout4" and self.ks_dashboard_item_theme in [
                "red",
                "blue",
                "yellow",
                "green",
            ...
            elif self.ks_layout == "layout4" and self.ks_dashboard_item_theme not in [
                "red",
                "blue",
                "yellow",
                "green",
    ...
    def unlink(self):
        channel = self.env["discuss.channel"].search(
            [("ks_dashboard_item_id", "in", self.ids)]
        )
        if channel:
            channel.unlink()
        ...
        if (
            ...
            if rec.ks_chart_data_count_type == "count":
                ...
                if not rec.ks_sort_by_field:
                    ...
                    if (
                        rec.ks_sort_by_field.ttype == "many2one"
                        and rec.ks_sort_by_field.name
                        == rec.ks_chart_relation_groupby.name
                    ):
                        ks_chart_measure_field_with_type.append(
                            rec.ks_sort_by_field.name
                        )
                    elif rec.ks_sort_by_field.ttype not in ["datetime", "date"]:
                        ks_chart_measure_field_with_type.append(
            ...
            if (
                rec.ks_dashboard_item_type == "ks_map_view"
                and ks_chart_data
                and ks_chart_data.get("groupByIds", False)
            ):
                map_fields = ["partner_latitude", "partner_longitude", "name"]
                map_domain = [["id", "in", ks_chart_data["groupByIds"]]]
                ks_chart_data["partner"] = self.env["res.partner"].search_read(
                    map_domain, map_fields
                )
                if rec.ks_partners_map:
                    ks_map_domain = [["id", "in", json.loads(rec.ks_partners_map)]]
                    ks_chart_data["ks_partners_map"] = self.env[
                        "res.partner"
                    ].search_read(ks_map_domain, map_fields)
            ...
            if self.ks_chart_groupby_type == "relational_type":
                ...
                ks_list_view_data["label"].append(
                    ...
                    + (
                        "Month-Year"
                        if name == "month_year"
                        else ks_chart_date_groupby.capitalize()
                    )
                )
                ks_list_view_data["fields"].append(
                    self.ks_chart_relation_groupby.ids[0]
            ...
            if action_line.ks_item_action_field.ttype == "many2one":
                ...
                for res in ks_list_view_records:
                    ...
                    for field_rec in ks_list_fields:
                        if counter == 0:
                            data_row["data"].append(
                                res[field_rec][1] if res[field_rec] else "False"
                            )
                        else:
                            data_row["data"].append(res[field_rec])
                        counter += 1
                ...
                if records:
                    if rec.ks_model_name == "res.partner":
                        ...
                        if "partner_id" in records:
                            ...
                            raise UserError(
                                _("Selected model is not supported for Map View.")
                            )
...
class KsDashboardCsvGroupBy(models.Model):
    ...
    ks_dashboard_csv_group_by_id = fields.Many2one(
        "ks_dashboard_ninja.item", string="Dashboard Item"
    )
...
class KsDashboardGroupBy(models.Model):
    ...
    ks_dashboard_group_by_id = fields.Many2one(
        "ks_dashboard_ninja.item", string="Dashboard Item Id"
    )
    name = fields.Char(string="Name")
    ttype = fields.Selection(
        [
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_action_item_name = fields.Char(
        string="Action item name", compute="ks_check_special_character"
    )
