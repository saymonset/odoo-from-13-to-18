...
class import_account_payment_from_xml(models.TransientModel):
    ...
    def import_xml_file_button(self):
        ...
        if ext[1:].lower() != "xml":
            raise UserError(
                _('Formato no soportado "{}", importa solo archivos XML').format(
                    self.file_name
                )
            )
        ...
        payment_vals = {
            ...
            "cep_numeroCertificado": tree.get(
                "numeroCertificado", tree.get("NumeroCertificado")
            ),
    ...
    def import_xml_file_button_cargar(self):
        ...
        cargar_values = {
            ...
            "methodo_pago": "MetodoPago" in xml_data.attrib
            and xml_data.attrib["MetodoPago"]
            or "",
            "forma_pago_id": "FormaPago" in xml_data.attrib
            and self.env["catalogo.forma.pago"]
            .sudo()
            .search([("code", "=", xml_data.attrib["FormaPago"])])
            or "",
            "uso_cfdi_id": self.env["catalogo.uso.cfdi"]
            .sudo()
            .search([("code", "=", Receptor.attrib["UsoCFDI"])]),
            "folio_fiscal": TimbreFiscalDigital.attrib["UUID"],
            "tipo_comprobante": xml_data.attrib["TipoDeComprobante"],
            "fecha_factura": xml_data.attrib["Fecha"]
            and parse(xml_data.attrib["Fecha"]).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            or False,
            ...
            "tipocambio": xml_data.find("TipoCambio")
            and xml_data.attrib["TipoCambio"]
            or "1",
                        ...
                        for traslado in traslados:
                            if "TasaOCuota" in traslado.attrib:
                                if traslado.attrib["TipoFactor"] == "Cuota":
                                    ...
                                    tasa = str(
                                        float(traslado.attrib["TasaOCuota"]) * 100
                                    )
                            else:
                                tasa = str(0)
                        ...
                        traslados.append(
                            {
                                ...
                                "tasa": tasa_tr,
                                "importe": invoice_id.set_decimals(
                                    line["amount"], invoice_id.currency_id.no_decimales
                                )
                                if tax.tipo_factor != "Exento"
                                else "",
                                "base": invoice_id.set_decimals(
                                    line["base"], invoice_id.currency_id.no_decimales
                                ),
                                "tax_id": line["tax_id"],
                            }
                        )
    ...
    def import_xml_file_payment(self):
        ...
        cargar_values = {
            ...
            "folio_fiscal": TimbreFiscalDigital.attrib["UUID"],
            #'tipo_comprobante': xml_data.attrib['TipoDeComprobante'],
            "fecha_pago": xml_data.attrib["Fecha"]
            and parse(xml_data.attrib["Fecha"]).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            or False,
            # 'xml_invoice_link': xml_file_link,
            #'factura_cfdi': True,
            ...
            "selo_digital_cdfi": TimbreFiscalDigital.attrib["SelloCFD"],
            "selo_sat": TimbreFiscalDigital.attrib["SelloSAT"],
            "tipocambiop": xml_data.find("TipoCambio")
            and xml_data.attrib["TipoCambio"]
            or "1",
