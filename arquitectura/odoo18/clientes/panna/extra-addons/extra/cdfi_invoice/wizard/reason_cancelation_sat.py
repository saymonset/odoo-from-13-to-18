    ...
    def Confirmar(self):
        if (
            ...
            ctx = {
                ...
                "foliosustitucion": self.foliosustitucion or False,
            }
            return move_obj.with_context(ctx).action_cfdi_cancel()
        if (
            self.env.context.get("active_id")
            and self.env.context.get("active_model") == "account.payment"
        ):
        ...
        if (
            self.env.context.get("active_id")
            and self.env.context.get("active_model") == "cfdi.traslado"
        ):
        ...
        if (
            self.env.context.get("active_id")
            and self.env.context.get("active_model") == "factura.global"
        ):
        ...
        if (
            self.env.context.get("active_id")
            and self.env.context.get("active_model") == "hr.payslip"
        ):
