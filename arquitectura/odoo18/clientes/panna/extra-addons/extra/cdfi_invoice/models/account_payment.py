...
class AccountRegisterPayment(models.TransientModel):
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        res.update(
            {
                "fecha_pago": datetime(
                    self.payment_date.year,
                    self.payment_date.month,
                    self.payment_date.day,
                    16,
                    0,
                    tzinfo=local,
...
class AccountPayment(models.Model):
    ...
    banco_emisor = fields.Char(
        "Banco del emisor", related="cuenta_emisor.bank_name", readonly=True
    )
    rfc_banco_emisor = fields.Char(
        _("RFC banco emisor"), related="cuenta_emisor.bank_bic", readonly=True
    )
    ...
    cuenta_beneficiario = fields.Char(
        _("Cuenta beneficiario"), compute="_compute_banco_receptor"
    )
    rfc_banco_receptor = fields.Char(
        _("RFC banco receptor"), compute="_compute_banco_receptor"
    )
    estado_pago = fields.Selection(
        selection=[
            ("pago_no_enviado", "REP no generado"),
            ("pago_correcto", "REP correcto"),
    ...
    cadena_origenal = fields.Char(
        string=_("Cadena Original del Complemento digital de SAT")
    )
    ...
    payment_mail_ids = fields.One2many(
        "account.payment.mail", "payment_id", string="Payment Mails"
    )
    ...
    different_currency = fields.Boolean(
        _("Diferente moneda"), compute="_compute_different_currency"
    )
    company_cfdi = fields.Boolean(related="company_id.company_cfdi", store=True)
    ...
    def importar_incluir_cep(self):
        ...
        return {
            ...
            "view_id": self.env.ref(
                "cdfi_invoice.view_import_xml_payment_in_payment_form_view"
            ).id,
    ...
    def add_resitual_amounts(self):
        for payment in self:
            ...
            tax_grouped_tras = {}
            tax_grouped_ret = {}
            mxn_currency = self.env["res.currency"].search(
                [("name", "=", "MXN")], limit=1
            )
            ...
            if payment.reconciled_invoice_ids:
                if payment.manual_partials:
                    for partial in payment.partials_payment_ids:
                        ...
                        if equivalenciadr == 0:
                            raise UserError(
                                "La equivalencia debe ser diferente de cero."
                            )
                        ...
                        paid_pct = (
                            ...
                            / partial.facturas_id.total_factura
                        )
                        ...
                        docto_relacionados.append(
                            {
                                ...
                                "ImpuestosDR": {
                                    "traslados": trasladodr,
                                    "retenciones": retenciondr,
                                },
                            }
                        )
                    ...
                    payment.write(
                        {
                            "docto_relacionados": json.dumps(docto_relacionados),
                    ...
                    if payment.currency_id == mxn_currency:
                        ...
                        paid_amount_comp_curr = payment.currency_id.round(
                            payment.amount * rate_payment_curr_mxn
                        )
                    ...
                    for match_field in ("credit", "debit"):
                        for partial in pay_rec_lines[f"matched_{match_field}_ids"]:
                            ...
                            payment_content = (
                                len(invoice.invoice_payments_widget["content"])
                                - exchange_number
                            )
                            ...
                            if invoice.currency_id == payment.currency_id:
                                ...
                                invoice.currency_id == mxn_currency
                                and invoice.currency_id != payment.currency_id
                            ):
                                amount_paid_invoice_curr = invoice_amount
                                amount_paid_invoice_comp_curr = (
                                    payment_line.company_currency_id.round(
                                        payment.amount
                                        * (
                                            abs(payment_line.balance)
                                            / (paid_amount_comp_curr + exchange_amount)
                                        )
                                    )
                                )
                                invoice_rate = partial.debit_amount_currency / (
                                    partial.amount + exchange_amount
                                )
                                exchange_rate = (
                                    amount_paid_invoice_curr
                                    / amount_paid_invoice_comp_curr
                                )
                                equivalenciadr = (
                                    payment.roundTraditional(exchange_rate, 6)
                                    + 0.000001
                                )
                            else:
                                amount_paid_invoice_curr = invoice_amount
                                exchange_rate = partial.debit_amount_currency / (
                                    partial.amount + exchange_amount
                                )
                                equivalenciadr = payment.roundTraditional(
                                    exchange_rate, 6
                                )  # + 0.000001
                            paid_pct = (
                                float_round(
                            ...
                            if "translados" in taxes:
                                ...
                                for traslado in traslados:
                                    ...
                                    trasladodr.append(
                                        {
                                            "BaseDR": payment.set_decimals(
                                                basedr, decimal_p
                                            ),
                                            "ImpuestoDR": traslado["impuesto"],
                                            "TipoFactorDR": traslado["TipoFactor"],
                                            "TasaOcuotaDR": traslado["tasa"],
                                            "ImporteDR": payment.set_decimals(
                                                importedr, decimal_p
                                            )
                                            if traslado["TipoFactor"] != "Exento"
                                            else "",
    ...
    def to_json(self):
        ...
        if self.currency_id.name == "MXN":
            ...
            self.tipocambiop = self.set_decimals(
                1 / self.currency_id.with_context(date=self.date).rate, no_decimales_tc
            )
        ...
        if not self.fecha_pago:
            raise UserError(
                _("Falta configurar fecha de pago en la sección de CFDI del documento.")
            )
        else:
            local = pytz.timezone(timezone)
            ...
            if taxes_traslado:
                for line in taxes_traslado.values():
                    trasladop.append(
                        {
                            ...
                            "ImporteP": self.roundTraditional(line["ImporteP"], 2)
                    ...
                    if line["ImpuestoP"] == "002" and line["TasaOCuotaP"] == "0.160000":
                        totales.update(
                            {
                                "TotalTrasladosBaseIVA16": self.selectRoundseparate(
                                    line["BaseP"] * float(self.tipocambiop),
                                    2,
                                    self.redondeo_t_base,
                                ),
                                "TotalTrasladosImpuestoIVA16": self.selectRoundseparate(
                                    line["ImporteP"] * float(self.tipocambiop),
                                    2,
                                    self.redondeo_t_impuesto,
                                ),
                            }
                        )
                    if line["ImpuestoP"] == "002" and line["TasaOCuotaP"] == "0.080000":
                        totales.update(
                            {
                                "TotalTrasladosBaseIVA8": self.roundTraditional(
                    ...
                    if line["ImpuestoP"] == "002" and line["TasaOCuotaP"] == "0.000000":
                        totales.update(
                            {
                                "TotalTrasladosBaseIVA0": self.roundTraditional(
                                    line["BaseP"] * float(self.tipocambiop), 2
                                ),
                    ...
                    if line["ImpuestoP"] == "002" and line["TipoFactorP"] == "Exento":
                        totales.update(
                            {
                                "TotalTrasladosBaseIVAExento": self.roundTraditional(
                                    line["BaseP"] * float(self.tipocambiop), 2
                                ),
                            }
                        )
                    if line["TipoFactorP"] != "Exento":
                        self.total_pago += round(
                            line["BaseP"] * float(self.tipocambiop), 2
                        ) + round(line["ImporteP"] * float(self.tipocambiop), 2)
                    else:
                        self.total_pago += round(
                            line["BaseP"] * float(self.tipocambiop), 2
                        )
            ...
            if taxes_retenciones:
                for line in taxes_retenciones.values():
                    ...
                    if line["ImpuestoP"] == "002":
                        totales.update(
                            {
                                "TotalRetencionesIVA": self.roundTraditional(
                                    line["ImporteP"] * float(self.tipocambiop), 2
                                ),
                            }
                        )
                    if line["ImpuestoP"] == "001":
                        totales.update(
                            {
                                "TotalRetencionesISR": self.roundTraditional(
                                    line["ImporteP"] * float(self.tipocambiop), 2
                                ),
                            }
                        )
                    if line["ImpuestoP"] == "003":
                        totales.update(
                            {
                                "TotalRetencionesIEPS": self.roundTraditional(
                                    line["ImporteP"] * float(self.tipocambiop), 2
                                ),
                            }
                        )
        ...
        totales.update(
            {
                "MontoTotalPagos": self.set_decimals(self.amount, 2)
                if self.monedap == "MXN"
                else self.selectRoundseparate(
                    self.amount * float(self.tipocambiop), 2, self.redondeo_t_total
                ),
            }
        )
        ...
        pagos.append(
            {
                ...
                "RfcEmisorCtaOrd": self.rfc_banco_emisor
                if self.forma_pago_id.code in ["02", "03", "04", "05", "28", "29"]
                else "",
                "NomBancoOrdExt": self.banco_emisor
                if self.forma_pago_id.code in ["02", "03", "04", "05", "28", "29"]
                else "",
                "CtaOrdenante": self.cuenta_emisor.acc_number
                if self.cuenta_emisor
                and self.forma_pago_id.code in ["02", "03", "04", "05", "28", "29"]
                else "",
                "RfcEmisorCtaBen": self.rfc_banco_receptor
                if self.forma_pago_id.code in ["02", "03", "04", "05", "28", "29"]
                else "",
                "CtaBeneficiario": self.cuenta_beneficiario
                if self.forma_pago_id.code in ["02", "03", "04", "05", "28", "29"]
                else "",
        ...
        if self.reconciled_invoice_ids:
            request_params = {
                "factura": {
                    "serie": str(re.sub(r"[0-9]+", "", self.name))
                    .replace("/", "")
                    .replace(".", ""),
                    ...
                    "total": "0",
                    "tipocomprobante": "P",
                    "LugarExpedicion": self.journal_id.codigo_postal
                    or self.company_id.zip,
        ...
        for p in self:
            ...
            try:
                ...
                if (
                    ...
                    raise UserError(
                        _("Servidor fuera de servicio, favor de intentar mas tarde")
                    )
                else:
    ...
    def send_payment(self):
        ...
        if xml_file:
            ...
            attachments.append(
                (self.name.replace(".", "").replace("/", "_") + ".xml", xml_file.datas)
            )
    ...
    def action_cfdi_cancel(self):
        for p in self:
            ...
            values = {
                ...
                "serie_factura": str(re.sub(r"[0-9]+", "", p.name))
                .replace("/", "")
                .replace(".", ""),
            ...
            if json_response["estado_factura"] == "problemas_factura":
                ...
                file_name = (
                    "CANCEL_" + p.name.replace(".", "").replace("/", "_") + ".xml"
                )
                    ...
                    for res_id in res_ids:
                        ...
                        domain = [
                            ...
                            (
                                "name",
                                "=",
                                payment.name.replace(".", "").replace("/", "_")
                                + ".xml",
                            ),
                        ]
                        xml_file = self.env["ir.attachment"].search(domain, limit=1)
