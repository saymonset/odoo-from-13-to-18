...
class ResCompany(models.Model):
    ...
    regimen_fiscal_id = fields.Many2one(
        "catalogo.regimen.fiscal", string="Régimen Fiscal"
    )
    ...
    serie_complemento = fields.Char(
        string=_("Serie complemento de pago")
    )  # quitar en proxima revisión
    telefono_sms = fields.Char(
    ...
    def get_saldo_by_cron(self):
        ...
        for company in companies:
            company.get_saldo()
            if (
                company.saldo_timbres < company.saldo_alarma and company.correo_alarma
            ):  # valida saldo de timbres
                email_template = self.env.ref(
                    "cdfi_invoice.email_template_alarma_de_saldo", False
                )
            ...
            if (
                company.aviso_csd and company.fecha_csd and company.correo_alarma
            ):  # valida vigencia de CSD
                if (
                    datetime.today() - timedelta(days=int(company.aviso_csd))
                    > company.fecha_csd
                ):
                    email_template = self.env.ref(
                        "cdfi_invoice.email_template_alarma_de_csd", False
                    )
                    if not email_template:
                        return
                    emails = company.correo_alarma.split(",")
            ...
            if (
                company.fecha_timbres and company.correo_alarma
            ):  # valida vigencia de timbres
                if (
                    datetime.today() + timedelta(days=7)
                ).date() > company.fecha_timbres:
                    email_template = self.env.ref(
                        "cdfi_invoice.email_template_alarma_vencimiento", False
                    )
                    if not email_template:
                        return
                    emails = company.correo_alarma.split(",")
    ...
    def get_saldo(self):
        ...
        values2 = {
            ...
            "fecha_timbres": parser.parse(json_response["vigencia"])
            if json_response["vigencia"]
            else "",
        }
        self.update(values2)
