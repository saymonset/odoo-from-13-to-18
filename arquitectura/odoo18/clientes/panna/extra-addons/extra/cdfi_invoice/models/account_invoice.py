...
class AccountMove(models.Model):
    ...
    def to_json(self):
        ...
        for line in self.invoice_line_ids:
            ...
            taxes_prod = line.tax_ids.compute_all(
                price_wo_discount,
                line.currency_id,
                line.quantity,
            ...
            total_wo_discount = self.roundTraditional(
                line.price_unit * line.quantity - tax_included, no_decimales_prod
            )
            if promocion:
                discount_prod = (
                    self.roundTraditional(
                        (line.price_unit * line.quantity - tax_included)
                        - (line.price_subtotal - promo),
                        no_decimales_prod,
                    )
            ...
            precio_unitario = self.roundTraditional(
                (line.price_unit * line.quantity - tax_included) / line.quantity,
                no_decimales_prod,
            )
            ...
            if line.product_id.product_parts_ids:
                for component in line.product_id.product_parts_ids:
                    if not component.product_id.clave_producto:
                        raise UserError(
                            _(
                                "El producto %s tiene un componente sin clave de producto."
                            )
                            % (line.product_id.name)
                        )
                    if not component.product_id.name:
                        raise UserError(
                            _("El producto %s tiene un componente sin nombre.")
                            % (line.product_id.name)
                        )
                    components.append(
                        {
                            "ClaveProdServ": component.product_id.clave_producto,
        ...
        if tax_local_ret or tax_local_tras:
            if tax_local_tras and not tax_local_ret:
                request_params.update(
                    {
                        "implocal10": {
                            "TotaldeTraslados": self.set_decimals(
                                tax_local_tras_tot, 2
                            ),
                            "TotaldeRetenciones": self.set_decimals(
                                tax_local_ret_tot, 2
                            ),
        ...
        if self.tipo_comprobante == "T":
            ...
            request_params["factura"].update(
                {
                    "descuento": self.roundTraditional(self.discount, no_decimales),
...
Si requiere timbrar la factura nuevamente deshabilite el checkbox de "Proceso de timbrado" de la pestaña CFDI.'
                    ...
                    .create(
                        {
                            ...
                            "description": _("Factura CFDI del documento %s.")
                            % invoice.name,
                        }
                    )
                )
    ...
    def force_invoice_send(self):
        for inv in self:
            ...
            if email_act and email_act.get("context"):
                ...
                inv.with_context(email_ctx).message_post_with_source(
    ...
    def check_cancel_status_by_cron(self):
        ...
        for invoice in invoices:
            ...
            if not xml_file:
                _logger.info(
                    "No se encontró XML de la factura %s", invoice.folio_fiscal
                )
                continue
            values = {
                "rfc": invoice.company_id.vat,
