...
class ProductTemplate(models.Model):
    ...
    warehouse_quantity = fields.Char(
        compute="_get_quantity_warehouse_location", string="Quantity per warehouse"
    )
    warehouse_id = fields.Many2one(
        compute="_get_quantity_warehouse_forcast_location", string="Warehouse Name"
    )
                ...
                for location in res:
                    ...
                    while not warehouse and location1:
                        warehouse_id = (
                            ...
                            .sudo()
                            .search([("lot_stock_id", "=", location1.id)])
                        )
                ...
                if different:
                    for line in different:
                        picking_type = self.env["stock.picking.type"].search(
                            [
                                ("warehouse_id.name", "=", line.stock_location_name),
                                ("code", "=", "outgoing"),
                                ("sequence_code", "=", "POS"),
                            ]
                        )
                        ...
                        diff_pick = (
                            ...
                            .with_context(diff_loc=line.stock_location_name)
...
class Product(models.Model):
    ...
    quant_text = fields.Text(
        "Quant Qty", compute="_compute_avail_locations", store=True
    )
