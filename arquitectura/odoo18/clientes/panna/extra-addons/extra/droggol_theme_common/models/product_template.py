...
class ProductTemplate(models.Model):
    ...
    dr_product_tab_ids = fields.Many2many(
        "dr.website.content",
        "product_template_tab_rel",
        "product_template_id",
        "tab_id",
        string="Product Tabs",
    )
    dr_product_offer_ids = fields.Many2many(
        "dr.website.content",
        "product_template_offer_rel",
        "product_template_id",
        "offer_id",
        string="Product Info",
    )
    ...
    dr_brand_value_id = fields.Many2one(
        ...
        compute="_compute_dr_brand_value_id",
        inverse="_inverse_dr_brand_value_id",
        search="_search_dr_brand_value_id",
        string="Brand",
    )
    dr_brand_attribute_ids = fields.Many2many(
        "product.attribute", compute="_compute_dr_brand_attribute_ids"
    )

    dr_free_qty = fields.Float(
        "Free To Use Quantity",
        compute="_compute_dr_free_qty",
        search="_search_dr_free_qty",
        compute_sudo=False,
    ...
    def _search_dr_brand_value_id(self, operator, value):
        if operator in ["in", "not in"]:
            ...
            values = self.env["product.attribute.value"].search(
                [
                    ("name", operator, value),
                    ("attribute_id", "in", brand_attribute_id.ids),
                ]
            )
    ...
    def _compute_dr_brand_value_id(self):
        for product in self:
            brand_lines = product.attribute_line_ids.filtered(
                lambda x: x.attribute_id.dr_is_brand
            )
    ...
    def _compute_dr_free_qty_quantities_dict(self):
        ...
        variants_available = {
            p["id"]: p
            for p in self.sudo()
            .with_context(warehouse_id=website.warehouse_id.id)
            .product_variant_ids.read(["free_qty"])
        }
        prod_available = {}
        for template in self:
            free_qty = 0
            ...
            if config.get("mode") == "real":
                domain = [
                    ...
                    (
                        ...
                        ">=",
                        fields.Datetime.now()
                        - relativedelta(hours=config.get("duration", 24)),
                    ),
                ]
                return {
                    product.id: int(qty) if (qty % 1) == 0 else qty
                    for product, qty in self.env["sale.report"]
                    .sudo()
                    ._read_group(
                        domain=domain,
                ...
                return {
                    product_id.id: self._get_random_number(
                        config.get("fake_min_threshold", 1),
                        config.get("fake_max_threshold", 15),
                        product_id.id,
                    )
            ...
            if config.get("mode") == "real":
                results = (
                    ...
                    ._read_group(
                        [
                            ...
                            (
                                "visitor_id.last_connection_datetime",
                                ">",
                                fields.datetime.now() - relativedelta(minutes=5),
                            ),
                        ],
                        ["visitor_id"],
                ...
                for visitor_id, product_ids in results:
                    for product_tmpl_id in self:
                        if set(product_tmpl_id.product_variant_ids.ids).intersection(
                            product_ids
                        ):
                ...
                return {
                    product_id.id: self._get_random_number(
                        config.get("fake_min_threshold", 1),
                        config.get("fake_max_threshold", 8),
                        product_id.id,
                        "per_minute",
                    )
                    for product_id in self
    ...
    def _get_random_number(
        self, min_value=1, max_value=10, unique_int=0, duration="per_hour"
    ):
        total_seed = None
        ...
        for ptav in self.dr_ptav_ids:
            if ptav.ptav_active and ptav.ptav_product_variant_ids:
                vals = {
                    "id": ptav.id,
                    "name": ptav.name,
                    "preview_image": "/web/image/product.product/%s"
                    % ptav.ptav_product_variant_ids.ids[0],
                }
                if ptav.dr_thumb_image:
                    vals.update(
                        {
                            "type": "image",
                            "value": "/web/image/product.template.attribute.value/%s/dr_thumb_image"
                            % ptav.id,
                        }
                    )
                    ...
                    vals.update(
                        {
                            "type": "image",
                            "value": "/web/image/product.template.attribute.value/%s/image"
                            % ptav.id,
                        }
                    )
                    swatches.append(vals)
    ...
    def _get_combination_info(
        ...
        if website and website_has_theme_prime:
            ...
            if combination_info["product_id"]:
                product_variant_id = self.env["product.product"].browse(
                    combination_info["product_id"]
                )
                # Product Price Offer
                combination_info["product_price_offer"] = (
                    product_variant_id._get_product_pricelist_offer()
                )
                # Bulk Price
                if (
                    website._get_dr_theme_config("bool_show_bulk_price")
                    and combination_info.get("has_b2b_access")
                    and combination_info.get("is_combination_possible")
                ):
                    ...
                    bulk_price = []
                    all_rule_ids = website.pricelist_id._get_applicable_rules(
                        product_variant_id, fields.Datetime.now()
                    ).filtered(lambda x: x.min_quantity)
                    applicable_rule_ids = self.env["product.pricelist.item"]
                    for rule in all_rule_ids:
                        if not applicable_rule_ids.filtered(
                            lambda x: x.min_quantity == rule.min_quantity
                        ):
                            applicable_rule_ids += rule

                    for rule_id in applicable_rule_ids.sorted(lambda x: x.min_quantity):
                        price = rule_id._compute_price(
                            product_variant_id,
                            rule_id.min_quantity,
                            product_variant_id.uom_id,
                            fields.Datetime.now(),
                            website.currency_id,
                        )
                        price = ProductTemplate._apply_taxes_to_price(
                            price,
                            website.currency_id,
                        ...
                        bulk_price.append(
                            {
                                ...
                                "formatted_qty": IrQwebFieldFloat.value_to_html(
                                    rule_id.min_quantity,
                                    {"decimal_precision": "Product Unit of Measure"},
                                ),  # TO-REMOVE
                                "price": price,
                                "formatted_price": IrQwebFieldMonetary.value_to_html(
                                    price, {"display_currency": website.currency_id}
                                ),
                                "saving_price": IrQwebFieldMonetary.value_to_html(
                                    (list_price - price) * rule_id.min_quantity,
                                    {"display_currency": website.currency_id},
                                )
                                if rule_id.sudo()._show_discount()
                                else False,
                ...
                combination_info["tp_extra_fields"] = IrUiView._render_template(
                    "theme_prime.product_extra_fields",
                    values={
                        "website": website,
                        "product_variant": product_variant_id,
                        "product": product_variant_id.product_tmpl_id,
                    },
                )
