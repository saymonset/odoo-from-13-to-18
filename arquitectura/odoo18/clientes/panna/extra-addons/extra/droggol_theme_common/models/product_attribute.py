...
class ProductAttribute(models.Model):
    ...
    display_type = fields.Selection(
        ...
        ondelete={
            "radio_circle": "cascade",
            "radio_square": "cascade",
            "radio_image": "cascade",
        },
    )
    dr_is_show_shop_search = fields.Boolean(
        "Show Searchbar in Shop Filter", default=False
    )
    dr_attribute_popup_id = fields.Many2one(
        "dr.website.content",
        string="Guide",
        domain='[("content_type", "=", "attribute_popup")]',
    )
    ...
    dr_search_suggestion = fields.Selection(
        [("auto", "Autocomplete"), ("auto_suggestion", "Autocomplete & Suggestion")],
        string="Search suggestion type",
    )
    ...
    def _constrains_dr_is_brand(self):
        for attribute in self:
            if attribute.dr_is_brand and not attribute.display_type == "radio_image":
                raise UserError(
                    _('Brand attribute must have display type "Radio Image".')
                )
...
class ProductTemplateAttributeValue(models.Model):
    ...
    dr_image = fields.Binary(
        "Value Image", related="product_attribute_value_id.dr_image"
    )
