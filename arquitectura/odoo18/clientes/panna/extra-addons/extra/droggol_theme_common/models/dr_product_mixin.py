...
class DrProductsMixin(models.AbstractModel):
    ...
    dr_show_out_of_stock = fields.Char(
        compute="_compute_dr_show_out_of_stock", compute_sudo=True
    )
    ...
    def _compute_dr_show_out_of_stock(self):
        ...
        for product in self:
            ...
            if (
                website
                and website._get_dr_theme_config("json_shop_product_item").get(
                    "show_stock_label"
                )
                ...
                free_qty = (
                    product.with_context(warehouse_id=website.warehouse_id.id).free_qty
                    if product._name == "product.product"
                    else product.dr_free_qty
                )
                if (
                    product.show_availability
                    and free_qty <= product.available_threshold
                ):
            ...
            if price_rule_id:
                ...
                if price_rule_id.date_end:
                    return {
                        ...
                        "date_end": price_rule_id.date_end.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    }
        return False
