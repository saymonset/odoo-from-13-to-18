...
class DrSearchReport(models.Model):
    ...
    clicked_type = fields.Selection(
        [
            ("product", "Product Clicked"),
            ("brand", "Brand"),
            ("category", "Category Clicked"),
            ("autocomplete", "Autocomplete Clicked"),
            ("suggestion", "Suggestion Clicked"),
            ("submit", "Submited"),
        ],
        string="Action",
        readonly=True,
    ...
    def _add_report_entry(self, report_data):
        ...
        report_data["lang_id"] = (
            self.env["res.lang"]._lang_get(self._context.get("lang")).id
        )
    ...
    def _cron_auto_delete_search_report(self):
        for website in self.env["website"].search([]):
            delete_days = website._get_dr_theme_config("json_product_search").get(
                "delete_search_report"
            )
            if delete_days:
                last_date = fields.Datetime.now() + timedelta(days=-delete_days)
                self.search(
                    [("website_id", "=", website.id), ("search_time", "<", last_date)]
                ).unlink()
