...
class Website(models.Model):
    ...
    dr_sale_special_offer = fields.Html(
        "Sale Special Offer",
    ...
    dr_product_tab_ids = fields.Many2many(
        "dr.website.content",
        "website_product_tab_rel",
        "website_id",
        "tab_id",
        string="Product Tabs",
    )
    dr_product_info_ids = fields.Many2many(
        "dr.website.content",
        "website_product_info_rel",
        "website_id",
        "info_id",
        string="Product Info",
    )
    ...
    dr_pwa_screenshots = fields.One2many(
        "dr.pwa.screenshots", "website_id", string="Screenshots"
    )
    dr_pwa_shortcuts = fields.One2many(
        "dr.pwa.shortcuts", "website_id", string="Shortcuts"
    )
    ...
    def _get_brand_attributes(self):
        ...
        current_website_products = self.env["product.template"].search(
            website.sale_product_domain()
        )
        return self.env["product.attribute"].search(
            [
                ("product_tmpl_ids", "in", current_website_products.ids),
                ("dr_is_brand", "=", True),
            ]
        )
    ...
    def _get_current_pricelist(self):
        if self._get_dr_theme_config("json_b2b_shop_config")[
    ...
    def get_theme_prime_shop_config(self):
        ...
        return {
            "is_rating_active": Website.sudo()
            .viewref("website_sale.product_comment")
            .active,
            "is_buy_now_active": Website.sudo()
            .viewref("website_sale.product_buy_now")
            .active,
            "is_multiplier_active": Website.sudo()
            .viewref("website_sale.product_quantity")
            .active,
            "is_wishlist_active": Website.sudo()
            .viewref("website_sale_wishlist.product_add_to_wishlist")
            .active,
            "is_comparison_active": Website.sudo()
            .viewref("website_sale_comparison.add_to_compare")
            .active,
        }

    def _get_website_category(self):
        return self.env["product.public.category"].search(
            [("website_id", "in", [False, self.id]), ("parent_id", "=", False)]
        )

    def _get_theme_prime_rating_template(self, rating_avg, rating_count=False):
        return self.env["ir.qweb"]._render(
            "theme_prime.d_rating_widget_stars_static",
            values={
    ...
    def get_theme_prime_bottom_bar_action_buttons(self):
        ...
        return {
            ...
            "tp_search": {
                "name": _("Search"),
                "icon": "dri dri-search",
                "action_class": "tp-search-sidebar-action",
            },
            "tp_wishlist": {
                "name": _("Wishlist"),
                "icon": "dri dri-wishlist",
                "url": "/shop/wishlist",
            },
            ...
            "tp_category": {
                "name": _("Category"),
                "icon": "dri dri-category",
                "action_class": "tp-category-action",
            },
            "tp_orders": {
                "name": _("Orders"),
    ...
    def _search_get_details(self, search_type, order, options):
        ...
        if search_type in ["category_synonyms"]:
            result.append(
                self.env["dr.category.synonyms"]._search_get_detail(
                    self, order, options
                )
            )
        return result
