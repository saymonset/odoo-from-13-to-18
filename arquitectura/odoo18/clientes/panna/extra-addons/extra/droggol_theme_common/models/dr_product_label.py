...
class DrProductLabel(models.Model):
    ...
    background_color_rgb = fields.Char(
        compute="_compute_background_color_rgb",
        store=True,
        string="Background Color RGB",
    )
    text_color = fields.Char("Text Color", default="#FFFFFF")
    style = fields.Selection(
        [("1", "Tag"), ("2", "Badge"), ("3", "Circle"), ("4", "Square")],
        default="1",
        required=True,
    )
    ...
    def _compute_product_count(self):
        label_data = self.env["product.template"].read_group(
            [("dr_label_id", "in", self.ids)], ["dr_label_id"], ["dr_label_id"]
        )
        mapped_data = dict(
            [(x["dr_label_id"][0], x["dr_label_id_count"]) for x in label_data]
        )
        for label in self:
    ...
    def _compute_background_color_rgb(self):
        for label in self:
            ...
            label.background_color_rgb = "%s, %s, %s" % (
                colors[0],
                colors[1],
                colors[2],
            )
