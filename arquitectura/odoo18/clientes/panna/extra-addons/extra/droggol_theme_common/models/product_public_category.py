...
class ProductPublicCategory(models.Model):
    ...
    dr_category_label_id = fields.Many2one(
        "dr.product.public.category.label", string="Label"
    )
    ...
    dr_search_formulate = fields.Boolean(
        "Formulated Search",
        help="Use to search multi level categories \
        e.g. Men Shirt (Here men and shirt are different category but will be displayed as one in smart search)",
    )
    ds_name = fields.Char(
        "Search DS Name", compute="_compute_ds_name", search="_search_ds_name"
    )
    ...
    def _compute_ds_name(self):
        for category in self:
            if self.env.context.get("dr_formulate"):
                category.ds_name = " ".join(
                    [
                        categ.name
                        for categ in category.parents_and_self
    ...
    def _search_ds_name(self, operator, value):
        ...
        categ_ids = [
            categ.id
            for categ in self.search(domain)
            if re.search(re.escape(value), categ.ds_name, re.IGNORECASE)
        ]
    ...
    def _get_search_db(self, website):
        search_db = self._get_category_search_tuples(
            self._context.get("lang"), website.id
        )
        ...
        for category in categories:
            ...
            if has_formulate:
                ...
                for categ in parents:
                    if category.id == categ.id or categ.dr_search_formulate:
                        cartesian_list.append(
                            [categ.name] + categ.mapped("ds_synonyms_ids.name")
                        )
                ...
                for synonym in category.ds_synonyms_ids:
                    _add_entry(
                        synonym.name,
                        synonym.category_id.id,
                        category.dr_search_formulate,
                    )
...
class DrPublicCategorySynonyms(models.Model):
    ...
    website_id = fields.Many2one(
        "website", related="category_id.website_id", store=True
    )
    dr_search_formulate = fields.Boolean(
        related="category_id.dr_search_formulate", store=True
    )
    ds_name = fields.Char(
        "Search DS Name", compute="_compute_ds_name", search="_search_ds_name"
    )
    ...
    def _compute_ds_name(self):
        for synonym in self:
            if self.env.context.get("dr_formulate"):
                categ_formulate_names = [
                    categ.name
                    for categ in (
                        synonym.category_id.parents_and_self - synonym.category_id
                    )
                    if (synonym.id == categ.id or categ.dr_search_formulate)
                ]
