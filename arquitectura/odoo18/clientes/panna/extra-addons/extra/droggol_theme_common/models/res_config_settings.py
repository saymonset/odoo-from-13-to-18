...
class ResConfigSettings(models.TransientModel):
    ...
    dr_pwa_activated = fields.Boolean(
        related="website_id.dr_pwa_activated", readonly=False
    )
    dr_pwa_name = fields.Char(related="website_id.dr_pwa_name", readonly=False)
    dr_pwa_short_name = fields.Char(
        related="website_id.dr_pwa_short_name", readonly=False
    )
    dr_pwa_background_color = fields.Char(
        related="website_id.dr_pwa_background_color", readonly=False
    )
    dr_pwa_theme_color = fields.Char(
        related="website_id.dr_pwa_theme_color", readonly=False
    )
    dr_pwa_icon_192 = fields.Binary(
        related="website_id.dr_pwa_icon_192", readonly=False
    )
    dr_pwa_icon_512 = fields.Binary(
        related="website_id.dr_pwa_icon_512", readonly=False
    )
    dr_pwa_start_url = fields.Char(
        related="website_id.dr_pwa_start_url", readonly=False
    )
    dr_pwa_screenshots = fields.One2many(
        related="website_id.dr_pwa_screenshots", readonly=False
    )
    dr_pwa_shortcuts = fields.One2many(
        related="website_id.dr_pwa_shortcuts", readonly=False
    )
    dr_pwa_show_install_banner = fields.Boolean(
        related="website_id.dr_pwa_show_install_banner", readonly=False
    )
    dr_pwa_offline_page = fields.Boolean(
        related="website_id.dr_pwa_offline_page", readonly=False
    )
    ...
    def _compute_dr_has_custom_module(self):
        ...
        for setting in self:
            ...
            if (
                setting.website_id
                and setting.website_id.theme_id
                and setting.website_id.theme_id.name in themes
            ):
                search_term = "%s_%%" % setting.website_id.theme_id.name
                has_custom_apps = IrModuleModule.sudo().search(
    ...
    def dr_open_pwa_screenshots(self):
        ...
        action = self.env.ref("droggol_theme_common.dr_pwa_screenshots_action").read()[
            0
        ]
    ...
    def dr_open_theme_custom_modules(self):
        ...
        if (
            self.website_id
            and self.website_id.theme_id
            and self.website_id.theme_id.name in themes
        ):
