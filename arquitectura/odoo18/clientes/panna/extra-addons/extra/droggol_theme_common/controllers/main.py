...
class DroggolThemeCommon(http.Controller):
    @http.route(
        ['/droggol_theme_common/design_content/<model("dr.website.content"):content>'],
        type="http",
        website=True,
        auth="user",
    )
    def design_content(self, content, **post):
        return request.render(
            "droggol_theme_common.design_content",
            {"content": content, "no_header": True, "no_footer": True},
        )


class DroggolThemeCommonSaleProductConfiguratorController(
    WebsiteSaleProductConfiguratorController
):
    def _get_product_information(
        ...
        **kwargs,
        ...
        result = super()._get_product_information(
            product_template,
            combination,
            currency,
            pricelist,
