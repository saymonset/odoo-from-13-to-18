...
def delete_views(domain, env):
    ...
    _logger.info(
        "DRGL-MIG: (%s) Views to delete: %s"
        % (len(views_to_delete.ids), views_to_delete.ids)
    )
    if views_to_delete:
        child_views = views_to_delete.mapped("inherit_children_ids")
        _logger.info(
            "DRGL-MIG: (%s) Child Views to delete: %s"
            % (len(child_views.ids), child_views.ids)
        )
        child_views.unlink()
    dr_views_to_delete = views_to_delete.filtered(
        lambda v: not len(v.inherit_children_ids)
    )
...
def deactivate_assets(domain, env):
    ...
    _logger.info(
        "DRGL-MIG: (%s) assets to delete: %s"
        % (len(assets_to_remove.ids), assets_to_remove.ids)
    )
    if assets_to_remove:
...
def migrate(cr, version):
    ...
    for product_id in env["product.template"].search(
        [("dr_document_ids", "!=", False)]
    ):
        for attachment_id in product_id.dr_document_ids:
            new_id = attachment_id.with_context(
                disable_product_documents_creation=True
            ).copy({"res_id": product_id.id, "res_model": "product.template"})
            document_id = ProductDocument.with_context(
                disable_product_documents_creation=True
            ).create(
                {
