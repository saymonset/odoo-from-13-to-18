...
class ResConfigSettings(models.TransientModel):
    ...
    activate_toppings = fields.Boolean(
        related="pos_config_id.activate_toppings", readonly=False
    )
    add_topping_default = fields.Boolean(
        related="pos_config_id.add_topping_default", readonly=False
    )
...
class pos_order(models.Model):
    ...
    def _process_preparation_changes(
        self, cancelled=False, general_note=None, note_history=None
    ):
        self.ensure_one()
        flag_change = False
        sound = False
        ...
        for pdis_line in pdis_lines:
            key = (
                ...
                json.dumps(pdis_line.line_topping_ids.ids),
            )
        ...
        for line in self.lines.filtered(lambda li: not li.skip_change):
            ...
            key = (
                line.product_id.id,
                line_note,
                json.dumps(line.attribute_value_ids.ids),
                line.uuid,
                json.dumps(line.line_topping_ids.ids),
            )
        ...
        if note_history:
            for line in pdis_lines[::-1]:
                ...
                for note in note_history.get(product_id, []):
                    if (
                        note["uuid"] == line.pos_order_line_uuid
                        and line.internal_note == note["old"]
                        and note["qty"] > 0
                        and line.product_quantity
                        <= note["qty"] - note.get("used_qty", 0)
                    ):
        ...
        if any(
            [
                quantities["order"] > quantities["display"]
                for quantities in quantity_data.values()
            ]
        ):
            flag_change = True
            sound = True
        ...
        product_ids = self.env["product.product"].browse(
            [data["product_id"] for data in quantity_data.values()]
        )
        for data in quantity_data.values():
            ...
            if data["order"] > data["display"]:
                ...
                filtered_lines = self.lines.filtered(
                    lambda li: li.product_id.id == product_id
                    and (li.note or "") == data["note"]
        ...
        if general_note is not None:
            for order in pdis_order:
                if order.pdis_general_note != general_note:
                    ...
                    flag_change = True
                    category_ids.update(
                        pdis_lines[0].product_id.pos_categ_ids.ids
                    )  # necessary to send when only ordernote changed
...
class POSSession(models.Model):
    ...
    def _pos_data_process(self, loaded_data):
        ...
        loaded_data["topping_group_by_id"] = {
            ppp["id"]: ppp for ppp in loaded_data["topping.groups"]
        }
        topping_prods = {}
        for prods in loaded_data["product.product"]:
            if prods.get("is_topping", False):
                topping_prods.update({prods["id"]: prods})
