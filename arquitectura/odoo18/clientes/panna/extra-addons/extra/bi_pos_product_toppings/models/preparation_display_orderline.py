    ...
    def _export_for_ui(self, preparation_display):
        ...
        if preparation_display_orderlines:
            ...
            return {
                ...
                "stage_id": current_order_stage.stage_id.id
                if current_order_stage
                else None,
                "last_stage_change": current_order_stage.write_date
                if current_order_stage
                else self.create_date,
...
class PosPreparationDisplay(models.Model):
    ...
    def get_preparation_display_data(self):
        return {
            "categories": self._get_pos_category_ids().read(
                ["id", "display_name", "sequence"]
            ),
            "stages": self.stage_ids.read(),
            "orders": self.env[
                "pos_preparation_display.order"
            ].get_preparation_display_order(self.id),
            "attributes": self.env["product.attribute"].search([]).read(["id", "name"]),
            "all_products": self.env["product.product"]
            .search([])
            .read(["id", "display_name"]),
            "attribute_values": self.env["product.template.attribute.value"]
            .search([])
            .read(["id", "name", "attribute_id"]),
        }
