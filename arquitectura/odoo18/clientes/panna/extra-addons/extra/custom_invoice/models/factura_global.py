...
class FacturaglobalLine(models.Model):
    ...
    factura_global_id = fields.Many2one(
        comodel_name="factura.global", string="Factura Global"
    )
    ...
    invoice_line_tax_ids = fields.Many2many(
        "account.tax", string="Impuestos", domain="[('type_tax_use', '=', 'sale')]"
    )
    currency_id = fields.Many2one(
        "res.currency",
        related="factura_global_id.currency_id",
        store=True,
        related_sudo=False,
        readonly=False,
    )
    ...
    discount = fields.Float(
        string="Descuento", digits=dp.get_precision("Product Price")
    )
    ...
    @api.depends(
        ...
        "quantity",
        "product_id",
        "factura_global_id.partner_id",
        "factura_global_id.currency_id",
    )
    def _compute_price(self):
        for line in self:
            currency = (
                line.factura_global_id and line.factura_global_id.currency_id or None
            )
            ...
            line.price_subtotal = (
                taxes["total_excluded"] if taxes else line.quantity * price
            )
    ...
    def _prepare_base_line_for_taxes_computation(self, **kwargs):
        ...
        return self.env["account.tax"]._prepare_base_line_for_taxes_computation(
            ...
            **{
                ...
                "currency_id": self.factura_global_id.currency_id
                or self.factura_global_id.company_id.currency_id,
...
class Facturaglobal(models.Model):
    ...
    factura_line_ids = fields.One2many(
        "factura.global.line",
        "factura_global_id",
        string="Factura global line",
        copy=True,
    )
    ...
    fecha_certificacion = fields.Char(
        string=_("Fecha y Hora Certificación"), copy=False
    )
    cadena_origenal = fields.Char(
        string=_("Cadena Origenal del Complemento digital de SAT"), copy=False
    )
    ...
    discount = fields.Float(
        string="Discount (%)", digits=dp.get_precision("Product Price")
    )
    monto = fields.Float(string="Amount", digits=dp.get_precision("Product Price"))
    precio_unitario = fields.Float(
        string="Precio unitario", digits=dp.get_precision("Product Price")
    )
    monto_impuesto = fields.Float(
        string="Monto impuesto", digits=dp.get_precision("Product Price")
    )
    ...
    def copy(self, default=None):
        ...
        if (
            ...
            or self.estado_factura == "factura_cancelada"
        ):
    ...
    def _compute_amount(self):
        for invoice in self:
            ...
            invoice.amount_untaxed = sum(
                line.price_subtotal for line in invoice.factura_line_ids
            )
            invoice.amount_total = sum(
                round_curr(line.price_total) for line in invoice.factura_line_ids
            )
    ...
    @api.depends(
        "factura_line_ids.price_unit",
        "factura_line_ids.quantity",
        "factura_line_ids.invoice_line_tax_ids",
    )
    def _compute_tax_totals(self):
        ...
        for order in self:
            ...
            base_lines = [
                ...
                for line in order.factura_line_ids
            ]
    ...
    def to_json(self):
        ...
        if (
            self.partner_id.vat == "XAXX010101000"
            or self.partner_id.vat == "XEXX010101000"
        ):
        ...
        if self.currency_id.name == "MXN":
            ...
            tipocambio = self.set_decimals(
                1 / self.currency_id.with_context(date=self.invoice_date).rate,
                no_decimales_tc,
            )

        request_params = {
            "factura": {
                "serie": str(re.sub("[^a-z]", "", self.number))
                + str(re.sub("[^A-Z]", "", self.number)),
            ...
            "receptor": {
                ...
                "ResidenciaFiscal": self.partner_id.country_id.codigo_mx
                if self.partner_id.country_id.code != "MX"
                else "",
                "NumRegIdTrib": self.partner_id.vat.upper()
                if self.partner_id.country_id.code != "MX"
                else "",
        ...
        for line in self.factura_line_ids:
            if line.quantity <= 0:
                raise UserError(
                    _("El producto %s tiene cantidades negativas, se debe corregir.")
            ...
            if not line.product_id.clave_producto:
                ...
                self.env.cr.commit()
                raise UserError(
                    _("El producto %s no tiene clave del SAT configurado.")
                    % (line.product_id.name)
                )
            if not line.product_id.cat_unidad_medida.clave:
                ...
                self.env.cr.commit()
                raise UserError(
                    _("El producto %s no tiene unidad de medida del SAT configurado.")
                    % (line.product_id.name)
                )
            promo = 0
            promocion = False
            ...
            if negative_lines:
                ...
                for promo_disc in negative_lines:
                    if promo_disc <= line.price_subtotal:
                        ...
                        price_wo_discount = round(
                            line.price_unit - (promo_disc / line.quantity),
                            no_decimales_prod,
                        )
                        promo = promo_disc
                        promocion = True
                ...
                price_wo_discount = round(
                    line.price_unit * (1 - (line.discount / 100.0)), no_decimales_prod
                )

            taxes_prod = line.invoice_line_tax_ids.compute_all(
                price_wo_discount,
                line.currency_id,
                line.quantity,
                product=line.product_id,
            ...
            for taxes in taxes_prod["taxes"]:
                ...
                if not tax.impuesto:
                    ...
                    self.env.cr.commit()
                    raise UserError(
                        _("El impuesto %s no tiene clave del SAT configurado.")
                        % (tax.name)
                    )
                if not tax.tipo_factor:
                    ...
                    self.env.cr.commit()
                    raise UserError(
                        _("El impuesto %s no tiene tipo de factor del SAT configurado.")
                        % (tax.name)
                    )
                if tax.impuesto != "004":
                    key = taxes["id"]
                    ...
                    if taxes["amount"] >= 0.0:
                        if tax.tipo_factor == "Exento":
                            tax_tras.append(
                                {
                                    "Base": self.set_decimals(
                                        taxes["base"], no_decimales_prod
                                    ),
                            ...
                            tax_tras.append(
                                {
                                    "Base": self.set_decimals(
                                        line.quantity, no_decimales_prod
                                    ),
                                    "Impuesto": tax.impuesto,
                                    "TipoFactor": tax.tipo_factor,
                                    "TasaOCuota": self.set_decimals(tax.amount, 6),
                                    "Importe": self.set_decimals(
                                        taxes["amount"], no_decimales_prod
                                    ),
                                }
                            )
                        else:
                        ...
                        tax_ret.append(
                            {
                                ...
                                "TipoFactor": tax.tipo_factor,
                                "TasaOCuota": self.set_decimals(
                                    tax.amount / 100.0 * -1, 6
                                )
                                if str(tax.amount) != "-1.25"
                                else self.set_decimals(tax.amount / 100.0 * -1, 4),
                                "Importe": self.set_decimals(
                                    taxes["amount"] * -1, no_decimales_prod
                                ),
                            }
                        )
                    ...
                    if taxes["amount"] >= 0.0:
                        ...
                        tax_local_tras.append(
                            {
                                "ImpLocTrasladado": tax.impuesto_local,
                        ...
                        tax_local_ret.append(
                            {
                                "ImpLocRetenido": tax.impuesto_local,
                                "TasadeRetencion": self.set_decimals(
                                    tax.amount * -1, 2
                                ),
            ...
            if promocion:
                ...
                discount_prod = (
                    round(
                        total_wo_discount - (line.price_subtotal - promo),
                        no_decimales_prod,
                    )
                    if line.discount or promo > 0
                    else 0
                )
                ...
                discount_prod = (
                    round(total_wo_discount - line.price_subtotal, no_decimales_prod)
                    if line.discount
                    else 0
                )
                precio_unitario = round(
            ...
            if product_string == "":
                if line.name.find("]") > 0:
                    product_string = (
                        line.name[line.name.find("[") + len("[") : line.name.find("]")]
                        or ""
                    )
                ...
                invoice_lines.append(
                    {
                        ...
                        "valorunitario": self.set_decimals(
                            precio_unitario, no_decimales_prod
                        ),
                        "importe": self.set_decimals(
                            total_wo_discount, no_decimales_prod
                        ),
                ...
                invoice_lines.append(
                    {
                        ...
                        "Descuento": self.set_decimals(
                            discount_prod, no_decimales_prod
                        ),
                        "ObjetoImp": objetoimp,
                    }
                )
                    ...
                    traslados.append(
                        {
                            ...
                            "TipoFactor": tax.tipo_factor,
                            "tasa": tasa_tr,
                            "importe": self.roundTraditional(
                                line["amount"], no_decimales
                            )
                            if tax.tipo_factor != "Exento"
                            else "",
                ...
                impuestos.update(
                    {
                        ...
                        "TotalImpuestosTrasladados": self.set_decimals(
                            tras_tot, no_decimales
                        )
                        if not only_exento
                        else "",
                    }
                    ...
                    retenciones.append(
                        {
                            "impuesto": tax.impuesto,
                            "TipoFactor": tax.tipo_factor,
                            "tasa": self.set_decimals(
                                float(tax.amount) / 100.0 * -1, 6
                            ),
                            "importe": self.roundTraditional(
                                line["amount"] * -1, no_decimales
                            ),
                            "base": self.roundTraditional(line["base"], no_decimales),
                            "tax_id": line["tax_id"],
                        }
                ...
                impuestos.update(
                    {
                        ...
                        "TotalImpuestosRetenidos": self.set_decimals(
                            ret_tot, no_decimales
                        ),
                    }
                )
        ...
        if tax_local_ret or tax_local_tras:
            if tax_local_tras and not tax_local_ret:
                request_params.update(
                    {
                        "implocal10": {
                            "TotaldeTraslados": tax_local_tras_tot,
                            "TotaldeRetenciones": tax_local_ret_tot,
                ...
                request_params.update(
                    {
                        "implocal10": {
                            "TotaldeTraslados": tax_local_tras_tot,
                            "TotaldeRetenciones": tax_local_ret_tot * -1,
        ...
        if self.tipo_comprobante == "T":
            ...
            request_params["factura"].update(
                {
                    "descuento": self.roundTraditional(discount, no_decimales),
                    "subtotal": self.roundTraditional(subtotal, no_decimales),
                    "total": self.roundTraditional(
                        subtotal
                        + tras_tot
                        - ret_tot
                        - discount
...
Si requiere timbrar la factura nuevamente deshabilite el checkbox de "Proceso de timbrado" de la pestaña CFDI.'
            ...
            if invoice.company_id.proveedor_timbrado == "servidor":
                ...
                raise UserError(
                    _(
                        "Error, falta seleccionar el servidor de timbrado en la configuración de la compañía."
                    )
                )
        ...
        for invoice in self:
            if invoice.source_document:
                ...
                if (
                    ...
                    .sudo()
                    .get_param("custom_invoice.fg_code")
                ):
        ...
        for invoice in self:
            if invoice.factura_cfdi:
                ...
                values = {
                    ...
                    "serie_factura": invoice.journal_id.serie_diario
                    or invoice.company_id.serie_factura,
        ...
        for invoice in self:
            if invoice.factura_cfdi:
                if (
                    ...
                    or invoice.estado_factura == "solicitud_cancelar"
                ):
    ...
    def check_cancel_status_by_cron(self):
        ...
        for invoice in invoices:
            ...
            if invoice.company_id.proveedor_timbrado == "servidor":
                ...
                url = "%s" % (
                    "https://facturacion2.itadmin.com.mx/api/consulta-cacelar"
                )
            else:
                raise UserError(
                    _(
            ...
            if estado_factura == "problemas_consulta":
                _logger.info(
                    "Error en la consulta %s", json_response["problemas_message"]
                )
    ...
    def liberar_cfdi(self):
        for invoice in self:
            values = {
                ...
                "serie_factura": str(re.sub("[^a-z]", "", invoice.number))
                + str(re.sub("[^A-Z]", "", invoice.number)),
        ...
        for rec in self:
            if self.model == "factura.global":
                ...
                template_id = self.env.ref(
                    "custom_invoice.email_template_factura_global"
                )
                    ...
                    for res_id in res_ids:
                        ...
                        domain = [
                            ...
                            (
                                "name",
                                "=",
                                fact_global.number.replace("/", "_") + ".xml",
                            ),
                        ]
                        xml_file = self.env["ir.attachment"].search(domain, limit=1)
                        if xml_file:
