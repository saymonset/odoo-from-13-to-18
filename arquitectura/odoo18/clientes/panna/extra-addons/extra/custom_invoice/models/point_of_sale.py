...
class PosConfig(models.Model):
    ...
    default_customer = fields.Many2one(
        "res.partner", string=_("Cliente Default"), domain=[("customer", "=", True)]
    )
...
class PosOrder(models.Model):
    ...
    factura_global_id = fields.Many2one(
        "factura.global", string="Factura Global", readonly=True, store=True
    )
    ...
    def get_invoice_information(self, name, session, sequence):
        order = self.search(
            [("session_id", "=", session), ("sequence_number", "=", sequence)], limit=1
        )
            ...
            cfdi_vals.update(
                {
                    ...
                    "regimen_fiscal": invoice.company_id.regimen_fiscal_id.description
                    or "",
                    ...
                    and invoice.invoice_date.strftime("%Y-%m-%d %H:%M:%S")
                    or "",
                    "folio_factura": invoice.name.replace("INV", "").replace("/", "")
                    or "",
                    "uso_cfdi_id": invoice.uso_cfdi_id.description or "",
                    "client_name": invoice.partner_id.name or "",
                    "client_rfc": invoice.partner_id.vat or "",
                    "client_regime": invoice.partner_id.regimen_fiscal_id.description
                    or "",
                }
            )
        else:
    ...
    def sync_from_ui(self, orders):
        ...
        for order in orders:
            if (
                order.get("to_invoice", False)
                and self.env["pos.session"]
                .browse(order["session_id"])
                .company_id.country_id.code
                == "MX"
            ):
                order["uso_cfdi"] = order.get("uso_cfdi", False)
                order["uso_cfdi_id"] = (
                    self.env["catalogo.uso.cfdi"]
                    .sudo()
                    .search([("code", "=", order.get("uso_cfdi", False))], limit=1)
                    .id
                )
                ...
                if amount_paid > 0:
                    if payment["amount"] > amount_paid:
                        metodo_pago_pos_id = (
                            ...
                            .sudo()
                            .search(
                                [("id", "=", payment["payment_method_id"])], limit=1
                            )
                        )
                else:
                    payment_method = (
                        self.env["pos.payment.method"]
                        .sudo()
                        .search([("id", "=", payment["payment_method_id"])], limit=1)
                    )
    ...
    def action_invoice(self, partner_total=None):
        ...
        sale_journal = self[
            0
        ].session_id.config_id.invoice_journal_id  # self[0].sale_journal
        ...
        for order in self:
            ...
            if partner_total and not self.env["ir.config_parameter"].sudo().get_param(
                "custom_invoice.fg_client"
            ):
            ...
            if group_key not in invoices:
                ...
                order.sudo().write(
                    {"account_move": new_invoice.id, "state": "invoiced"}
                )
                ...
                invoice_obj = (
                    ...
                    .browse(invoices[group_key])
                )
                order.sudo().write(
                    {"account_move": invoice_obj.id, "state": "invoiced"}
                )
                if order_name not in invoice_obj.invoice_origin:
                    invoice_obj.write(
                        {
                            "invoice_origin": invoice_obj.invoice_origin
                            + ", "
                            + order_name
                        }
                    )
    ...
    def action_invoice_compacta(self, partner_total=None):
            ...
            for order_line in order.lines:
                if not all_order_lines:
                    ...
                    for data in all_order_lines:
                        if (
                            data.get("product_id") == order_line.product_id.id
                            and data.get("tax_ids")[0][2]
                            == order_line.tax_ids_after_fiscal_position.ids
                        ):
        ...
        for order in self:
            ...
            if group_key not in invoices:
                ...
                invoice_obj = Invoice.with_context(local_context).browse(
                    invoices[group_key]
                )
    ...
    def _process_order(self, order, existing_order):
        ...
        if not draft:
            try:
                ...
                _logger.error(
                    "No pudo procesar completamente el pedido de venta: %s", str(e)
                )
        ...
        if pos_order.to_invoice and pos_order.state == "paid":
            ...
            # Add BY Nilesh
            pos_order.account_move.update(
                {
                    "forma_pago_id": pos_order.forma_pago_id.id,
    ...
    def action_invoice_total(
        self, product_total=None, partner_total=None, invoice_format=None
    ):
        Invoice = self.env["account.move"]
            ...
            for line in order.lines:
                if invoice_format == "cfdi":
                    if (line.tax_ids_after_fiscal_position, order) not in taxes_group:
                        taxes_group.update(
                            {(line.tax_ids_after_fiscal_position, order): line}
                        )
                    else:
                    ...
                    if (line.tax_ids_after_fiscal_position, self) not in taxes_group:
                        taxes_group.update(
                            {(line.tax_ids_after_fiscal_position, self): line}
                        )
                    else:
                        taxes_group[(line.tax_ids_after_fiscal_position, self)] += line

                invoice_line_vals = order.with_context(
                    local_context
                )._prepare_invoice_line(line)
                move_lines.append((0, 0, invoice_line_vals))
            if order.pos_reference:
                note += "%s, %s;" % (
                    order.pos_reference.replace("Pedido ", ""),
                    order.amount_total,
                )
            else:
        ...
        if taxes_group:
            ...
            inv = {
                ...
                "currency_id": self[
                    0
                ].pricelist_id.currency_id.id,  # considering partner's sale pricelist's currency
                ...
                if order_taxes[1]:
                    if order_taxes[1][0].pos_reference:
                        inv_line_name = (
                            ...
                            and "[%s] %s"
                            % (
                                order_taxes[1].pos_reference.replace("Pedido ", ""),
                                product_total.name,
                            )
                            or product_total.name
                        )
                    else:
                        inv_line_name = (
                            (invoice_format == "cfdi")
                            and "[%s] %s" % (order_taxes[1][0].name, product_total.name)
                            or product_total.name
                        )
                inv_line = {
                    "move_id": invoice.id,
    ...
    def action_factura_global(self, partner_total=None):
        ...
        for order in self:
            ...
            if group_key not in invoices:
                new_invoice = (
                    factura.with_context(local_context).sudo().create(factura_dic)
                )
                order.write({"factura_global_id": new_invoice.id, "state": "invoiced"})
                invoices[group_key] = new_invoice.id
                factura += new_invoice
            elif group_key in invoices:
                invoice_obj = factura.with_context(local_context).browse(
                    invoices[group_key]
                )
                ...
                if invoice_obj.source_document:
                    if order_name not in invoice_obj.source_document.split(", "):
                        invoice_obj.write(
                            {
                                "source_document": invoice_obj.source_document
                                + ", "
                                + order_name
                            }
                        )
        ...
        fg_nc_create = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("custom_invoice.fg_nc_create")
        )
        if fg_nc_create:
    ...
    def action_factura_global_total(
        self, product_total=None, partner_total=None, invoice_format=None
    ):
        factura = self.env["factura.global"]
        inv_ids = []
            ...
            for order_taxes, order_lines in taxes_group.items():
                ...
                if order_taxes[1]:
                    if order_taxes[1][0].pos_reference:
                        ...
                        inv_line_name = (
                            (invoice_format == "cfdi")
                            and "[%s] %s" % (order_name, product_total.name)
                            or product_total.name
                        )
                    else:
                        if (
            ...
            for line in order.lines:
                if not all_factura_line:
                    ...
                    for data in all_factura_line:
                        if (
                            data.get("product_id") == line.product_id.id
                            and data.get("tax_ids")[0][2]
                            == line.tax_ids_after_fiscal_position.ids
                        ):
    ...
    def _prepare_invoice_line(self, order_line):
        ...
        name = order_line.product_id.with_context(
            lang=order_line.order_id.partner_id.lang or self.env.user.lang
