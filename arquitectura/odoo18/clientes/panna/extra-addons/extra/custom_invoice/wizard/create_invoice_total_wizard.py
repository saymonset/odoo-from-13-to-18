...
class CerateInvoiceTotalWizard(models.TransientModel):
    ...
    def action_validate_invoice_total(self):
        ...
        if self.date_from:
            domain.append(
                ("date_order", ">=", self.date_from.strftime("%Y-%m-%d %H:%M:%S"))
            )
        if self.date_to:
            domain.append(
                ("date_order", "<=", self.date_to.strftime("%Y-%m-%d %H:%M:%S"))
            )
        ...
        self.write(
            {
                "order_num": len(order_ids),
                "total": amount_total,
                "oreder_ids": [(6, 0, order_ids)],
            }
        )
        return {
    ...
    def action_create_invoice_total(self):
        ...
        if self.invoice_format in ["one", "cfdi"]:
            orders.action_factura_global_total(
                product_total=self.product_id,
                partner_total=self.partner_id,
