...
class account_invoice(models.Model):
    ...
    def invoice_validate(self):
        ...
        for invoice in self:
            self.env.cr.execute(
                "delete from ir_attachment where name like 'Invoice_%s' and res_id = %s and res_model='account.move';"
                % (
                    "%",
                    invoice.id,
                )
...
class website_self_invoice_web(models.Model):
    ...
    state = fields.Selection(
        [("draft", "Borrador"), ("error", "Error"), ("done", "Relizado")]
    )

    attachment_ids = fields.One2many(
        "website.self.invoice.web.attach", "website_auto_id", "Adjuntos del Portal"
    )
    ...
    def website_form_input_filter(self, request, values):
        values["medium_id"] = (
            ...
            or self.sudo()
            .env["ir.model.data"]
            .xmlid_to_res_id("utm.utm_medium_website")
        )
        return values
    ...
    def create(self, values):
        ...
        if not result.partner_id:
            ...
            try:
                ...
                result.write(
                    {
                        "error_message": "El RFC %s no existe en la Base de Datos."
                        % result.rfc_partner,
                        "state": "error",
                    }
                )
        ...
        try:
            ...
            if enable_pos:
                ...
                try:
                    ...
                    result.write(
                        {
                            "error_message": "El Ticket %s no existe en la Base de Datos."
                            % result.order_number,
                            "state": "error",
                        }
                    )
                ...
                result.write(
                    {
                        "error_message": "El Pedido %s no existe en la Base de Datos."
                        % result.order_number,
                        "state": "error",
                    }
                )
        ...
        if order_id and ticket_pos == False:
            ...
            if order_br.state in ("draft", "sent"):
                result.write(
                    {
                        "error_message": "El Pedido %s se encuentra en espera de ser procesado, por favor comuniquese con la compañia."
                        % order_br.name,
                        "state": "error",
                    }
                )
            ...
            if order_br.invoice_status != "no":
                ...
                if order_br.invoice_status == "invoiced":
                    invoice_return = order_br.invoice_ids.filtered(
                        lambda r: r.state != "cancel"
                    )
                    if invoice_return and invoice_return[0].estado_factura in [
                        "factura_correcta",
                        "factura_cancelada",
                    ]:
                        result.write(
                            {
                                "error_message": "El Pedido %s ya fue Facturado."
                                % result.order_number,
                                "state": "error",
                            }
                        )
                    ...
                    if not result.uso_cfdi:
                        result.write(
                            {
                                "error_message": "No tiene asignado un USO DE CFDI en su cuenta de usuario. Favor de asignar uno antes de facturar el pedido %s."
                                % order_br.name,
                                "state": "error",
                            }
                        )
                    ...
                    if not order_br.forma_pago_id:
                        result.write(
                            {
                                "error_message": "El pedido %s no pudo facturarse ya que no cuenta con una forma de pago asignada, comuniquese con la compañia."
                                % order_br.name,
                                "state": "error",
                            }
                        )
                    ...
                    if not order_br.methodo_pago:
                        result.write(
                            {
                                "error_message": "El pedido %s no pudo facturarse ya que no cuenta con un método de pago asignado, comuniquese con la compañia."
                                % order_br.name,
                                "state": "error",
                            }
                        )
                ...
                vals.update(
                    {"fecha_factura": (datetime.datetime.now() - timedelta(hours=6))}
                )
                ...
                try:
                    ...
                    result.write(
                        {
                            "error_message": "El pedido %s no se pudo timbrar correctamente, favor de contactar al departamento de facturación para asistirlo."
                            % result.order_number,
                            "state": "error",
                        }
                    )
                ...
                attachment_ids = ir_attach.search(
                    [("res_model", "=", "account.move"), ("res_id", "=", invoice_br.id)]
                )
                Attachment = self.env["ir.attachment"].sudo()

                if not attachment_ids.filtered(
                    lambda x: ".pdf" in x.store_fname or ".pdf" in x.name
                ):
                    report = (
                        self.env["ir.actions.report"]
                        .with_user(SUPERUSER_ID)
                        ._render_qweb_pdf("account.account_invoices", invoice_br.id)[0]
                    )
                ...
                if not attachment_ids.filtered(
                    lambda x: ".xml" in x.store_fname or ".xml" in x.name
                ):
        ...
        if order_id and ticket_pos == True:
            ...
            if pos_br.state != "cancel" and pos_br.state != "draft":
                ...
                factglob_return = (
                    self.env["factura.global"]
                    .sudo()
                    .search(
                        [
                            ("source_document", "=", pos_br.name),
                            ("state", "!=", "cancel"),
                        ],
                        limit=1,
                    )
                    ...
                    invoice_br.write(
                        {
                            "uso_cfdi_id": self.env["catalogo.uso.cfdi"]
                            .sudo()
                            .search([("code", "=", result.uso_cfdi)], limit=1)
                            .id,
                            "forma_pago_id": self.env["catalogo.forma.pago"]
                            .sudo()
                            .search([("code", "=", result.forma_pago)], limit=1)
                            .id,
                        },
                    )
                    tryagain = True
                ...
                if pos_br.state == "invoiced" and not tryagain:
                    result.write(
                        {
                            "error_message": "El ticket %s ya fue facturado previamente."
                            % (result.order_number,),
                            "state": "error",
                        }
                    )
                ...
                if not result.uso_cfdi:
                    result.write(
                        {
                            "error_message": "No tiene asignado un USO DE CFDI. Favor de asignar uno antes de facturar el pedido %s."
                            % result.order_number,
                            "state": "error",
                        }
                    )
                ...
                if not result.forma_pago:
                    result.write(
                        {
                            "error_message": "No tiene asignada la FORMA DE PAGO. Favor de asignar uno antes de facturar el pedido %s."
                            % result.order_number,
                            "state": "error",
                        }
                    )
                ...
                if invoice_br.estado_factura != "factura_correcta":
                    result.write(
                        {
                            "error_message": "El ticket %s no se pudo timbrar correctamente, favor de contactar al departamento de facturación para asistirlo."
                            % result.order_number,
                            "state": "error",
                        }
                    )
                ...
                if not attachment_ids.filtered(
                    ...
                    report = (
                        ...
                        .with_user(SUPERUSER_ID)
                        ._render_qweb_pdf(
                            "custom_invoice.report_facturaglobals", invoice_br.id
                        )[0]
                    )
                    report = base64.b64encode(report)
                ...
                if attachment_ids:
                    ...
                    result.write({"state": "done"})
                    email_template = self.env.ref(
                        "custom_invoice.email_template_factura_global", False
                    )
                    if email_template:
                ...
                result.write(
                    {
                        "error_message": "El Ticket %s no se puede facturar, favor de contactar al departamento de facturación."
                        % result.order_number,
                        "state": "error",
                    }
                )
